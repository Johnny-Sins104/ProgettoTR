...
6. La funzione `print_paper_once_console_summary` is stata modificata in `core/paper_once_console_summary.py` per stampare con `flush=True` in modo deterministico su stdout (o stream personalizzato) ed includere i campi `prompt` e `footer_source`.
