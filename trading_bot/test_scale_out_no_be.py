...
def simulate_backtest_custom(
    ...
    tp1_rr: float,
    use_be_on_tp1: bool,
    be_sl_ratio: float = 0.0 # 0.0 means breakeven, -0.5 means half loss, etc.
) -> dict:
    ...
    for i in range(1, len(rows)):
        ...
        if open_trade is not None:
            ...
            if not tp1_hit:
                ...
                if hit_tp1:
                    ...
                    if use_be_on_tp1:
                        # Muovi Stop Loss della seconda metà a Breakeven o livello parziale
                        # be_sl_ratio = 0.0 -> entry + commissions
                        # be_sl_ratio = -0.5 -> entry - 0.5 * (entry - initial_sl)
                        if be_sl_ratio == 0.0:
        ...
        if open_trade is None and pending_trigger is not None:
            ...
            if triggered:
                ...
                # Sovrascriviamo tp1 con il nostro tp1_rr di test
                if side == "BUY":
...
async def main():
    ...
    print("\n[TEST] Analisi Strategia Scale-Out: Impatto del Trailing a Breakeven")
    print("==========================================================================")
    print(f"{'TP1_RR':<8}{'Trailing SL type':<24}{'Win Rate %':<12}{'Saldo LOW':<14}{'Trades':<8}")
    print("--------------------------------------------------------------------------")
    
    for tp1_rr in [1.5, 2.0, 2.5]:
        # Scenario A: Trailing SL a Breakeven (0.0)
        res_a = simulate_backtest_custom(df, 0.03, 4.0, tp1_rr, use_be_on_tp1=True, be_sl_ratio=0.0)
        print(f"{tp1_rr:<8.1f}{'Breakeven (0.0)':<24}{res_a['win_rate']:<12.1f}{res_a['final_balance']:<14.2f}{res_a['total_trades']:<8}")
        
        # Scenario B: Trailing SL a meta perdita (-0.5x R:R)
        res_b = simulate_backtest_custom(df, 0.03, 4.0, tp1_rr, use_be_on_tp1=True, be_sl_ratio=-0.5)
        print(f"{tp1_rr:<8.1f}{'Meta perdita (-0.5x)':<24}{res_b['win_rate']:<12.1f}{res_b['final_balance']:<14.2f}{res_b['total_trades']:<8}")
        
        # Scenario C: Nessun Trailing (mantiene Stop Loss iniziale)
        res_c = simulate_backtest_custom(df, 0.03, 4.0, tp1_rr, use_be_on_tp1=False)
        print(f"{tp1_rr:<8.1f}{'Nessuno (SL originale)':<24}{res_c['win_rate']:<12.1f}{res_c['final_balance']:<14.2f}{res_c['total_trades']:<8}")
