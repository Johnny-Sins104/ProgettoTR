...
from trading_bot.core.lsr_v2_third_trade_submit_execution import (
    EVENT_TYPE,
    EXECUTED_DECISION,
    NOT_ARMED_DECISION,
    ...
    build_lsr_v2_third_trade_submit_execution_event,
    build_lsr_v2_third_trade_submit_execution_report_from_files,
)
from trading_bot.core.lsr_v2_third_trade_submit_boundary import LSRV2ThirdTradeSubmitBoundarySettings
...
def _payload(cycle_id: str = "pc_third") -> dict:
    return {
        "cycle_id": cycle_id,
        "symbol": "BTC/USDT",
        "timeframe": "5m",
...
def _boundary_event(cycle_id: str = "pc_third", *, ready: bool = True) -> dict:
    ...
    return {
        ...
        "third_trade_submit_ready": ready,
        "third_trade_submit_armed": True,
        "third_trade_submit_confirmation_ok": True,
        "would_submit": False,
...
def _boundary_report(cycle_id: str = "pc_third", *, ready: bool = True) -> dict:
    return {
        ...
        "third_trade_submit_ready_count": 1 if ready else 0,
        "third_trade_submit_armed": True,
        "third_trade_submit_confirmation_ok": True,
...
def _write_boundary(tmp_path: Path, cycle_id: str = "pc_third", *, ready: bool = True) -> None:
    (tmp_path / "lsr_v2_third_trade_submit_boundary_report.json").write_text(json.dumps(_boundary_report(cycle_id, ready=ready)), encoding="utf-8")
    _append_jsonl(tmp_path / "lsr_v2_third_trade_submit_boundary.jsonl", _boundary_event(cycle_id, ready=ready))
    _write_clean_state(tmp_path)


def _fake_submitter(payload: dict):
    return {"order_submitted": True, "position_opened": True, "order_id": "ord_third", "paper_broker_adapter": "FakePaperBrokerAdapter"}
...
def test_execution_event_calls_fake_submitter_when_all_controls_ok() -> None:
    execution_settings = LSRV2ThirdTradeSubmitExecutionSettings(
        execute_arm="1",
        execute_confirmation="I_UNDERSTAND_EXECUTE_THIRD_PAPER_ORDER_ONLY",
        max_orders=1,
    )
    boundary_settings = LSRV2ThirdTradeSubmitBoundarySettings(
        submit_arm="1",
        submit_confirmation="I_UNDERSTAND_THIRD_SINGLE_PAPER_ORDER",
        max_orders=1,
    )
    event = build_lsr_v2_third_trade_submit_execution_event(
    ...
    assert event["trade_ordinal"] == 3
    assert event["broker_submit_called"] is True
    assert event["orders_submitted_by_third_trade_execution"] == 1
    assert event["positions_opened_by_third_trade_execution"] == 1
    assert event["positions_closed_by_third_trade_execution"] == 0
...
def test_report_blocks_when_not_armed(tmp_path: Path) -> None:
    ...
    report = build_lsr_v2_third_trade_submit_execution_report_from_files(data_dir=tmp_path)

    assert report["status"] == "WARN"
    assert report["decision"] == NOT_ARMED_DECISION
    assert report["orders_submitted_by_third_trade_execution"] == 0
    assert report["positions_opened_by_third_trade_execution"] == 0
...
def test_report_blocks_when_execution_confirmation_missing(tmp_path: Path, monkeypatch) -> None:
    _write_boundary(tmp_path)
    monkeypatch.setenv("LSR_V2_THIRD_TRADE_SUBMIT_ARM", "1")
    monkeypatch.setenv("LSR_V2_THIRD_TRADE_SUBMIT_CONFIRMATION", "I_UNDERSTAND_THIRD_SINGLE_PAPER_ORDER")
    monkeypatch.setenv("LSR_V2_THIRD_TRADE_EXECUTE", "1")

    report = build_lsr_v2_third_trade_submit_execution_report_from_files(data_dir=tmp_path)

    assert report["status"] == "WARN"
    assert report["decision"] == CONFIRMATION_MISSING_DECISION
    assert report["third_trade_execute_armed"] is True
    assert report["third_trade_execute_confirmation_ok"] is False
    assert report["broker_submit_called"] is False
...
def test_report_blocks_when_submitter_not_available(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("LSR_V2_THIRD_TRADE_SUBMIT_CONFIRMATION", "I_UNDERSTAND_THIRD_SINGLE_PAPER_ORDER")
    monkeypatch.setenv("LSR_V2_THIRD_TRADE_EXECUTE", "1")
    monkeypatch.setenv("LSR_V2_THIRD_TRADE_EXECUTE_CONFIRMATION", "I_UNDERSTAND_EXECUTE_THIRD_PAPER_ORDER_ONLY")

    report = build_lsr_v2_third_trade_submit_execution_report_from_files(data_dir=tmp_path, allow_project_submitter=False)
...
def test_report_executes_one_order_with_injected_submitter(tmp_path: Path, monkeypatch) -> None:
    ...
    report = build_lsr_v2_third_trade_submit_execution_report_from_files(data_dir=tmp_path, paper_submitter=_fake_submitter)

    assert report["status"] == "PASS"
    assert report["decision"] == EXECUTED_DECISION
    assert report["orders_submitted_by_third_trade_execution"] == 1
    assert report["positions_opened_by_third_trade_execution"] == 1
    assert report["positions_closed_by_third_trade_execution"] == 0
...
def test_max_order_cap_blocks_execution(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("LSR_V2_THIRD_TRADE_EXECUTE", "1")
    monkeypatch.setenv("LSR_V2_THIRD_TRADE_EXECUTE_CONFIRMATION", "I_UNDERSTAND_EXECUTE_THIRD_PAPER_ORDER_ONLY")
    monkeypatch.setenv("LSR_V2_THIRD_TRADE_MAX_ORDERS", "2")
