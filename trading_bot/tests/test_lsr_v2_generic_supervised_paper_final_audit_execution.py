...
def _write_markers(root: Path) -> None:
    ...
    (core / "lsr_v2_generic_supervised_paper_final_audit_execution_scaffold.py").write_text(
...
def _u36_report() -> dict:
    scaffold_map = {
        ...
        "final_audit_execution_scaffold_context": {
        ...
        "final_audit_execution_scaffold_gate_model": {
            "final_audit_execution_scaffold_model_ready": True,
            "final_audit_preflight_ready": True,
            "future_final_audit_execution_conditions_satisfied": False,
    ...
    report = {
        "prompt": "29.4.4u-36",
        "status": "PASS",
        "decision": "LSR_V2_GENERIC_SUPERVISED_PAPER_FINAL_AUDIT_EXECUTION_SCAFFOLD_READY",
        "generic_supervised_paper_final_audit_execution_scaffold_ready": True,
        "generic_final_audit_execution_scaffold_ready": True,
        ...
        "final_audit_execution_scaffold_diagnostic_count": 401,
        "final_audit_execution_scaffold_contract_shape_modelable": True,
        "final_audit_execution_scaffold_contract_required_fields": REQUIRED,
        "final_audit_execution_scaffold_contract_present_fields": REQUIRED,
        "final_audit_execution_scaffold_contract_missing_fields": [],
...
def test_final_audit_execution_ready_but_no_runtime_execution_without_receipts(tmp_path, monkeypatch):
    for key in list(os.environ):
        if key.startswith("LSR_V2"):
            monkeypatch.delenv(key, raising=False)
    _write_markers(tmp_path)
    _write_json(tmp_path / "data/lsr_v2_generic_supervised_paper_final_audit_execution_scaffold_report.json", _u36_report())

    report = run_generic_supervised_paper_final_audit_execution(_settings(tmp_path))

    assert report["status"] == "PASS"
    assert report["decision"] == READY_DECISION
    assert report["generic_supervised_paper_final_audit_execution_ready"] is True
    assert report["generic_final_audit_execution_model_ready"] is True
    assert report["generic_final_audit_execution_patch_ready"] is True
    assert report["generic_final_audit_execution_gate_model_ready"] is True
    assert report["paper_final_audit_execution_model_ready"] is True
    assert report["final_audit_execution_diagnostic_available"] is True
    assert report["final_audit_execution_diagnostic_count"] == 401
    assert report["final_audit_execution_contract_shape_modelable"] is True
    assert report["final_audit_execution_contract_missing_fields"] == []
    ...
    assert report["realized_pnl_written"] is False
    ...
    assert report["orders_submitted_by_generic_final_audit_execution"] == 0
    assert report["positions_closed_by_generic_final_audit_execution"] == 0
    assert report["paper_state_modified_by_generic_final_audit_execution"] is False
    assert report["paper_status_modified_by_generic_final_audit_execution"] is False
    assert report["live_enabled"] is False
    assert report["testnet_enabled"] is False
    assert report["exchange_broker_enabled"] is False
    execution_map = report["generic_supervised_paper_final_audit_execution_map"]
    assert execution_map["read_only"] is True
    assert execution_map["final_audit_execution_gate_model"]["final_audit_execution_model_ready"] is True
    assert execution_map["final_audit_execution_gate_model"]["final_audit_execution_allowed"] is False
    assert "broker_close_receipt_absent" in execution_map["final_audit_execution_gate_model"]["final_audit_execution_blocked_reasons"]


def test_final_audit_execution_requires_upstream_scaffold_report(tmp_path):
    _write_markers(tmp_path)
    ...
    assert report["generic_supervised_paper_final_audit_execution_ready"] is False
    assert report["final_audit_execution_diagnostic_available"] is False
    assert report["generic_final_audit_execution_allowed"] is False
    assert report["would_run_final_audit"] is False


def test_final_audit_execution_rejects_not_ready_upstream_report(tmp_path):
    _write_markers(tmp_path)
    upstream = _u36_report()
    upstream["decision"] = "NOT_READY"
    _write_json(tmp_path / "data/lsr_v2_generic_supervised_paper_final_audit_execution_scaffold_report.json", upstream)
...
def test_final_audit_execution_requires_source_markers(tmp_path):
    _write_json(tmp_path / "data/lsr_v2_generic_supervised_paper_final_audit_execution_scaffold_report.json", _u36_report())
...
def test_final_audit_execution_operator_env_cannot_override_missing_real_receipts(tmp_path, monkeypatch):
    _write_markers(tmp_path)
    _write_json(tmp_path / "data/lsr_v2_generic_supervised_paper_final_audit_execution_scaffold_report.json", _u36_report())
    ...
    assert report["generic_submit_operator_gate_satisfied"] is True
    assert report["generic_close_operator_gate_satisfied"] is True
    assert report["generic_realized_pnl_operator_gate_satisfied"] is True
