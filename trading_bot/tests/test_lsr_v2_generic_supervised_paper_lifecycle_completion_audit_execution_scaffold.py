...
from trading_bot.core.lsr_v2_generic_supervised_paper_lifecycle_completion_audit_execution_scaffold import (
    KEEP_DIAGNOSTIC_DECISION,
    READY_DECISION,
    Settings,
    run_generic_supervised_paper_lifecycle_completion_audit_execution_scaffold,
)
...
def _u41_report() -> dict:
    ...
    report = {
        "decision": "LSR_V2_GENERIC_SUPERVISED_PAPER_LIFECYCLE_COMPLETION_AUDIT_PREFLIGHT_READY",
        ...
        "generic_lifecycle_completion_audit_preflight_patch_ready": True,
        ...
        "lifecycle_completion_audit_preflight_contract_present_fields": REQUIRED,
        "lifecycle_completion_audit_preflight_contract_required_fields": REQUIRED,
        "lifecycle_completion_audit_preflight_contract_missing_fields": [],
        "generic_supervised_paper_lifecycle_completion_audit_preflight_map": preflight_map,
...
def test_lifecycle_completion_execution_scaffold_ready_but_runtime_completion_blocked(tmp_path, monkeypatch):
    ...
    _write_json(tmp_path / "data/lsr_v2_generic_supervised_paper_lifecycle_completion_audit_preflight_report.json", _u41_report())

    report = run_generic_supervised_paper_lifecycle_completion_audit_execution_scaffold(_settings(tmp_path))

    assert report["status"] == "PASS"
    assert report["decision"] == READY_DECISION
    assert report["generic_supervised_paper_lifecycle_completion_audit_execution_scaffold_ready"] is True
    assert report["generic_lifecycle_completion_audit_execution_scaffold_ready"] is True
    assert report["generic_lifecycle_completion_audit_execution_scaffold_model_ready"] is True
    assert report["generic_lifecycle_completion_audit_execution_scaffold_patch_ready"] is True
    assert report["paper_lifecycle_completion_audit_execution_scaffold_ready"] is True
    assert report["lifecycle_completion_audit_execution_scaffold_diagnostic_available"] is True
    assert report["lifecycle_completion_audit_execution_scaffold_diagnostic_count"] == 401
    assert report["lifecycle_completion_audit_execution_scaffold_contract_shape_modelable"] is True
    assert report["lifecycle_completion_audit_execution_scaffold_contract_missing_fields"] == []
    ...
    assert report["orders_submitted_by_generic_lifecycle_completion_audit_execution_scaffold"] == 0
    assert report["positions_closed_by_generic_lifecycle_completion_audit_execution_scaffold"] == 0
    assert report["paper_state_modified_by_generic_lifecycle_completion_audit_execution_scaffold"] is False
    scaffold_map = report["generic_supervised_paper_lifecycle_completion_audit_execution_scaffold_map"]
    assert scaffold_map["lifecycle_completion_audit_execution_scaffold_patch"] is True
    assert scaffold_map["scaffold_only"] is True
    gate = scaffold_map["lifecycle_completion_audit_execution_scaffold_gate_model"]
    assert gate["lifecycle_completion_audit_execution_scaffold_model_ready"] is True
    assert gate["lifecycle_completion_audit_execution_allowed"] is False
    assert "postmortem_runtime_execution_absent" in gate["lifecycle_completion_audit_execution_scaffold_blocked_reasons"]
    assert "explicit_lifecycle_completion_audit_execution_patch_required" in gate["lifecycle_completion_audit_execution_scaffold_blocked_reasons"]
...
def test_lifecycle_completion_execution_scaffold_requires_upstream_preflight_report(tmp_path):
    ...
    assert report["generic_supervised_paper_lifecycle_completion_audit_execution_scaffold_ready"] is False
    assert report["lifecycle_completion_audit_execution_scaffold_diagnostic_available"] is False
    assert report["generic_lifecycle_completion_audit_execution_allowed"] is False
    assert report["would_run_lifecycle_completion_audit"] is False
...
def test_lifecycle_completion_execution_scaffold_rejects_not_ready_upstream_report(tmp_path):
    _write_markers(tmp_path)
    upstream = _u41_report()
    upstream["decision"] = "NOT_READY"
    _write_json(tmp_path / "data/lsr_v2_generic_supervised_paper_lifecycle_completion_audit_preflight_report.json", upstream)
...
def test_lifecycle_completion_execution_operator_env_cannot_override_missing_runtime_evidence(tmp_path, monkeypatch):
    ...
    assert report["paper_lifecycle_completion_ready"] is False
    assert report["would_run_lifecycle_completion_audit"] is False
    reasons = report["generic_supervised_paper_lifecycle_completion_audit_execution_scaffold_map"]["lifecycle_completion_audit_execution_scaffold_gate_model"]["lifecycle_completion_audit_execution_scaffold_blocked_reasons"]
