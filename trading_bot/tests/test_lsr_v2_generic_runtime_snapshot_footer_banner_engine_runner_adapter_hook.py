...
def _seed_project(root: Path) -> None:
    ...
    for filename, decision in upstream.items():
        ...
        if filename == "lsr_v2_generic_paper_cycle_runtime_snapshot_hook_report.json":
            ...
            payload["generic_runtime_snapshot_artifact_ready"] = True
        if filename == "lsr_v2_generic_runtime_snapshot_footer_banner_wiring_hook_report.json":
            ...
            payload["generic_runtime_snapshot_footer_banner_read_only_wiring_hook_ready"] = True
        if filename == "lsr_v2_generic_runtime_snapshot_footer_banner_engine_runner_adapter_preflight_report.json":
            payload["generic_runtime_snapshot_footer_banner_engine_runner_adapter_preflight_ready"] = True
            payload["generic_engine_runner_adapter_map"] = {
                "adapter_payload_bridge": {
                    "runtime_snapshot_visible_read_only": True,
                    "runtime_snapshot_sections": ["generic_cycle_state", "safety_state"],
                    "runner_footer_payload": {"generic_lsr_v2_runtime_snapshot": "READY_READ_ONLY"},
                    "launcher_banner_payload": {"generic_lsr_v2_runtime_snapshot": "READY_READ_ONLY"},
                    "console_visibility_payload": {"runtime_snapshot_visible": True},
                    "safety_payload": {"orders_submitted": 0, "positions_opened": 0, "positions_closed": 0},
                }
            }
...
def test_read_only_adapter_hook_passes_and_writes_artifacts(tmp_path: Path, monkeypatch) -> None:
    _seed_project(tmp_path)
    monkeypatch.delenv("LSR_V2_GENERIC_REARM_ENABLE", raising=False)

    result = run_generic_runtime_snapshot_footer_banner_engine_runner_adapter_hook(Settings(project_root=tmp_path))

    assert result["status"] == "PASS"
    assert result["decision"] == READY_DECISION
    assert result["generic_runtime_snapshot_footer_banner_engine_runner_read_only_adapter_hook_ready"] is True
    assert result["engine_read_only_adapter_hook_ready"] is True
    assert result["paper_runner_adapter_hook_ready"] is True
    assert result["launcher_adapter_hook_ready"] is True
    assert result["console_visibility_adapter_hook_ready"] is True
    assert result["runtime_snapshot_visible_read_only"] is True
    assert result["generic_runtime_snapshot_footer_banner_engine_runner_read_only_adapter_execution_allowed"] is False
    ...
    assert result["orders_submitted_by_engine_runner_read_only_adapter_hook"] == 0
    assert result["positions_opened_by_engine_runner_read_only_adapter_hook"] == 0
    assert result["paper_state_modified_by_engine_runner_read_only_adapter_hook"] is False
    assert (tmp_path / "data/lsr_v2_generic_runtime_snapshot_footer_banner_engine_runner_adapter_hook_report.json").exists()
    assert (tmp_path / "data/lsr_v2_generic_runtime_snapshot_footer_banner_engine_runner_adapter_hook.jsonl").exists()


def test_missing_upstream_report_warns(tmp_path: Path) -> None:
    _seed_project(tmp_path)
    (tmp_path / "data/lsr_v2_generic_runtime_snapshot_footer_banner_engine_runner_adapter_preflight_report.json").unlink()

    result = run_generic_runtime_snapshot_footer_banner_engine_runner_adapter_hook(Settings(project_root=tmp_path))

    assert result["status"] == "WARN"
    assert "missing_upstream_reports" in result["blockers"]
    assert result["generic_runtime_snapshot_footer_banner_engine_runner_read_only_adapter_hook_ready"] is False
    assert result["generic_runtime_snapshot_footer_banner_engine_runner_read_only_adapter_execution_allowed"] is False
