...
from trading_bot.core.lsr_v2_second_trade_route_preflight import (
    ...
    REJECT_DECISION,
    LSRV2SecondTradeRoutePreflightSettings,
    build_lsr_v2_second_trade_route_preflight_report_from_files,
)
...
def _rearm(cycle_id: str = "pc_second", *, ready: bool = True, live: bool = False) -> dict:
    return {
        "status": "PASS",
        "decision": "LSR_V2_SECOND_TRADE_REARM_READY_DIAGNOSTIC" if ready else "LSR_V2_SECOND_TRADE_CANDIDATE_WAIT_GATE_READY",
        "cycle_id": cycle_id,
        "second_trade_eligible": True,
        "candidate_wait_gate_ready": True,
        "second_trade_rearm_ready": ready,
        "second_trade_rearm_enabled": True,
        "second_trade_rearm_confirmation_ok": True,
...
def _bridge(cycle_id: str = "pc_second", *, would_route: bool = False) -> dict:
    return {
...
def _seed(tmp_path: Path, *, clean: bool = True, rearm_ready: bool = True, live: bool = False, candidate: bool = True) -> Path:
    data = tmp_path / "data"
    _write_json(data / "lsr_v2_second_trade_eligibility_gate_report.json", _eligibility())
    _write_json(data / "lsr_v2_second_trade_rearm_gate_report.json", _rearm(ready=rearm_ready, live=live))
    ...
    _append_jsonl(events, _bridge("pc_second", would_route=False))
    return data


def _settings(*, enable: bool, confirmation: bool = True, max_orders: int = 1) -> LSRV2SecondTradeRoutePreflightSettings:
    return LSRV2SecondTradeRoutePreflightSettings(
        route_enable="1" if enable else "",
        route_confirmation="I_UNDERSTAND_SECOND_PAPER_TRADE_ROUTE_ONLY" if confirmation else "WRONG",
...
def test_route_confirmation_required_even_with_candidate(tmp_path: Path) -> None:
    data = _seed(tmp_path)
    report = build_lsr_v2_second_trade_route_preflight_report_from_files(data_dir=data, settings=_settings(enable=False))
    assert report["decision"] == OPERATOR_CONFIRMATION_MISSING_DECISION
    assert report["second_trade_route_preflight_ready"] is False
    assert report["orders_submitted_by_second_trade_route_preflight"] == 0
    assert report["positions_opened_by_second_trade_route_preflight"] == 0


def test_confirmation_wrong_blocks(tmp_path: Path) -> None:
    data = _seed(tmp_path)
    report = build_lsr_v2_second_trade_route_preflight_report_from_files(data_dir=data, settings=_settings(enable=True, confirmation=False))
    assert report["decision"] == OPERATOR_CONFIRMATION_MISSING_DECISION
    assert report["second_trade_route_confirmation_ok"] is False


def test_rearm_missing_blocks(tmp_path: Path) -> None:
    data = _seed(tmp_path, rearm_ready=False)
    report = build_lsr_v2_second_trade_route_preflight_report_from_files(data_dir=data, settings=_settings(enable=True))
...
def test_candidate_missing_blocks(tmp_path: Path) -> None:
    ...
    assert "second_trade_candidate_missing" in report["blockers"]
...
def test_route_preflight_pass_builds_second_trade_order_intent(tmp_path: Path) -> None:
    data = _seed(tmp_path)
    report = build_lsr_v2_second_trade_route_preflight_report_from_files(data_dir=data, settings=_settings(enable=True))
    assert report["decision"] == READY_DECISION
    assert report["status"] == "PASS"
    assert report["second_trade_would_route_count"] == 1
    ...
    assert report["second_trade_submit_enabled"] is False
    assert report["paper_order_submission_enabled"] is False
    assert report["total_risk_amount"] == 2.5
    assert (data / "lsr_v2_second_trade_route_preflight_report.json").exists()
    rows = [json.loads(line) for line in (data / "lsr_v2_second_trade_route_preflight.jsonl").read_text().splitlines()]
    assert [r["event_type"] for r in rows] == ["LSR_V2_SECOND_TRADE_ROUTE_PREFLIGHT", "LSR_V2_SECOND_TRADE_ORDER_INTENT_PREFLIGHT"]
...
def test_unsafe_or_max_orders_rejects(tmp_path: Path) -> None:
    ...
    report2 = build_lsr_v2_second_trade_route_preflight_report_from_files(data_dir=data2, settings=_settings(enable=True, max_orders=2))
