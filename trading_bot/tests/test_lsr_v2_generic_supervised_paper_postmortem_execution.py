...
from trading_bot.core.lsr_v2_generic_supervised_paper_postmortem_execution import (
    KEEP_DIAGNOSTIC_DECISION,
    READY_DECISION,
    Settings,
    run_generic_supervised_paper_postmortem_execution,
)
...
def _u39_report() -> dict:
    scaffold_map = {
        "dry_run_order_intent_payload": _dry_run_payload(),
        "postmortem_execution_scaffold_gate_model": {
            "postmortem_execution_scaffold_model_ready": True,
            "postmortem_execution_scaffold_gate_model_ready": True,
            "future_postmortem_execution_conditions_satisfied": False,
            "postmortem_execution_allowed": False,
            "runtime_values_present": False,
        },
    }
    report = {
        "decision": "LSR_V2_GENERIC_SUPERVISED_PAPER_POSTMORTEM_EXECUTION_SCAFFOLD_READY",
        "generic_supervised_paper_postmortem_execution_scaffold_ready": True,
        "generic_postmortem_execution_scaffold_ready": True,
        "generic_postmortem_execution_scaffold_model_ready": True,
        "generic_postmortem_execution_scaffold_patch_ready": True,
        "paper_postmortem_execution_scaffold_ready": True,
        "postmortem_execution_scaffold_diagnostic_available": True,
        "postmortem_execution_scaffold_diagnostic_count": 401,
        "postmortem_execution_scaffold_contract_present_fields": REQUIRED,
        "postmortem_execution_scaffold_contract_required_fields": REQUIRED,
        "postmortem_execution_scaffold_contract_missing_fields": [],
        ...
        "generic_supervised_paper_postmortem_preflight_map": {"dry_run_order_intent_payload": _dry_run_payload()},
...
def test_postmortem_execution_ready_but_no_runtime_postmortem(tmp_path, monkeypatch):
    for key in list(os.environ):
        if key.startswith("LSR_V2"):
            monkeypatch.delenv(key, raising=False)
    _write_markers(tmp_path)
    _write_json(tmp_path / "data/lsr_v2_generic_supervised_paper_postmortem_execution_scaffold_report.json", _u39_report())

    report = run_generic_supervised_paper_postmortem_execution(_settings(tmp_path))

    assert report["status"] == "PASS"
    assert report["decision"] == READY_DECISION
    assert report["generic_supervised_paper_postmortem_execution_ready"] is True
    assert report["generic_postmortem_execution_ready"] is True
    assert report["generic_postmortem_execution_model_ready"] is True
    assert report["generic_postmortem_execution_patch_ready"] is True
    assert report["paper_postmortem_execution_ready"] is True
    assert report["postmortem_execution_diagnostic_available"] is True
    assert report["postmortem_execution_diagnostic_count"] == 401
    assert report["postmortem_execution_contract_shape_modelable"] is True
    assert report["postmortem_execution_contract_missing_fields"] == []
    ...
    assert report["orders_submitted_by_generic_postmortem_execution"] == 0
    assert report["positions_closed_by_generic_postmortem_execution"] == 0
    assert report["paper_state_modified_by_generic_postmortem_execution"] is False
    assert report["paper_status_modified_by_generic_postmortem_execution"] is False
    execution_map = report["generic_supervised_paper_postmortem_execution_map"]
    assert execution_map["postmortem_execution_patch"] is True
    assert execution_map["postmortem_execution_gate_model"]["postmortem_execution_model_ready"] is True
    assert execution_map["postmortem_execution_gate_model"]["postmortem_execution_allowed"] is False
    reasons = execution_map["postmortem_execution_gate_model"]["postmortem_execution_blocked_reasons"]
    assert "explicit_postmortem_execution_patch_required" not in reasons
    assert "final_audit_runtime_execution_absent" in reasons


def test_postmortem_execution_requires_upstream_scaffold_report(tmp_path):
    _write_markers(tmp_path)
    ...
    assert report["generic_supervised_paper_postmortem_execution_ready"] is False
    assert report["postmortem_execution_diagnostic_available"] is False
    assert report["generic_postmortem_execution_allowed"] is False
    assert report["would_run_postmortem"] is False


def test_postmortem_execution_rejects_not_ready_upstream_report(tmp_path):
    _write_markers(tmp_path)
    upstream = _u39_report()
    upstream["decision"] = "NOT_READY"
    _write_json(tmp_path / "data/lsr_v2_generic_supervised_paper_postmortem_execution_scaffold_report.json", upstream)
...
def test_postmortem_execution_requires_source_markers(tmp_path):
    _write_json(tmp_path / "data/lsr_v2_generic_supervised_paper_postmortem_execution_scaffold_report.json", _u39_report())
...
def test_postmortem_operator_env_cannot_override_missing_final_audit_and_receipts(tmp_path, monkeypatch):
    _write_markers(tmp_path)
    _write_json(tmp_path / "data/lsr_v2_generic_supervised_paper_postmortem_execution_scaffold_report.json", _u39_report())
    ...
    assert report["paper_postmortem_ready"] is False
    assert report["would_run_postmortem"] is False
    reasons = report["generic_supervised_paper_postmortem_execution_map"]["postmortem_execution_gate_model"]["postmortem_execution_blocked_reasons"]
    assert "final_audit_runtime_execution_absent" in reasons
