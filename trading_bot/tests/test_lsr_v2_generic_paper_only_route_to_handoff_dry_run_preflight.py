...
def _write_markers(root: Path) -> None:
    ...
    (root / "trading_bot/core/lsr_v2_generic_paper_only_candidate_route_preflight.py").write_text(
...
def _u18_report(**overrides: object) -> dict:
    payload = {
        "status": "PASS",
        "decision": "LSR_V2_GENERIC_PAPER_ONLY_CANDIDATE_ROUTE_PREFLIGHT_READY",
        "generic_candidate_route_preflight_ready": True,
        "candidate_route_preflight_diagnostic_available": True,
        "candidate_route_preflight_diagnostic_count": 401,
        ...
        "candidate_route_preflight_map": {
            "route_preflight_context": {
                "diagnostic_candidates_available": True,
                "route_candidate_available": False,
                "paper_order_intent_ready": False,
                "would_route": False,
...
def test_handoff_dry_run_preflight_passes_with_route_preflight_diagnostics(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.delenv("LSR_V2_GENERIC_REARM_ENABLE", raising=False)
    _write_markers(tmp_path)
    _write_json(tmp_path / "data/lsr_v2_generic_paper_only_candidate_route_preflight_report.json", _u18_report())

    report = run_generic_paper_only_route_to_handoff_dry_run_preflight(Settings(project_root=tmp_path))

    assert report["status"] == "PASS"
    assert report["decision"] == READY_DECISION
    assert report["generic_route_to_handoff_dry_run_preflight_ready"] is True
    assert report["handoff_dry_run_preflight_diagnostic_available"] is True
    assert report["handoff_dry_run_preflight_diagnostic_count"] == 401
    assert report["route_candidate_available"] is False
    assert report["handoff_candidate_available"] is False
    assert report["paper_order_intent_ready"] is False
    assert report["would_route"] is False
    assert report["would_handoff"] is False
    assert report["would_create_order"] is False
    assert report["would_submit"] is False
    assert report["generic_handoff_execution_allowed"] is False
    assert report["generic_order_intent_creation_allowed"] is False
    assert report["orders_submitted_by_generic_route_to_handoff_dry_run_preflight"] == 0
    assert report["positions_opened_by_generic_route_to_handoff_dry_run_preflight"] == 0
    assert report["paper_state_modified_by_generic_route_to_handoff_dry_run_preflight"] is False
    assert report["live_enabled"] is False
    assert report["testnet_enabled"] is False
    assert report["exchange_broker_enabled"] is False


def test_handoff_dry_run_preflight_warns_when_upstream_missing(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.delenv("LSR_V2_GENERIC_REARM_ENABLE", raising=False)
    _write_markers(tmp_path)
    ...
    assert "candidate_route_preflight_report_present" in report["blockers"]
    assert report["generic_route_to_handoff_dry_run_preflight_ready"] is False
    assert report["generic_handoff_execution_allowed"] is False


def test_handoff_dry_run_preflight_warns_without_route_preflight_diagnostic(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.delenv("LSR_V2_GENERIC_REARM_ENABLE", raising=False)
    _write_markers(tmp_path)
    _write_json(
        tmp_path / "data/lsr_v2_generic_paper_only_candidate_route_preflight_report.json",
        _u18_report(candidate_route_preflight_diagnostic_available=False, candidate_route_preflight_diagnostic_count=0),
    )

    report = run_generic_paper_only_route_to_handoff_dry_run_preflight(Settings(project_root=tmp_path))

    assert report["status"] == "WARN"
    assert "candidate_route_preflight_diagnostic_available" in report["blockers"]
    assert report["handoff_dry_run_preflight_diagnostic_available"] is False
    assert report["paper_order_intent_ready"] is False


def test_handoff_dry_run_preflight_warns_when_order_intent_already_exists(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.delenv("LSR_V2_GENERIC_REARM_ENABLE", raising=False)
    _write_markers(tmp_path)
    _write_json(
        tmp_path / "data/lsr_v2_generic_paper_only_candidate_route_preflight_report.json",
        _u18_report(paper_order_intent_ready=True),
    )
...
def test_handoff_dry_run_preflight_warns_on_operator_env(tmp_path: Path, monkeypatch) -> None:
