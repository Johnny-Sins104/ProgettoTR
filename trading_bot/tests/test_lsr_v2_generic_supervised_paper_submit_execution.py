...
from core.lsr_v2_generic_supervised_paper_submit_execution import (
...
def _write_markers(root: Path) -> None:
    ...
    (core / "lsr_v2_generic_supervised_paper_order_intent_activation.py").write_text(
...
def _u28_report() -> dict:
    activation_map = {
        ...
        "activation_gate_model": {
            ...
            "runtime_numeric_values_valid": False,
            "directional_stop_take_profit_valid": False,
            "max_open_positions": 1,
            "max_open_positions_valid": True,
            "paper_only": True,
            "paper_only_valid": True,
            "real_order_intent_activation_allowed": False,
            "paper_order_intent_materialization_allowed": False,
    ...
    report = {
        "prompt": "29.4.4u-28",
        "status": "PASS",
        "decision": "LSR_V2_GENERIC_SUPERVISED_PAPER_ORDER_INTENT_ACTIVATION_READY",
        "generic_supervised_paper_order_intent_activation_ready": True,
        "generic_order_intent_activation_model_ready": True,
        "generic_order_intent_activation_patch_ready": True,
    ...
    for key in FALSE_EXECUTION_KEYS:
        report[key] = False
    report.update({"generic_order_intent_activation_allowed": False, "generic_order_intent_persistence_allowed": False})
    return report
...
def test_submit_execution_ready_but_no_broker_submit_without_order_intent_candidate_gates_or_runtime(tmp_path, monkeypatch):
    for key in list(__import__("os").environ):
        if key.startswith("LSR_V2"):
            monkeypatch.delenv(key, raising=False)
    _write_markers(tmp_path)
    _write_json(tmp_path / "data/lsr_v2_generic_supervised_paper_order_intent_activation_report.json", _u28_report())

    report = run_generic_supervised_paper_submit_execution(_settings(tmp_path))

    assert report["status"] == "PASS"
    assert report["decision"] == READY_DECISION
    assert report["generic_supervised_paper_submit_execution_ready"] is True
    assert report["generic_submit_execution_patch_ready"] is True
    assert report["generic_submit_execution_gate_model_ready"] is True
    assert report["paper_submit_execution_model_ready"] is True
    ...
    assert report["orders_submitted_by_generic_submit_execution"] == 0
    assert report["positions_opened_by_generic_submit_execution"] == 0
    assert report["paper_state_modified_by_generic_submit_execution"] is False
    assert report["paper_status_modified_by_generic_submit_execution"] is False
    assert report["live_enabled"] is False
    assert report["testnet_enabled"] is False
    assert report["exchange_broker_enabled"] is False


def test_submit_execution_requires_upstream_activation_report(tmp_path):
    _write_markers(tmp_path)
    ...
    assert report["generic_supervised_paper_submit_execution_ready"] is False


def test_submit_execution_requires_source_markers(tmp_path):
    _write_json(tmp_path / "data/lsr_v2_generic_supervised_paper_order_intent_activation_report.json", _u28_report())
    ...
    assert report["generic_submit_execution_source_files_present"] is False


def test_submit_execution_fails_closed_if_operator_env_is_set_during_validation(tmp_path, monkeypatch):
    _write_markers(tmp_path)
    _write_json(tmp_path / "data/lsr_v2_generic_supervised_paper_order_intent_activation_report.json", _u28_report())
...
def test_submit_execution_writes_report_and_jsonl(tmp_path, monkeypatch):
    for key in list(__import__("os").environ):
        if key.startswith("LSR_V2"):
            monkeypatch.delenv(key, raising=False)
    _write_markers(tmp_path)
    ...
    report_path = tmp_path / "data/lsr_v2_generic_supervised_paper_submit_execution_report.json"
    jsonl_path = tmp_path / "data/lsr_v2_generic_supervised_paper_submit_execution.jsonl"
