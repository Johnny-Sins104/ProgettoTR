...
from core.lsr_v2_generic_supervised_paper_integrated_operation_terminal_ready_synthesis_execution import (
    READY_DECISION,
    Settings,
...
def _base_u60_report() -> dict:
    ...
    return {
        "status": "PASS",
        "decision": "LSR_V2_GENERIC_SUPERVISED_PAPER_INTEGRATED_OPERATION_TERMINAL_READY_SYNTHESIS_EXECUTION_SCAFFOLD_READY",
        "generic_supervised_paper_integrated_operation_terminal_ready_synthesis_execution_scaffold_ready": True,
        ...
        "integrated_operation_terminal_ready_synthesis_execution_scaffold_diagnostic_count": 402,
        "generic_supervised_paper_integrated_operation_terminal_ready_synthesis_execution_scaffold_map": {
            ...
            "integrated_operation_terminal_ready_synthesis_execution_scaffold_gate_model": gate,
        },
...
def _write_u60_report(tmp_path: Path, payload: dict | None = None) -> None:
    _write_json(
        tmp_path / "data/lsr_v2_generic_supervised_paper_integrated_operation_terminal_ready_synthesis_execution_scaffold_report.json",
        payload if payload is not None else _base_u60_report(),
    )


def test_terminal_ready_synthesis_execution_ready_but_fail_closed(tmp_path: Path) -> None:
    _write_markers(tmp_path)
    _write_u60_report(tmp_path)

    report = run_generic_supervised_paper_integrated_operation_terminal_ready_synthesis_execution(_settings(tmp_path))

    assert report["status"] == "PASS"
    assert report["decision"] == READY_DECISION
    assert report["generic_supervised_paper_integrated_operation_terminal_ready_synthesis_execution_ready"] is True
    assert report["generic_integrated_operation_terminal_ready_synthesis_execution_model_ready"] is True
    assert report["generic_integrated_operation_terminal_ready_synthesis_execution_patch_ready"] is True
    assert report["paper_integrated_operation_terminal_ready_synthesis_execution_ready"] is True
...
def test_gate_model_requires_runtime_evidence_after_execution_patch_current_step(tmp_path: Path) -> None:
    _write_markers(tmp_path)
    _write_u60_report(tmp_path)

    report = run_generic_supervised_paper_integrated_operation_terminal_ready_synthesis_execution(_settings(tmp_path))
    gate = report["generic_supervised_paper_integrated_operation_terminal_ready_synthesis_execution_map"][
        "integrated_operation_terminal_ready_synthesis_execution_gate_model"
    ]

    assert gate["integrated_operation_terminal_ready_synthesis_execution_gate_model_ready"] is True
    assert gate["integrated_operation_terminal_ready_synthesis_execution_allowed"] is False
    assert gate["integrated_operation_terminal_ready_synthesis_execution_scaffold_report_ready"] is True
    assert "explicit_integrated_operation_terminal_ready_synthesis_execution_patch_required" not in gate[
        "integrated_operation_terminal_ready_synthesis_execution_blocked_reasons"
    ]
    assert "broker_submit_receipt_absent" in gate["integrated_operation_terminal_ready_synthesis_execution_blocked_reasons"]
...
def test_not_ready_upstream_report_fails_closed(tmp_path: Path) -> None:
    _write_markers(tmp_path)
    bad = _base_u60_report()
    bad["decision"] = "KEEP_DIAGNOSTIC"
    _write_u60_report(tmp_path, bad)
...
def test_report_and_jsonl_are_written(tmp_path: Path) -> None:
    ...
    report_path = tmp_path / "data/lsr_v2_generic_supervised_paper_integrated_operation_terminal_ready_synthesis_execution_report.json"
    jsonl_path = tmp_path / "data/lsr_v2_generic_supervised_paper_integrated_operation_terminal_ready_synthesis_execution.jsonl"
    ...
    assert report["report"].endswith("lsr_v2_generic_supervised_paper_integrated_operation_terminal_ready_synthesis_execution_report.json")
