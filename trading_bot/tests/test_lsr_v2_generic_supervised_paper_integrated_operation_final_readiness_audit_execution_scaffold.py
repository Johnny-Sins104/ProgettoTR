...
from core.lsr_v2_generic_supervised_paper_integrated_operation_final_readiness_audit_execution_scaffold import (
    READY_DECISION,
    Settings,
    run_generic_supervised_paper_integrated_operation_final_readiness_audit_execution_scaffold,
)
...
def _base_u53_report() -> dict:
    gate = {
        ...
        "integrated_operation_readiness_execution_conditions_satisfied": False,
    ...
    return {
        "status": "PASS",
        "decision": "LSR_V2_GENERIC_SUPERVISED_PAPER_INTEGRATED_OPERATION_FINAL_READINESS_AUDIT_PREFLIGHT_READY",
        "generic_supervised_paper_integrated_operation_final_readiness_audit_preflight_ready": True,
        ...
        "integrated_operation_final_readiness_audit_preflight_diagnostic_count": 401,
        "generic_supervised_paper_integrated_operation_final_readiness_audit_preflight_map": {
            ...
            "integrated_operation_final_readiness_audit_preflight_gate_model": gate,
        },
...
def _write_u53_report(tmp_path: Path, payload: dict | None = None) -> None:
    _write_json(
        tmp_path / "data/lsr_v2_generic_supervised_paper_integrated_operation_final_readiness_audit_preflight_report.json",
        payload if payload is not None else _base_u53_report(),
    )


def test_integrated_operation_final_readiness_audit_execution_scaffold_ready_but_fail_closed(tmp_path: Path) -> None:
    _write_markers(tmp_path)
    _write_u53_report(tmp_path)

    report = run_generic_supervised_paper_integrated_operation_final_readiness_audit_execution_scaffold(_settings(tmp_path))

    assert report["status"] == "PASS"
    assert report["decision"] == READY_DECISION
    assert report["generic_supervised_paper_integrated_operation_final_readiness_audit_execution_scaffold_ready"] is True
    assert report["generic_integrated_operation_final_readiness_audit_execution_scaffold_model_ready"] is True
    assert report["generic_integrated_operation_final_readiness_audit_execution_scaffold_patch_ready"] is True
    assert report["paper_integrated_operation_final_readiness_audit_execution_scaffold_ready"] is True
    assert report["generic_integrated_operation_final_readiness_audit_execution_scaffold_allowed"] is False
...
def test_gate_model_requires_future_execution_patch_and_runtime_evidence(tmp_path: Path) -> None:
    _write_markers(tmp_path)
    _write_u53_report(tmp_path)

    report = run_generic_supervised_paper_integrated_operation_final_readiness_audit_execution_scaffold(_settings(tmp_path))
    gate = report["generic_supervised_paper_integrated_operation_final_readiness_audit_execution_scaffold_map"][
        "integrated_operation_final_readiness_audit_execution_scaffold_gate_model"
    ]

    assert gate["integrated_operation_final_readiness_audit_execution_scaffold_gate_model_ready"] is True
    assert gate["integrated_operation_final_readiness_audit_execution_scaffold_allowed"] is False
    assert gate["explicit_integrated_operation_final_readiness_audit_execution_patch_required"] is True
    assert "explicit_integrated_operation_final_readiness_audit_execution_patch_required" in gate[
        "integrated_operation_final_readiness_audit_execution_scaffold_blocked_reasons"
    ]
    assert "broker_submit_receipt_absent" in gate["integrated_operation_final_readiness_audit_execution_scaffold_blocked_reasons"]
    assert "runtime_values_absent_or_deferred" in gate["integrated_operation_final_readiness_audit_execution_scaffold_blocked_reasons"]
...
def test_not_ready_upstream_report_fails_closed(tmp_path: Path) -> None:
    _write_markers(tmp_path)
    bad = _base_u53_report()
    bad["decision"] = "KEEP_DIAGNOSTIC"
    _write_u53_report(tmp_path, bad)
...
def test_report_and_jsonl_are_written(tmp_path: Path) -> None:
    ...
    report_path = tmp_path / "data/lsr_v2_generic_supervised_paper_integrated_operation_final_readiness_audit_execution_scaffold_report.json"
    jsonl_path = tmp_path / "data/lsr_v2_generic_supervised_paper_integrated_operation_final_readiness_audit_execution_scaffold.jsonl"
    ...
    assert report["report"].endswith("lsr_v2_generic_supervised_paper_integrated_operation_final_readiness_audit_execution_scaffold_report.json")
