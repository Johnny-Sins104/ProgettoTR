...
from core.lsr_v2_paper_cycle_controller import Settings, run_controller_draft
...
def _seed_ready_reports(data: Path) -> None:
    _write(data / "lsr_v2_generic_paper_cycle_refactor_preflight_report.json", {
        ...
        "generic_cycle_refactor_preflight_ready": True,
    ...
    _write(data / "lsr_v2_fourth_trade_candidate_detection_audit_report.json", {"status": "PASS", "decision": "LSR_V2_FOURTH_TRADE_CANDIDATE_DETECTION_AUDIT_READY"})
    _write(data / "lsr_v2_trade_lifecycle_auto_monitor_report.json", {"status": "PASS", "decision": "LSR_V2_TRADE_LIFECYCLE_AUTO_MONITOR_READY", "lifecycle_state": "FLAT_LOCKED"})
    _write(data / "lsr_v2_telegram_trade_dashboard_report.json", {"status": "PASS", "decision": "LSR_V2_TELEGRAM_TRADE_DASHBOARD_READY", "dashboard_ready": True, "telegram_payload_ready": True})
...
def test_ready_controller_draft(tmp_path, monkeypatch):
    monkeypatch.delenv("LSR_V2_GENERIC_CONTROLLER_ENABLE", raising=False)
    data = tmp_path / "data"
    _seed_ready_reports(data)

    result = run_controller_draft(Settings(project_root=tmp_path))

    assert result["status"] == "PASS"
    assert result["decision"] == "LSR_V2_GENERIC_PAPER_CYCLE_CONTROLLER_DRAFT_READY"
    assert result["generic_cycle_controller_draft_ready"] is True
    assert result["generic_trade_state_machine_ready"] is True
    assert result["generic_controller_state_machine"]["execution_enabled"] is False
    ...
    assert result["orders_submitted_by_generic_cycle_controller_draft"] == 0
    assert result["paper_state_modified_by_generic_cycle_controller_draft"] is False


def test_missing_u1_refactor_preflight_warns(tmp_path):
    data = tmp_path / "data"
    _write(data / "paper_state.json", {"orders": {}, "positions": {}})
    _write(data / "paper_status.json", {"open_positions": 0, "pending_orders": 0})
    ...
    assert result["generic_cycle_controller_draft_ready"] is False
    assert "missing_upstream_reports" in result["blockers"]
    assert result["generic_cycle_controller_execution_allowed"] is False


def test_active_lsr_v2_env_keeps_controller_fail_closed(tmp_path, monkeypatch):
    data = tmp_path / "data"
    _seed_ready_reports(data)
    monkeypatch.setenv("LSR_V2_GENERIC_CONTROLLER_ENABLE", "1")
...
def test_controller_blocks_ordinal_expansion(tmp_path):
    data = tmp_path / "data"
    _seed_ready_reports(data)

    result = run_controller_draft(Settings(project_root=tmp_path))

    assert result["generic_controller_contract"]["trade_ordinal"] == "N"
    assert result["generic_controller_contract"]["ordinal_module_generation_allowed"] is False
