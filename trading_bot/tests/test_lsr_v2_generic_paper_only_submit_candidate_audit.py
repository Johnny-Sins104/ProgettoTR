...
from core.lsr_v2_generic_paper_only_submit_candidate_audit import (
    READY_DECISION,
...
def _write_markers(root: Path) -> None:
    ...
    (root / "trading_bot/core/lsr_v2_generic_paper_only_submit_readiness_preflight.py").write_text(
...
def _u24_report(**overrides: object) -> dict:
    ...
    submit_map = {
        ...
        "submit_readiness_contract_ready": True,
        "submit_readiness_preflight_model_ready": True,
    ...
    report = {
        "prompt": "29.4.4u-24",
        "status": "PASS",
        "decision": "LSR_V2_GENERIC_PAPER_ONLY_SUBMIT_READINESS_PREFLIGHT_READY",
        "generic_submit_readiness_preflight_ready": True,
        "generic_submit_readiness_preflight_contract_ready": True,
        "generic_submit_readiness_preflight_gate_model_ready": True,
        "paper_submit_readiness_preflight_ready": True,
...
def test_submit_candidate_audit_passes_with_u24_submit_readiness_preflight(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.delenv("LSR_V2_GENERIC_SUBMIT_ENABLE", raising=False)
    _write_markers(tmp_path)
    _write_json(tmp_path / "data/lsr_v2_generic_paper_only_submit_readiness_preflight_report.json", _u24_report())

    report = run_generic_paper_only_submit_candidate_audit(Settings(project_root=tmp_path))

    assert report["status"] == "PASS"
    assert report["decision"] == READY_DECISION
    assert report["generic_submit_candidate_audit_ready"] is True
    assert report["paper_submit_candidate_audit_ready"] is True
    assert report["submit_candidate_audit_diagnostic_available"] is True
    assert report["submit_candidate_contract_shape_modelable"] is True
    assert report["paper_submit_candidate_ready"] is False
    assert report["generic_submit_candidate_ready"] is False
    assert report["paper_submit_execution_ready"] is False
    assert report["generic_submit_execution_allowed"] is False
    assert report["would_prepare_submit_candidate"] is False
    assert report["would_call_paper_broker_submit"] is False
    assert report["would_submit"] is False
    assert report["orders_submitted_by_generic_submit_candidate_audit"] == 0
    assert report["positions_opened_by_generic_submit_candidate_audit"] == 0
    assert report["paper_state_modified_by_generic_submit_candidate_audit"] is False
    assert report["paper_status_modified_by_generic_submit_candidate_audit"] is False
    assert report["live_enabled"] is False
    assert report["testnet_enabled"] is False
    assert report["exchange_broker_enabled"] is False


def test_submit_candidate_audit_warns_when_upstream_missing(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.delenv("LSR_V2_GENERIC_SUBMIT_ENABLE", raising=False)
    _write_markers(tmp_path)
    ...
    assert "submit_readiness_preflight_report_present" in report["blockers"]
    assert report["generic_submit_candidate_audit_ready"] is False
    assert report["would_submit"] is False


def test_submit_candidate_audit_warns_when_submit_readiness_not_ready(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.delenv("LSR_V2_GENERIC_SUBMIT_ENABLE", raising=False)
    _write_markers(tmp_path)
    _write_json(
        tmp_path / "data/lsr_v2_generic_paper_only_submit_readiness_preflight_report.json",
        _u24_report(generic_submit_readiness_preflight_ready=False, paper_submit_readiness_preflight_ready=False),
    )

    report = run_generic_paper_only_submit_candidate_audit(Settings(project_root=tmp_path))

    assert report["status"] == "WARN"
    assert "submit_readiness_preflight_report_ready" in report["blockers"]
    assert "paper_submit_readiness_preflight_ready" in report["blockers"]
    assert report["paper_submit_candidate_ready"] is False


def test_submit_candidate_audit_warns_when_contract_policy_invalid(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.delenv("LSR_V2_GENERIC_SUBMIT_ENABLE", raising=False)
    _write_markers(tmp_path)
    _write_json(
        tmp_path / "data/lsr_v2_generic_paper_only_submit_readiness_preflight_report.json",
        _u24_report(dry_run_payload=_dry_run_payload(max_open_positions=2, paper_only=False)),
    )

    report = run_generic_paper_only_submit_candidate_audit(Settings(project_root=tmp_path))

    assert report["status"] == "WARN"
    assert "submit_candidate_contract_shape_modelable" in report["blockers"]
    assert "single_position_policy_ready" in report["blockers"]
    assert "paper_only_policy_ready" in report["blockers"]
    assert report["generic_submit_candidate_audit_map"]["submit_candidate_contract_audit"]["paper_only_valid"] is False
    assert report["would_submit"] is False


def test_submit_candidate_audit_warns_when_submit_unblocked(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.delenv("LSR_V2_GENERIC_SUBMIT_ENABLE", raising=False)
    _write_markers(tmp_path)
    _write_json(
        tmp_path / "data/lsr_v2_generic_paper_only_submit_readiness_preflight_report.json",
        _u24_report(generic_submit_execution_allowed=True),
    )
...
def test_submit_candidate_audit_warns_when_operator_env_present(tmp_path: Path, monkeypatch) -> None:
...
def test_submit_candidate_audit_writes_report_and_jsonl(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.delenv("LSR_V2_GENERIC_SUBMIT_ENABLE", raising=False)
    _write_markers(tmp_path)
    _write_json(tmp_path / "data/lsr_v2_generic_paper_only_submit_readiness_preflight_report.json", _u24_report())

    settings = Settings(project_root=tmp_path)
    report = run_generic_paper_only_submit_candidate_audit(settings)

    report_path = tmp_path / "data/lsr_v2_generic_paper_only_submit_candidate_audit_report.json"
    jsonl_path = tmp_path / "data/lsr_v2_generic_paper_only_submit_candidate_audit.jsonl"
    assert report_path.exists()
    assert jsonl_path.exists()
    saved = json.loads(report_path.read_text(encoding="utf-8"))
    assert saved["decision"] == READY_DECISION
    assert report["generic_submit_candidate_audit_map"]["submit_candidate_audit_context"]["broker_submit_called"] is False
