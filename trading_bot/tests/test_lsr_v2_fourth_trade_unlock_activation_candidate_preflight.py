...
from core.lsr_v2_fourth_trade_unlock_activation_candidate_preflight import (
    build_lsr_v2_fourth_trade_unlock_activation_candidate_preflight_report_from_files,
    PASS_DECISION,
...
def _write_source_tree(root: Path, *, missing_marker: bool = False) -> None:
    files = {
        "trading_bot/core/lsr_v2_fourth_trade_operator_unlock_activation_preflight.py": "LSR_V2_FOURTH_TRADE_OPERATOR_UNLOCK_ACTIVATION_PREFLIGHT\noperator_unlock_activation_preflight_ready=True\noperator_unlock_activation_requires_execution_patch=True\n",
    ...
    if missing_marker:
        files["trading_bot/core/lsr_v2_fourth_trade_operator_unlock_activation_preflight.py"] = "# marker intentionally missing\n"
...
def _seed(
    tmp_path: Path,
    *,
    missing_activation_report: bool = False,
    ...
    if not missing_activation_report:
        _write_json(data / "lsr_v2_fourth_trade_operator_unlock_activation_preflight_report.json", {
            **common,
            "decision": "LSR_V2_FOURTH_TRADE_OPERATOR_UNLOCK_ACTIVATION_PREFLIGHT_READY",
            "operator_unlock_activation_preflight_ready": True,
            "guarded_operator_unlock_activation_preflight_ready": True,
            "operator_unlock_activation_requires_execution_patch": True,
        })
...
def test_candidate_preflight_passes_default_but_keeps_candidate_detection_disabled(tmp_path: Path) -> None:
    data, project = _seed(tmp_path)
    report = build_lsr_v2_fourth_trade_unlock_activation_candidate_preflight_report_from_files(data_dir=data, project_root=project)
    assert report["status"] == "PASS"
    assert report["decision"] == PASS_DECISION
    assert report["unlock_activation_candidate_preflight_ready"] is True
    assert report["candidate_detection_preflight_ready"] is True
    assert report["operator_unlock_activation_candidate_preflight_armed"] is False
    ...
    assert report["orders_submitted_by_unlock_activation_candidate_preflight"] == 0
    assert report["positions_opened_by_unlock_activation_candidate_preflight"] == 0
    assert report["positions_closed_by_unlock_activation_candidate_preflight"] == 0


def test_valid_operator_env_only_arms_preflight_and_still_blocks_candidate_execution(monkeypatch, tmp_path: Path) -> None:
    data, project = _seed(tmp_path)
    ...
    assert report["status"] == "PASS"
    assert report["decision"] == PASS_DECISION
    assert report["operator_unlock_activation_candidate_preflight_armed"] is True
    assert report["operator_unlock_activation_candidate_controls_valid"] is True
    assert report["operator_unlock_activation_candidate_would_unlock_fourth_trade"] is True
    assert report["operator_unlock_activation_candidate_would_enable_candidate_detection"] is True
    assert report["future_candidate_detection_allowed"] is False
    assert report["candidate_detection_allowed"] is False
    assert report["candidate_submit_execution_allowed"] is False
    assert report["paper_only_execution_allowed"] is False
...
def test_invalid_operator_env_warns(tmp_path: Path, monkeypatch) -> None:
    ...
    assert "operator_unlock_activation_candidate_env_invalid" in report["blockers"]


def test_missing_activation_report_warns(tmp_path: Path) -> None:
    data, project = _seed(tmp_path, missing_activation_report=True)
    report = build_lsr_v2_fourth_trade_unlock_activation_candidate_preflight_report_from_files(data_dir=data, project_root=project)
    assert report["status"] == "WARN"
    assert report["decision"] == REPORTS_NOT_READY_DECISION
    assert "required_unlock_activation_candidate_reports_not_ready" in report["blockers"]
...
def test_missing_source_marker_warns(tmp_path: Path) -> None:
    ...
    assert "unlock_activation_candidate_required_markers_missing" in report["blockers"]
