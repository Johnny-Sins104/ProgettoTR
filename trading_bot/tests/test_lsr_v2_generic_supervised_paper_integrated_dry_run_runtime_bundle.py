...
def _write_markers(tmp_path: Path) -> None:
    ...
    (core / "lsr_v2_generic_supervised_paper_runtime_arming_bundle.py").write_text(
        "LSR_V2_GENERIC_SUPERVISED_PAPER_RUNTIME_ARMING_BUNDLE\n"
        "generic_paper_runtime_arming_bundle_ready\n",
        encoding="utf-8",
    )
    (core / "lsr_v2_generic_supervised_paper_integrated_dry_run_runtime_preflight.py").write_text(
        "run_generic_supervised_paper_integrated_dry_run_runtime_preflight\n",
        encoding="utf-8",
    )
    (core / "lsr_v2_generic_supervised_paper_integrated_dry_run_runtime_execution_scaffold.py").write_text(
        "run_generic_supervised_paper_integrated_dry_run_runtime_execution_scaffold\n",
        encoding="utf-8",
    )
    (core / "lsr_v2_generic_supervised_paper_integrated_dry_run_runtime_execution.py").write_text(
        "run_generic_supervised_paper_integrated_dry_run_runtime_execution\n",
        encoding="utf-8",
    )
...
def _u66_report() -> dict:
    return {
        "status": "PASS",
        "decision": "LSR_V2_GENERIC_SUPERVISED_PAPER_RUNTIME_ARMING_BUNDLE_READY",
        "generic_supervised_paper_runtime_arming_bundle_ready": True,
        "generic_paper_runtime_arming_bundle_ready": True,
        "generic_paper_runtime_arming_execution_allowed": False,
        "generic_supervised_paper_runtime_arming_bundle_map": {
            "mode": "generic_supervised_paper_runtime_arming_bundle",
            "read_only": True,
            "fail_closed": True,
...
def _write_u66_report(tmp_path: Path, payload: dict | None = None) -> None:
    _write_json(
        tmp_path / "data/lsr_v2_generic_supervised_paper_runtime_arming_bundle_report.json",
        payload if payload is not None else _u66_report(),
    )


def test_integrated_dry_run_runtime_bundle_runs_three_substeps_and_stays_fail_closed(tmp_path: Path) -> None:
    _write_markers(tmp_path)
    _write_u66_report(tmp_path)

    report = run_generic_supervised_paper_integrated_dry_run_runtime_bundle(_settings(tmp_path))

    assert report["status"] == "PASS"
    assert report["decision"] == BUNDLE_READY_DECISION
    assert report["generic_supervised_paper_integrated_dry_run_runtime_bundle_ready"] is True
    ...
    assert report["generic_paper_integrated_dry_run_runtime_execution_allowed"] is False
    ...
    assert report["would_run_integrated_dry_run_runtime"] is False
...
def test_preflight_keeps_explicit_execution_patch_blocker(tmp_path: Path) -> None:
    ...
    report = run_generic_supervised_paper_integrated_dry_run_runtime_preflight(_settings(tmp_path))
    gate = report["generic_supervised_paper_integrated_dry_run_runtime_preflight_map"][
        "paper_integrated_dry_run_runtime_preflight_gate_model"
    ]

    assert report["decision"] == PREFLIGHT_READY_DECISION
    assert "explicit_integrated_dry_run_runtime_execution_patch_required" in gate[
        "paper_integrated_dry_run_runtime_preflight_blocked_reasons"
    ]
    assert gate["integrated_dry_run_runtime_executed"] is False
    assert gate["operator_paper_integrated_dry_run_runtime_gate_satisfied"] is False


def test_execution_removes_future_patch_blocker_but_requires_runtime_evidence(tmp_path: Path) -> None:
    _write_markers(tmp_path)
    _write_u66_report(tmp_path)
    run_generic_supervised_paper_integrated_dry_run_runtime_bundle(_settings(tmp_path))

    report = run_generic_supervised_paper_integrated_dry_run_runtime_execution(_settings(tmp_path))
    gate = report["generic_supervised_paper_integrated_dry_run_runtime_execution_map"][
        "paper_integrated_dry_run_runtime_execution_gate_model"
    ]

    assert report["decision"] == EXECUTION_READY_DECISION
    assert "explicit_integrated_dry_run_runtime_execution_patch_required" not in gate[
        "paper_integrated_dry_run_runtime_execution_blocked_reasons"
    ]
    assert "integrated_dry_run_runtime_absent" in gate[
        "paper_integrated_dry_run_runtime_execution_blocked_reasons"
    ]
    assert "broker_submit_receipt_absent" in gate["paper_integrated_dry_run_runtime_execution_blocked_reasons"]
    assert "broker_close_receipt_absent" in gate["paper_integrated_dry_run_runtime_execution_blocked_reasons"]
    assert gate["runtime_values_present"] is False


def test_missing_u66_upstream_fails_closed(tmp_path: Path) -> None:
    _write_markers(tmp_path)
...
def test_not_ready_u66_upstream_fails_closed(tmp_path: Path) -> None:
    _write_markers(tmp_path)
    bad = _u66_report()
    bad["decision"] = "KEEP_DIAGNOSTIC"
    _write_u66_report(tmp_path, bad)
...
def test_reports_and_jsonl_are_written(tmp_path: Path) -> None:
    ...
    expected = [
        "lsr_v2_generic_supervised_paper_integrated_dry_run_runtime_preflight_report.json",
        "lsr_v2_generic_supervised_paper_integrated_dry_run_runtime_execution_scaffold_report.json",
        "lsr_v2_generic_supervised_paper_integrated_dry_run_runtime_execution_report.json",
        "lsr_v2_generic_supervised_paper_integrated_dry_run_runtime_bundle_report.json",
    ]
    for name in expected:
        assert (tmp_path / "data" / name).exists()
    saved = json.loads((tmp_path / "data/lsr_v2_generic_supervised_paper_integrated_dry_run_runtime_bundle_report.json").read_text(encoding="utf-8"))
    assert saved["decision"] == BUNDLE_READY_DECISION
    assert report["report"].endswith("lsr_v2_generic_supervised_paper_integrated_dry_run_runtime_bundle_report.json")
