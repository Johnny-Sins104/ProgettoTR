...
from core.lsr_v2_generic_paper_cycle_runner_launcher_hook_preflight import (
    Settings,
    run_generic_paper_cycle_runner_launcher_hook_preflight,
)
...
def _seed_ready_reports(data: Path) -> None:
    _write(data / "lsr_v2_generic_paper_cycle_engine_hook_preflight_report.json", {
        "status": "PASS",
        "decision": "LSR_V2_GENERIC_PAPER_CYCLE_ENGINE_HOOK_PREFLIGHT_READY",
        "generic_cycle_engine_hook_preflight_ready": True,
    ...
    _write(data / "lsr_v2_launcher_runner_visibility_parity_audit_report.json", {
        "status": "PASS",
    ...
    _write(data / "lsr_v2_launcher_read_only_dashboard_banner_report.json", {
        "status": "PASS",
    ...
    _write(data / "lsr_v2_trade_lifecycle_auto_monitor_report.json", {
        "status": "PASS", "decision": "LSR_V2_TRADE_LIFECYCLE_AUTO_MONITOR_READY"
    })
    _write(data / "lsr_v2_telegram_trade_dashboard_report.json", {
        "status": "PASS", "decision": "LSR_V2_TELEGRAM_TRADE_DASHBOARD_READY", "telegram_payload_ready": True
    })
...
def test_ready_generic_cycle_runner_launcher_hook_preflight(tmp_path, monkeypatch):
    monkeypatch.delenv("LSR_V2_GENERIC_REARM_ENABLE", raising=False)
    data = tmp_path / "data"
    _seed_sources(tmp_path)
    _seed_ready_reports(data)

    result = run_generic_paper_cycle_runner_launcher_hook_preflight(Settings(project_root=tmp_path))

    assert result["status"] == "PASS"
    assert result["decision"] == "LSR_V2_GENERIC_PAPER_CYCLE_RUNNER_LAUNCHER_HOOK_PREFLIGHT_READY"
    assert result["generic_runner_launcher_hook_preflight_ready"] is True
    assert result["generic_runner_launcher_hook_map_ready"] is True
    assert result["generic_cycle_engine_hook_ready"] is True
    assert result["launcher_visibility_ready"] is True
    assert result["runner_visibility_ready"] is True
    assert result["visibility_parity_ok"] is True
    assert result["generic_runner_launcher_hook_execution_allowed"] is False
    assert result["runner_mutation_allowed"] is False
    assert result["launcher_mutation_allowed"] is False
    assert result["orders_submitted_by_generic_runner_launcher_hook_preflight"] == 0


def test_missing_engine_hook_report_warns_fail_closed(tmp_path):
    _seed_sources(tmp_path)
...
def test_missing_launcher_marker_warns_fail_closed(tmp_path):
    data = tmp_path / "data"
    _seed_sources(tmp_path)
    _seed_ready_reports(data)
    _write(tmp_path / "trading_bot/avvia_bot_live.py", "Live real-money execution is disabled\n")

    result = run_generic_paper_cycle_runner_launcher_hook_preflight(Settings(project_root=tmp_path))

    assert result["status"] == "WARN"
    assert "generic_runner_launcher_hook_source_markers_missing" in result["blockers"]
    assert result["generic_runner_launcher_required_markers_present"] is False
    assert result["generic_submit_execution_allowed"] is False


def test_runner_launcher_hook_map_is_read_only_and_non_ordinal(tmp_path):
    data = tmp_path / "data"
    _seed_sources(tmp_path)
    _seed_ready_reports(data)

    result = run_generic_paper_cycle_runner_launcher_hook_preflight(Settings(project_root=tmp_path))
    hook_map = result["generic_paper_cycle_runner_launcher_hook_map"]

    assert hook_map["execution_enabled"] is False
    assert hook_map["mutation_enabled"] is False
    assert hook_map["runner_hook_enabled"] is False
    assert hook_map["launcher_hook_enabled"] is False
