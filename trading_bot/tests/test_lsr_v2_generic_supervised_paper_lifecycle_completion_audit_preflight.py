...
from trading_bot.core.lsr_v2_generic_supervised_paper_lifecycle_completion_audit_preflight import (
    KEEP_DIAGNOSTIC_DECISION,
    READY_DECISION,
    Settings,
    run_generic_supervised_paper_lifecycle_completion_audit_preflight,
)
...
def _u40_report() -> dict:
    ...
    report = {
        "decision": "LSR_V2_GENERIC_SUPERVISED_PAPER_POSTMORTEM_EXECUTION_READY",
        ...
        "postmortem_execution_diagnostic_count": 401,
        "postmortem_execution_contract_present_fields": REQUIRED,
        "postmortem_execution_contract_required_fields": REQUIRED,
        "postmortem_execution_contract_missing_fields": [],
        "generic_supervised_paper_postmortem_execution_map": execution_map,
...
def test_lifecycle_completion_preflight_ready_but_runtime_completion_blocked(tmp_path, monkeypatch):
    for key in list(os.environ):
        if key.startswith("LSR_V2"):
            monkeypatch.delenv(key, raising=False)
    _write_markers(tmp_path)
    _write_json(tmp_path / "data/lsr_v2_generic_supervised_paper_postmortem_execution_report.json", _u40_report())

    report = run_generic_supervised_paper_lifecycle_completion_audit_preflight(_settings(tmp_path))

    assert report["status"] == "PASS"
    assert report["decision"] == READY_DECISION
    assert report["generic_supervised_paper_lifecycle_completion_audit_preflight_ready"] is True
    assert report["generic_lifecycle_completion_audit_preflight_ready"] is True
    assert report["generic_lifecycle_completion_audit_preflight_model_ready"] is True
    assert report["generic_lifecycle_completion_audit_preflight_patch_ready"] is True
    assert report["paper_lifecycle_completion_audit_preflight_ready"] is True
    assert report["lifecycle_completion_audit_preflight_diagnostic_available"] is True
    assert report["lifecycle_completion_audit_preflight_diagnostic_count"] == 401
    assert report["lifecycle_completion_audit_preflight_contract_shape_modelable"] is True
    assert report["lifecycle_completion_audit_preflight_contract_missing_fields"] == []
    ...
    assert report["orders_submitted_by_generic_lifecycle_completion_audit_preflight"] == 0
    assert report["positions_closed_by_generic_lifecycle_completion_audit_preflight"] == 0
    assert report["paper_state_modified_by_generic_lifecycle_completion_audit_preflight"] is False
    preflight_map = report["generic_supervised_paper_lifecycle_completion_audit_preflight_map"]
    assert preflight_map["lifecycle_completion_audit_preflight_patch"] is True
    gate = preflight_map["lifecycle_completion_audit_preflight_gate_model"]
    assert gate["lifecycle_completion_audit_preflight_model_ready"] is True
    assert gate["lifecycle_completion_audit_allowed"] is False
    assert "postmortem_runtime_execution_absent" in gate["lifecycle_completion_audit_blocked_reasons"]
    assert "explicit_lifecycle_completion_audit_execution_patch_required" in gate["lifecycle_completion_audit_blocked_reasons"]


def test_lifecycle_completion_preflight_requires_upstream_postmortem_report(tmp_path):
    _write_markers(tmp_path)
    ...
    assert report["generic_supervised_paper_lifecycle_completion_audit_preflight_ready"] is False
    assert report["lifecycle_completion_audit_preflight_diagnostic_available"] is False
    assert report["generic_lifecycle_completion_audit_allowed"] is False
    assert report["would_run_lifecycle_completion_audit"] is False


def test_lifecycle_completion_preflight_rejects_not_ready_upstream_report(tmp_path):
    _write_markers(tmp_path)
    upstream = _u40_report()
    upstream["decision"] = "NOT_READY"
    _write_json(tmp_path / "data/lsr_v2_generic_supervised_paper_postmortem_execution_report.json", upstream)
...
def test_lifecycle_completion_preflight_requires_source_markers(tmp_path):
    _write_json(tmp_path / "data/lsr_v2_generic_supervised_paper_postmortem_execution_report.json", _u40_report())
...
def test_lifecycle_completion_operator_env_cannot_override_missing_runtime_evidence(tmp_path, monkeypatch):
    _write_markers(tmp_path)
    _write_json(tmp_path / "data/lsr_v2_generic_supervised_paper_postmortem_execution_report.json", _u40_report())
    ...
    assert report["paper_lifecycle_completion_ready"] is False
    assert report["would_run_lifecycle_completion_audit"] is False
    reasons = report["generic_supervised_paper_lifecycle_completion_audit_preflight_map"]["lifecycle_completion_audit_preflight_gate_model"]["lifecycle_completion_audit_blocked_reasons"]
