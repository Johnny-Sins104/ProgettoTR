...
from trading_bot.core.lsr_v2_second_trade_close_preflight import (
    EVENT_TYPE,
    READY_DECISION,
...
def _seed_second_close_required(tmp_path: Path, *, close_required: bool = True, status_open_positions: int = 1, with_position: bool = True) -> Path:
    data = tmp_path / "data"
    monitor = {
    ...
    _write_jsonl(data / "lsr_v2_second_open_position_monitor.jsonl", [monitor])
    _write_json(data / "lsr_v2_second_open_position_monitor_report.json", {
        "status": "WARN" if close_required else "PASS",
        "decision": "KEEP_DIAGNOSTIC_LSR_V2_SECOND_CLOSE_REQUIRED_DIAGNOSTIC" if close_required else "LSR_V2_SECOND_OPEN_POSITION_MONITOR_READY",
        "cycle_id": "pc_second",
        "monitor_events": 1,
    ...
    _write_json(data / "lsr_v2_second_trade_position_lifecycle_report.json", {
        "status": "PASS",
        "decision": "LSR_V2_SECOND_TRADE_POSITION_LIFECYCLE_READY",
        "cycle_id": "pc_second",
        "paper_state_consistency": True,
        "paper_status_consistency": True,
...
def test_second_close_preflight_event_never_closes() -> None:
    event = build_lsr_v2_second_trade_close_preflight_event(
        monitor_event={
            "cycle_id": "pc_second",
            "symbol": "BTC/USDT",
            "side": "BUY",
        ...
        position={"position_id": "p2", "symbol": "BTC/USDT", "side": "BUY", "status": "OPEN", "open": True},
...
def test_report_ready_when_second_tp_close_required(tmp_path: Path) -> None:
    data = _seed_second_close_required(tmp_path, close_required=True)
    report = build_lsr_v2_second_trade_close_preflight_report_from_files(data_dir=data)
    ...
    rows = [json.loads(line) for line in (data / "lsr_v2_second_trade_close_preflight.jsonl").read_text().splitlines()]
    assert rows[0]["close_reason"] == "TAKE_PROFIT_HIT_DIAGNOSTIC"
    assert rows[0]["source"] == "lsr_v2_second_open_position_monitor"


def test_report_second_close_not_required(tmp_path: Path) -> None:
    data = _seed_second_close_required(tmp_path, close_required=False)
    report = build_lsr_v2_second_trade_close_preflight_report_from_files(data_dir=data)
...
def test_report_second_position_not_found(tmp_path: Path) -> None:
    data = _seed_second_close_required(tmp_path, close_required=True, status_open_positions=0, with_position=False)
    report = build_lsr_v2_second_trade_close_preflight_report_from_files(data_dir=data)
    assert report["decision"] == POSITION_NOT_FOUND_DECISION
    assert "matching_second_position_not_found" in report["blockers"]
    assert report["orders_submitted_by_second_trade_close_preflight"] == 0


def test_report_second_state_inconsistent_when_status_mismatch(tmp_path: Path) -> None:
    data = _seed_second_close_required(tmp_path, close_required=True, status_open_positions=0, with_position=True)
    report = build_lsr_v2_second_trade_close_preflight_report_from_files(data_dir=data)
    assert report["decision"] == STATE_INCONSISTENT_DECISION
    assert "paper_status_inconsistent" in report["blockers"]


def test_second_env_state_requires_exact_confirmation(monkeypatch) -> None:
    monkeypatch.setenv("LSR_V2_SECOND_TRADE_CLOSE_PREFLIGHT_ENABLE", "1")
    monkeypatch.setenv("LSR_V2_SECOND_TRADE_CLOSE_PREFLIGHT_CONFIRMATION", "wrong")
    state = second_close_preflight_env_state()
