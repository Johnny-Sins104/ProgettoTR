...
from trading_bot.core.lsr_v2_generic_supervised_paper_integrated_operation_readiness_execution_scaffold import (
    READY_DECISION,
    Settings,
...
def _base_u50_report() -> dict:
    ...
    return {
        "status": "PASS",
        "decision": "LSR_V2_GENERIC_SUPERVISED_PAPER_INTEGRATED_OPERATION_READINESS_PREFLIGHT_READY",
        "generic_supervised_paper_integrated_operation_readiness_preflight_ready": True,
        "generic_integrated_operation_readiness_preflight_ready": True,
        "generic_integrated_operation_readiness_preflight_model_ready": True,
        "generic_integrated_operation_readiness_preflight_patch_ready": True,
        "generic_integrated_operation_readiness_preflight_allowed": False,
        "integrated_operation_readiness_preflight_diagnostic_available": True,
        "integrated_operation_readiness_preflight_diagnostic_count": 401,
        "generic_supervised_paper_integrated_operation_readiness_preflight_map": {
            ...
            "integrated_operation_readiness_preflight_gate_model": gate,
        },
...
def _write_u50_report(tmp_path: Path, payload: dict | None = None) -> None:
    _write_json(
        tmp_path / "data/lsr_v2_generic_supervised_paper_integrated_operation_readiness_preflight_report.json",
        payload if payload is not None else _base_u50_report(),
    )


def test_integrated_operation_readiness_execution_scaffold_ready_but_fail_closed(tmp_path: Path) -> None:
    _write_markers(tmp_path)
    _write_u50_report(tmp_path)

    report = run_generic_supervised_paper_integrated_operation_readiness_execution_scaffold(_settings(tmp_path))

    assert report["status"] == "PASS"
    assert report["decision"] == READY_DECISION
    assert report["generic_supervised_paper_integrated_operation_readiness_execution_scaffold_ready"] is True
    assert report["generic_integrated_operation_readiness_execution_scaffold_model_ready"] is True
    assert report["paper_integrated_operation_readiness_execution_scaffold_ready"] is True
    assert report["generic_integrated_operation_readiness_execution_scaffold_allowed"] is False
...
def test_gate_model_retains_explicit_future_execution_patch_requirement(tmp_path: Path) -> None:
    _write_markers(tmp_path)
    _write_u50_report(tmp_path)

    report = run_generic_supervised_paper_integrated_operation_readiness_execution_scaffold(_settings(tmp_path))
    gate = report["generic_supervised_paper_integrated_operation_readiness_execution_scaffold_map"][
        "integrated_operation_readiness_execution_scaffold_gate_model"
    ]

    assert gate["integrated_operation_readiness_execution_scaffold_gate_model_ready"] is True
    assert gate["integrated_operation_readiness_execution_scaffold_allowed"] is False
    assert gate["future_integrated_operation_execution_patch_required"] is True
    assert "explicit_integrated_operation_execution_patch_required" in gate[
        "integrated_operation_readiness_execution_scaffold_blocked_reasons"
    ]
...
def test_not_ready_upstream_report_fails_closed(tmp_path: Path) -> None:
    _write_markers(tmp_path)
    bad = _base_u50_report()
    bad["decision"] = "KEEP_DIAGNOSTIC"
    _write_u50_report(tmp_path, bad)
...
def test_report_and_jsonl_are_written(tmp_path: Path) -> None:
    ...
    report_path = tmp_path / "data/lsr_v2_generic_supervised_paper_integrated_operation_readiness_execution_scaffold_report.json"
    jsonl_path = tmp_path / "data/lsr_v2_generic_supervised_paper_integrated_operation_readiness_execution_scaffold.jsonl"
    ...
    assert report["report"].endswith("lsr_v2_generic_supervised_paper_integrated_operation_readiness_execution_scaffold_report.json")
