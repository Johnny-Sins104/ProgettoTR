...
from trading_bot.core.lsr_v2_fourth_trade_operator_unlock_draft import (
    ...
    build_lsr_v2_fourth_trade_operator_unlock_draft_report_from_files,
)
...
def _write_source_tree(root: Path, *, missing_marker: bool = False) -> None:
    ...
    if missing_marker:
        files["trading_bot/core/lsr_v2_fourth_trade_operator_rearm_gate_preflight.py"] = "# missing unlock marker\n"
...
def test_unlock_draft_passes_default_but_keeps_permissions_disabled(tmp_path: Path) -> None:
    data, project = _seed(tmp_path)
    report = build_lsr_v2_fourth_trade_operator_unlock_draft_report_from_files(data_dir=data, project_root=project)
    assert report["status"] == "PASS"
    assert report["decision"] == PASS_DECISION
    assert report["operator_unlock_draft_ready"] is True
    assert report["guarded_operator_unlock_draft_ready"] is True
    assert report["operator_unlock_env_model_ready"] is True
    assert report["operator_unlock_draft_armed"] is False
    assert report["operator_unlock_would_rearm_fourth_trade"] is False
    assert report["operator_unlock_enable_env_name"] == OPERATOR_UNLOCK_ENABLE_ENV
    assert report["operator_unlock_confirmation_env_name"] == OPERATOR_UNLOCK_CONFIRMATION_ENV
    assert report["operator_unlock_max_positions_env_name"] == OPERATOR_UNLOCK_MAX_POSITIONS_ENV
    assert report["operator_unlock_confirmation_phrase"] == OPERATOR_UNLOCK_CONFIRMATION_PHRASE
    assert report["operator_unlock_allowed"] is False
    ...
    assert report["orders_submitted_by_operator_unlock_draft"] == 0
    assert report["positions_opened_by_operator_unlock_draft"] == 0
    assert report["positions_closed_by_operator_unlock_draft"] == 0


def test_valid_operator_env_is_only_draft_armed_and_still_disables_execution(monkeypatch, tmp_path: Path) -> None:
    data, project = _seed(tmp_path)
    ...
    assert report["status"] == "PASS"
    assert report["decision"] == PASS_DECISION
    assert report["operator_unlock_draft_armed"] is True
    assert report["operator_unlock_controls_valid_for_future_patch"] is True
    assert report["operator_unlock_would_rearm_fourth_trade"] is True
    assert report["operator_unlock_allowed"] is False
...
def test_invalid_operator_env_warns(tmp_path: Path, monkeypatch) -> None:
    ...
    assert "operator_unlock_env_invalid" in report["blockers"]
    assert report["operator_unlock_draft_armed"] is False


def test_missing_operator_gate_report_warns(tmp_path: Path) -> None:
    data, project = _seed(tmp_path, missing_report=True)
    report = build_lsr_v2_fourth_trade_operator_unlock_draft_report_from_files(data_dir=data, project_root=project)
    assert report["status"] == "WARN"
    assert report["decision"] == REPORTS_NOT_READY_DECISION
    assert "required_operator_unlock_draft_reports_not_ready" in report["blockers"]
...
def test_missing_source_marker_warns(tmp_path: Path) -> None:
    ...
    assert "operator_unlock_required_markers_missing" in report["blockers"]
