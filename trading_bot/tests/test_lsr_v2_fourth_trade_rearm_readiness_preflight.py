...
from trading_bot.core.lsr_v2_fourth_trade_rearm_readiness_preflight import (
    ENV_ACTIVE_DECISION,
    FAILED_DECISION,
    PASS_DECISION,
...
def _write_source_tree(root: Path, *, missing_marker: bool = False) -> None:
    files = {
        ...
        "trading_bot/core/lsr_v2_three_trade_postmortem_stability_lock.py": "LSR_V2_THREE_TRADE_POSTMORTEM\nfourth_trade_locked=True\n",
...
def test_rearm_readiness_preflight_passes_but_keeps_execution_disabled(tmp_path: Path) -> None:
    data, project = _seed(tmp_path)
    report = build_lsr_v2_fourth_trade_rearm_readiness_preflight_report_from_files(data_dir=data, project_root=project)
    assert report["status"] == "PASS"
    assert report["decision"] == PASS_DECISION
    assert report["rearm_readiness_preflight_ready"] is True
    assert report["fourth_trade_rearm_readiness_ready"] is True
    ...
    assert report["orders_submitted_by_fourth_rearm_readiness_preflight"] == 0
    assert report["positions_opened_by_fourth_rearm_readiness_preflight"] == 0
    assert report["positions_closed_by_fourth_rearm_readiness_preflight"] == 0


def test_missing_candidate_hook_report_warns(tmp_path: Path) -> None:
    data, project = _seed(tmp_path, missing_report=True)
    report = build_lsr_v2_fourth_trade_rearm_readiness_preflight_report_from_files(data_dir=data, project_root=project)
    assert report["status"] == "WARN"
    assert report["decision"] == REPORTS_NOT_READY_DECISION
    assert "required_readiness_reports_not_ready" in report["blockers"]


def test_missing_rearm_source_marker_warns(tmp_path: Path) -> None:
    data, project = _seed(tmp_path, missing_marker=True)
    report = build_lsr_v2_fourth_trade_rearm_readiness_preflight_report_from_files(data_dir=data, project_root=project)
    assert report["status"] == "WARN"
    assert report["decision"] == SOURCE_MARKERS_MISSING_DECISION
    assert "rearm_required_markers_missing" in report["blockers"]
