...
def _write_markers(root: Path) -> None:
    ...
    (root / "trading_bot/core/lsr_v2_generic_paper_only_order_intent_candidate_audit.py").write_text(
        "GENERIC_LSR_V2_PAPER_ONLY_ORDER_INTENT_CANDIDATE_AUDIT\n"
        "order_intent_candidate_contract_shape_modelable\n",
        encoding="utf-8",
    )
    (root / "trading_bot/core/paper_engine.py").write_text("class PaperTradingEngine:\n    pass\n", encoding="utf-8")
...
def _u21_report(**overrides: object) -> dict:
    ...
    payload = {
        ...
        "order_intent_candidate_contract_shape_modelable": True,
        ...
        "order_intent_candidate_audit_map": {
...
def test_order_intent_validation_preflight_passes_with_u21_candidate_audit(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.delenv("LSR_V2_GENERIC_REARM_ENABLE", raising=False)
    _write_markers(tmp_path)
    _write_json(
        tmp_path / "data/lsr_v2_generic_paper_only_order_intent_candidate_audit_report.json",
        _u21_report(),
    )

    report = run_generic_paper_only_order_intent_validation_preflight(Settings(project_root=tmp_path))

    assert report["status"] == "PASS"
    assert report["decision"] == READY_DECISION
    assert report["generic_order_intent_validation_preflight_ready"] is True
    assert report["paper_order_intent_contract_ready"] is True
    assert report["paper_order_intent_contract_validatable"] is True
    ...
    assert report["generic_order_intent_validation_execution_allowed"] is False
    ...
    assert report["orders_submitted_by_generic_order_intent_validation_preflight"] == 0
    assert report["positions_opened_by_generic_order_intent_validation_preflight"] == 0
    assert report["paper_state_modified_by_generic_order_intent_validation_preflight"] is False
    assert report["live_enabled"] is False
    assert report["testnet_enabled"] is False
    assert report["exchange_broker_enabled"] is False


def test_order_intent_validation_preflight_warns_when_upstream_missing(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.delenv("LSR_V2_GENERIC_REARM_ENABLE", raising=False)
    _write_markers(tmp_path)
    ...
    assert "order_intent_candidate_audit_report_present" in report["blockers"]
    assert report["generic_order_intent_validation_preflight_ready"] is False
    assert report["paper_order_intent_contract_ready"] is False
    assert report["generic_order_intent_creation_allowed"] is False


def test_order_intent_validation_preflight_warns_when_contract_field_missing(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.delenv("LSR_V2_GENERIC_REARM_ENABLE", raising=False)
    _write_markers(tmp_path)
    ...
    _write_json(
        ...
        _u21_report(
            ...
            model=_model(take_profit="deferred_until_explicit_order_intent_patch"),
        ),
    )
    ...
    assert report["status"] == "WARN"
    assert "present_contract_fields_complete" in report["blockers"]
    assert "missing_contract_fields_empty" in report["blockers"]
    assert report["paper_order_intent_validated"] is False
    assert report["would_submit"] is False


def test_order_intent_validation_preflight_warns_when_policy_contract_invalid(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.delenv("LSR_V2_GENERIC_REARM_ENABLE", raising=False)
    _write_markers(tmp_path)
    _write_json(
        tmp_path / "data/lsr_v2_generic_paper_only_order_intent_candidate_audit_report.json",
        _u21_report(model=_model(max_open_positions=2)),
    )

    report = run_generic_paper_only_order_intent_validation_preflight(Settings(project_root=tmp_path))

    assert report["status"] == "WARN"
    assert "validation_contract_policy_ready" in report["blockers"]
    assert report["order_intent_validation_preflight_map"]["single_position_policy_preflight"]["max_open_positions_valid"] is False
    assert report["generic_order_intent_validation_allowed"] is False


def test_order_intent_validation_preflight_warns_when_order_intent_materialized(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.delenv("LSR_V2_GENERIC_REARM_ENABLE", raising=False)
    _write_markers(tmp_path)
    _write_json(
        tmp_path / "data/lsr_v2_generic_paper_only_order_intent_candidate_audit_report.json",
        _u21_report(paper_order_intent_materialized=True),
    )

    report = run_generic_paper_only_order_intent_validation_preflight(Settings(project_root=tmp_path))

    assert report["status"] == "WARN"
    assert "no_order_intent_materialized" in report["blockers"]
    assert report["paper_order_intent_materialized"] is False
    assert report["would_submit"] is False


def test_order_intent_validation_preflight_warns_when_validation_or_creation_unblocked(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.delenv("LSR_V2_GENERIC_REARM_ENABLE", raising=False)
    _write_markers(tmp_path)
    _write_json(
        tmp_path / "data/lsr_v2_generic_paper_only_order_intent_candidate_audit_report.json",
        _u21_report(generic_order_intent_validation_allowed=True, generic_order_intent_creation_allowed=True),
    )

    report = run_generic_paper_only_order_intent_validation_preflight(Settings(project_root=tmp_path))

    assert report["status"] == "WARN"
    assert "order_intent_validation_execution_blocked" in report["blockers"]
...
def test_order_intent_validation_preflight_warns_on_operator_env(tmp_path: Path, monkeypatch) -> None:
