...
from trading_bot.core.lsr_v2_second_trade_close_execution import (
    CLOSED_DECISION,
    NOT_ENABLED_DECISION,
    CONFIRMATION_MISSING_DECISION,
    POSITION_NOT_FOUND_DECISION,
    STATE_INCONSISTENT_DECISION,
    build_lsr_v2_second_trade_close_execution_report_from_files,
)
...
def _seed_ready(tmp_path: Path, *, with_position: bool = True, status_open_positions: int = 1) -> Path:
    ...
    _write_jsonl(data / "lsr_v2_second_trade_close_preflight.jsonl", [event])
    _write_json(data / "lsr_v2_second_trade_close_preflight_report.json", {
        "status": "PASS",
    ...
    _write_jsonl(data / "paper_events.jsonl", [{"event_type": "CYCLE_COMPLETED", "cycle_id": "pc_second_close"}])
    return data
...
def test_default_not_enabled_does_not_mutate(tmp_path: Path) -> None:
    ...
    report = build_lsr_v2_second_trade_close_execution_report_from_files(data_dir=data)
    after = json.loads((data / "paper_state.json").read_text())
    assert report["decision"] == NOT_ENABLED_DECISION
    assert report["positions_closed_by_second_trade_close_execution"] == 0
...
def test_enabled_without_confirmation_blocks(tmp_path: Path, monkeypatch) -> None:
    data = _seed_ready(tmp_path)
    monkeypatch.setenv("LSR_V2_SECOND_TRADE_CLOSE_ENABLE", "1")
    report = build_lsr_v2_second_trade_close_execution_report_from_files(data_dir=data)
    assert report["decision"] == CONFIRMATION_MISSING_DECISION
    assert report["positions_closed_by_second_trade_close_execution"] == 0


def test_confirmed_close_marks_second_position_closed_and_updates_status(tmp_path: Path, monkeypatch) -> None:
    data = _seed_ready(tmp_path)
    monkeypatch.setenv("LSR_V2_SECOND_TRADE_CLOSE_ENABLE", "1")
    monkeypatch.setenv("LSR_V2_SECOND_TRADE_CLOSE_CONFIRMATION", "I_UNDERSTAND_CLOSE_SECOND_PAPER_POSITION_ONLY")
    monkeypatch.setenv("LSR_V2_SECOND_TRADE_CLOSE_MAX_POSITIONS", "1")
    report = build_lsr_v2_second_trade_close_execution_report_from_files(data_dir=data)
    assert report["status"] == "PASS"
    assert report["decision"] == CLOSED_DECISION
    assert report["positions_closed_by_second_trade_close_execution"] == 1
    assert report["broker_close_called"] is True
    assert report["orders_submitted_by_second_trade_close_execution"] == 0
    assert report["positions_opened_by_second_trade_close_execution"] == 0
    assert report["open_second_lsr_v2_positions_after"] == 0
    ...
    pos = state["positions"]["pos_second"]
    assert pos["status"] == "CLOSED"
    assert pos["open"] is False
    assert pos["close_reason"] == "TAKE_PROFIT_HIT_DIAGNOSTIC"
    assert pos["realized_pnl"] == 24.0
    assert pos["metadata"]["trade_sequence"] == "SECOND"
    status = json.loads((data / "paper_status.json").read_text())
    assert status["open_positions"] == 0
    rows = [json.loads(line) for line in (data / "lsr_v2_second_trade_close_execution.jsonl").read_text().splitlines()]
    assert len(rows) == 1
    assert rows[0]["event_type"] == "LSR_V2_SECOND_TRADE_CLOSE_EXECUTION"
...
def test_max_position_cap_blocks_duplicates(tmp_path: Path, monkeypatch) -> None:
    data = _seed_ready(tmp_path)
    state = json.loads((data / "paper_state.json").read_text())
    state["positions"]["pos_second_2"] = dict(state["positions"]["pos_second"], position_id="pos_second_2")
    ...
    monkeypatch.setenv("LSR_V2_SECOND_TRADE_CLOSE_MAX_POSITIONS", "1")
    report = build_lsr_v2_second_trade_close_execution_report_from_files(data_dir=data)
    assert report["decision"] == "KEEP_DIAGNOSTIC_LSR_V2_SECOND_CLOSE_MAX_POSITION_CAP_BLOCKED"
    assert report["positions_closed_by_second_trade_close_execution"] == 0
