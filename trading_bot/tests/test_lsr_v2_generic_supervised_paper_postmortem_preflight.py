...
from trading_bot.core.lsr_v2_generic_supervised_paper_postmortem_preflight import (
    READY_DECISION,
    KEEP_DIAGNOSTIC_DECISION,
    Settings,
    run_generic_supervised_paper_postmortem_preflight,
)
...
def _u37_report() -> dict:
    final_map = {
        "dry_run_order_intent_payload": _dry_run_payload(),
        "final_audit_execution_gate_model": {
    ...
    report = {
        "decision": "LSR_V2_GENERIC_SUPERVISED_PAPER_FINAL_AUDIT_EXECUTION_READY",
        "generic_supervised_paper_final_audit_execution_ready": True,
        "generic_final_audit_execution_model_ready": True,
        "generic_final_audit_execution_patch_ready": True,
        "generic_final_audit_execution_gate_model_ready": True,
        ...
        "final_audit_execution_contract_present_fields": REQUIRED,
        "final_audit_execution_contract_required_fields": REQUIRED,
        "final_audit_execution_contract_missing_fields": [],
        "generic_supervised_paper_final_audit_execution_map": final_map,
...
def test_postmortem_preflight_ready_but_no_runtime_postmortem_without_final_audit(tmp_path, monkeypatch):
    for key in list(os.environ):
        if key.startswith("LSR_V2"):
            monkeypatch.delenv(key, raising=False)
    _write_markers(tmp_path)
    _write_json(tmp_path / "data/lsr_v2_generic_supervised_paper_final_audit_execution_report.json", _u37_report())

    report = run_generic_supervised_paper_postmortem_preflight(_settings(tmp_path))

    assert report["status"] == "PASS"
    assert report["decision"] == READY_DECISION
    assert report["generic_supervised_paper_postmortem_preflight_ready"] is True
    assert report["generic_postmortem_preflight_ready"] is True
    assert report["generic_postmortem_preflight_model_ready"] is True
    assert report["generic_postmortem_preflight_patch_ready"] is True
    assert report["paper_postmortem_preflight_ready"] is True
    assert report["postmortem_preflight_diagnostic_available"] is True
    assert report["postmortem_preflight_diagnostic_count"] == 401
    assert report["postmortem_preflight_contract_shape_modelable"] is True
    assert report["postmortem_preflight_contract_missing_fields"] == []
    ...
    assert report["orders_submitted_by_generic_postmortem_preflight"] == 0
    assert report["positions_closed_by_generic_postmortem_preflight"] == 0
    assert report["paper_state_modified_by_generic_postmortem_preflight"] is False
    assert report["paper_status_modified_by_generic_postmortem_preflight"] is False
    postmortem_map = report["generic_supervised_paper_postmortem_preflight_map"]
    assert postmortem_map["preflight_only"] is True
    assert postmortem_map["postmortem_preflight_gate_model"]["postmortem_preflight_model_ready"] is True
    assert postmortem_map["postmortem_preflight_gate_model"]["postmortem_execution_allowed"] is False
    assert "final_audit_execution_not_allowed" in postmortem_map["postmortem_preflight_gate_model"]["postmortem_preflight_blocked_reasons"]


def test_postmortem_preflight_requires_upstream_final_audit_execution_report(tmp_path):
    _write_markers(tmp_path)
    ...
    assert report["generic_supervised_paper_postmortem_preflight_ready"] is False
    assert report["postmortem_preflight_diagnostic_available"] is False
    assert report["generic_postmortem_execution_allowed"] is False
    assert report["would_run_postmortem"] is False


def test_postmortem_preflight_rejects_not_ready_upstream_report(tmp_path):
    _write_markers(tmp_path)
    upstream = _u37_report()
    upstream["decision"] = "NOT_READY"
    _write_json(tmp_path / "data/lsr_v2_generic_supervised_paper_final_audit_execution_report.json", upstream)
...
def test_postmortem_preflight_requires_source_markers(tmp_path):
    _write_json(tmp_path / "data/lsr_v2_generic_supervised_paper_final_audit_execution_report.json", _u37_report())
...
def test_postmortem_operator_env_cannot_override_missing_real_final_audit(tmp_path, monkeypatch):
    _write_markers(tmp_path)
    _write_json(tmp_path / "data/lsr_v2_generic_supervised_paper_final_audit_execution_report.json", _u37_report())
    ...
    assert report["paper_postmortem_ready"] is False
    assert report["would_run_postmortem"] is False
    reasons = report["generic_supervised_paper_postmortem_preflight_map"]["postmortem_preflight_gate_model"]["postmortem_preflight_blocked_reasons"]
    assert "final_audit_execution_not_allowed" in reasons
