#!/usr/bin/env python
"""Prompt 29.4.4s-10am — LSR-v2 third-trade submit preflight runner."""
...
try:
    from core.lsr_v2_third_trade_submit_preflight import build_lsr_v2_third_trade_submit_preflight_report_from_files  # type: ignore
except Exception:
    from trading_bot.core.lsr_v2_third_trade_submit_preflight import build_lsr_v2_third_trade_submit_preflight_report_from_files  # type: ignore


def main() -> int:
    parser = argparse.ArgumentParser(description="LSR-v2 third supervised paper trade submit preflight")
    parser.add_argument("--data-dir", default="data")
    parser.add_argument("--cycle-id", default="", help="Optional cycle_id; defaults to handoff dry-run report cycle")
    args = parser.parse_args()

    report = build_lsr_v2_third_trade_submit_preflight_report_from_files(
    ...
    summary = {
        ...
        "third_trade_handoff_ready": report.get("third_trade_handoff_ready"),
        "handoff_pass": report.get("handoff_pass"),
        "handoff_dry_run_events": report.get("handoff_dry_run_events"),
        "creatable_handoff_events": report.get("creatable_handoff_events"),
