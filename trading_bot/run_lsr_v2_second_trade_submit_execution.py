#!/usr/bin/env python
"""Prompt 29.4.4s-10y — LSR-v2 second supervised paper submit execution runner."""
...
try:
    from core.lsr_v2_second_trade_submit_execution import (  # type: ignore
        LSRV2SecondTradeSubmitExecutionSettings,
        build_lsr_v2_second_trade_submit_execution_report_from_files,
    )
except Exception:
    from trading_bot.core.lsr_v2_second_trade_submit_execution import (  # type: ignore
        LSRV2SecondTradeSubmitExecutionSettings,
        build_lsr_v2_second_trade_submit_execution_report_from_files,
    )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="LSR-v2 second supervised paper-only submit execution")
    parser.add_argument("--data-dir", default="data")
    parser.add_argument("--cycle-id", default="", help="Optional cycle_id. Defaults to latest second-trade submit-boundary cycle.")
    args = parser.parse_args(argv)

    settings = LSRV2SecondTradeSubmitExecutionSettings.from_env(data_dir=str(Path(args.data_dir)))
    allow_project_submitter = bool(settings.execute_armed and settings.execute_confirmation_ok)
    report = build_lsr_v2_second_trade_submit_execution_report_from_files(
    ...
    summary = {
        ...
        "second_trade_execute_armed": report.get("second_trade_execute_armed"),
        "second_trade_execute_confirmation_ok": report.get("second_trade_execute_confirmation_ok"),
        ...
        "broker_submit_called_by_second_trade_execution": report.get("broker_submit_called_by_second_trade_execution"),
        "orders_submitted_by_second_trade_execution": report.get("orders_submitted_by_second_trade_execution"),
        "positions_opened_by_second_trade_execution": report.get("positions_opened_by_second_trade_execution"),
        "positions_closed_by_second_trade_execution": report.get("positions_closed_by_second_trade_execution"),
