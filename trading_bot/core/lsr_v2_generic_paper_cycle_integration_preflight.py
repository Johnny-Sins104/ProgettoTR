"""Generic LSR-v2 paper-cycle integration preflight.

29.4.4u-5 is intentionally read-only and fail-closed. It verifies that the
new generic controller, order lifecycle, and supervised re-arm policy drafts
can be integrated as a future paper-only LSR-v2 cycle without extending the
old ordinal trade patch chain. It does not enable candidate detection, route,
...
PROMPT = "29.4.4u-5"
EVENT_TYPE = "LSR_V2_GENERIC_PAPER_CYCLE_INTEGRATION_PREFLIGHT"
READY_DECISION = "LSR_V2_GENERIC_PAPER_CYCLE_INTEGRATION_PREFLIGHT_READY"
KEEP_DIAGNOSTIC_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_GENERIC_PAPER_CYCLE_INTEGRATION_PREFLIGHT_NOT_READY"
...
T17_READY_DECISION = "LSR_V2_FOURTH_SUPERVISED_PAPER_SUBMIT_EXECUTION_READY"
...
REQUIRED_UPSTREAM_REPORTS: Tuple[Tuple[str, str, str], ...] = (
    ...
    ("fourth_submit_execution_scaffold", "lsr_v2_fourth_trade_submit_execution_report.json", T17_READY_DECISION),
...
REQUIRED_SOURCE_MARKERS: Tuple[Tuple[str, Tuple[str, ...]], ...] = (
    (
        ...
        ("GENERIC_LSR_V2_PAPER_CYCLE_CONTROLLER_DRAFT", "generic_cycle_controller"),
    ),
    (
        "trading_bot/core/lsr_v2_paper_order_lifecycle.py",
        ("GENERIC_LSR_V2_PAPER_ORDER_LIFECYCLE_DRAFT", "generic_order_lifecycle"),
    ),
    (
        "trading_bot/core/lsr_v2_supervised_rearm_policy.py",
        ("GENERIC_LSR_V2_SUPERVISED_REARM_POLICY_DRAFT", "generic_rearm"),
    ),
)

GENERIC_INTEGRATION_STAGES: Tuple[str, ...] = (
    ...
    "verify_operator_gate_model",
    "verify_cooldown_policy",
    "map_candidate_diagnostic_stage",
    "map_route_preflight_stage",
    "map_handoff_dry_run_stage",
    "map_submit_preflight_stage",
    "map_guarded_submit_stage",
    "map_open_monitoring_stage",
    "map_close_preflight_stage",
    "map_guarded_close_stage",
    "map_final_audit_stage",
    "map_postmortem_stage",
)

BLOCKED_UNTIL_EXPLICIT_GENERIC_INTEGRATION_IMPLEMENTATION_PATCH: Tuple[str, ...] = (
    "generic_cycle_integration_activation",
...
class Settings:
    ...
    data_dir: str = "data"
    report_name: str = "lsr_v2_generic_paper_cycle_integration_preflight_report.json"
    jsonl_name: str = "lsr_v2_generic_paper_cycle_integration_preflight.jsonl"
...
def _build_integration_map() -> Dict[str, Any]:
    return {
        "mode": "preflight_only",
        "paper_only": True,
        ...
        "stages": [
            ...
            for stage in GENERIC_INTEGRATION_STAGES
        ],
        "modules": {
            "controller": "core/lsr_v2_paper_cycle_controller.py",
            "order_lifecycle": "core/lsr_v2_paper_order_lifecycle.py",
            "rearm_policy": "core/lsr_v2_supervised_rearm_policy.py",
            "future_dashboard": "core/lsr_v2_trade_dashboard_generic.py",
            "future_postmortem": "core/lsr_v2_trade_postmortem_generic.py",
        },
    }


def run_generic_paper_cycle_integration_preflight(settings: Optional[Settings] = None) -> Dict[str, Any]:
    if settings is None:
        settings = Settings()
    ...
    t17 = reports.get("fourth_submit_execution_scaffold", {})
    ...
    fourth_trade_locked = _as_bool(u4.get("fourth_trade_locked", u3.get("fourth_trade_locked", u2.get("fourth_trade_locked", t17.get("fourth_trade_locked", postmortem.get("fourth_trade_locked", True))))))
    stability_lock_active = _as_bool(u4.get("stability_lock_active", u3.get("stability_lock_active", u2.get("stability_lock_active", t17.get("stability_lock_active", postmortem.get("stability_lock_active", True))))))

    ordinal_expansion_blocked = (
        u4.get("ordinal_trade_patch_expansion_allowed") is False
        and u4.get("fifth_trade_patch_allowed") is False
        and u4.get("sixth_trade_patch_allowed") is False
        and u4.get("seventh_trade_patch_allowed") is False
        and u3.get("ordinal_trade_patch_expansion_allowed") is False
        and u2.get("ordinal_trade_patch_expansion_allowed") is False
        and u1.get("ordinal_trade_patch_expansion_allowed") is False
    )

    no_order_intent = not _as_bool(t17.get("paper_order_intent_ready", u3.get("paper_order_intent_ready", False)))
    no_route_candidate = not _as_bool(t17.get("route_candidate_available", u4.get("route_candidate_available", False)))
    no_submit_occurred = (
    ...
    if not source_markers_present:
        blockers.append("generic_source_markers_missing")
    ...
    result: Dict[str, Any] = {
        ...
        "classification_labels": [
            "GENERIC_LSR_V2_PAPER_CYCLE_INTEGRATION_PREFLIGHT",
            "READ_ONLY",
            "PREFLIGHT_ONLY",
            ...
            "NO_SCHEDULER",
            "FAIL_CLOSED",
        ] + (["GENERIC_CYCLE_INTEGRATION_PREFLIGHT_READY"] if ready else []),
        "blockers": blockers,
        "blocked_until_explicit_generic_integration_implementation_patch": list(BLOCKED_UNTIL_EXPLICIT_GENERIC_INTEGRATION_IMPLEMENTATION_PATCH),
        "generic_cycle_integration_preflight_ready": ready,
        "generic_cycle_integration_plan_ready": ready,
        "generic_cycle_integration_contract_ready": ready,
        "generic_controller_ready": upstream_ready.get("generic_cycle_controller", False),
        "generic_order_lifecycle_ready": upstream_ready.get("generic_order_lifecycle", False),
        "generic_rearm_policy_ready": upstream_ready.get("generic_rearm_policy", False),
        "generic_refactor_preflight_ready": upstream_ready.get("generic_refactor_preflight", False),
        "generic_engine_integration_map_ready": ready,
        "generic_launcher_runner_integration_map_ready": ready,
        "generic_paper_cycle_integration_map": _build_integration_map(),
        "generic_integration_target_modules": [
        ...
        "no_order_intent_currently_available": no_order_intent,
        "no_fourth_route_candidate": no_route_candidate,
        "lifecycle_state": t17.get("lifecycle_state", u4.get("lifecycle_state", "FLAT_LOCKED")),
        "fourth_trade_locked": fourth_trade_locked,
        "stability_lock_active": stability_lock_active,
        "flat_state_confirmed": flat_state_confirmed,
        ...
        "paper_state_modified_by_generic_cycle_integration_preflight": False,
        "paper_status_modified_by_generic_cycle_integration_preflight": False,
        ...
        "generic_source_files_present": not missing_source_files,
        "generic_required_source_markers_present": not missing_source_markers,
        "missing_generic_source_files": missing_source_files,
        "missing_generic_source_markers": missing_source_markers,
        ...
        "recommended_next_patch": "29.4.4u-6 — Generic LSR-v2 paper cycle engine hook preflight",
        "next_step": "prepare_generic_cycle_engine_hook_preflight_or_wait_for_real_candidate",
        "settings": {
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    _ = argv
    result = run_generic_paper_cycle_integration_preflight()
