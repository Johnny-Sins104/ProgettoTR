...
Read-only/fail-closed preflight that validates whether the already drafted and
activation-preflighted fourth-trade operator unlock model is ready to progress to
an explicit candidate-detection preflight phase.

This module never unlocks the fourth trade, never enables candidate detection,
routing, submit, close, scheduler, Telegram network send, live/testnet/exchange,
and never mutates paper_state or paper_status.
"""
...
PROMPT_ID = "29.4.4t-12"
EVENT_TYPE = "LSR_V2_FOURTH_TRADE_UNLOCK_ACTIVATION_CANDIDATE_PREFLIGHT"
REPORT_NAME = "lsr_v2_fourth_trade_unlock_activation_candidate_preflight_report.json"
JSONL_NAME = "lsr_v2_fourth_trade_unlock_activation_candidate_preflight.jsonl"

ACTIVATION_PREFLIGHT_REPORT_NAME = "lsr_v2_fourth_trade_operator_unlock_activation_preflight_report.json"
UNLOCK_DRAFT_REPORT_NAME = "lsr_v2_fourth_trade_operator_unlock_draft_report.json"
...
ACTIVATION_PREFLIGHT_PASS_DECISION = "LSR_V2_FOURTH_TRADE_OPERATOR_UNLOCK_ACTIVATION_PREFLIGHT_READY"
UNLOCK_DRAFT_PASS_DECISION = "LSR_V2_FOURTH_TRADE_OPERATOR_UNLOCK_DRAFT_READY"
...
PASS_DECISION = "LSR_V2_FOURTH_TRADE_UNLOCK_ACTIVATION_CANDIDATE_PREFLIGHT_READY"
REPORTS_NOT_READY_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_FOURTH_UNLOCK_CANDIDATE_PREFLIGHT_REPORTS_NOT_READY"
SOURCE_MARKERS_MISSING_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_FOURTH_UNLOCK_CANDIDATE_PREFLIGHT_SOURCE_MARKERS_MISSING"
STATE_NOT_LOCKED_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_FOURTH_UNLOCK_CANDIDATE_PREFLIGHT_STATE_NOT_LOCKED"
ENV_INVALID_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_FOURTH_UNLOCK_CANDIDATE_PREFLIGHT_ENV_INVALID"
FAILED_DECISION = "REJECT_LSR_V2_FOURTH_UNLOCK_CANDIDATE_PREFLIGHT_SAFETY_FAILED"
...
_OPERATOR_ENV_MARKERS = (
    "ARM", "EXECUTE", "ENABLE", "CONFIRMATION", "MAX_POSITIONS", "REARM", "UNLOCK", "SUBMIT", "CLOSE", "ROUTE",
)

_SOURCE_MARKERS: dict[str, tuple[str, ...]] = {
    "trading_bot/core/lsr_v2_fourth_trade_operator_unlock_activation_preflight.py": (
        "LSR_V2_FOURTH_TRADE_OPERATOR_UNLOCK_ACTIVATION_PREFLIGHT",
        "operator_unlock_activation_preflight_ready",
        "operator_unlock_activation_requires_execution_patch",
    ),
...
def _operator_env_controls(env_map: Mapping[str, str] | None = None) -> dict[str, Any]:
    ...
    max_positions_ok = max_positions == OPERATOR_UNLOCK_MAX_POSITIONS
    required_present = all(k in active for k in (OPERATOR_UNLOCK_ENABLE_ENV, OPERATOR_UNLOCK_CONFIRMATION_ENV, OPERATOR_UNLOCK_MAX_POSITIONS_ENV))
    unrelated = sorted(k for k in active if k not in {OPERATOR_UNLOCK_ENABLE_ENV, OPERATOR_UNLOCK_CONFIRMATION_ENV, OPERATOR_UNLOCK_MAX_POSITIONS_ENV})
    env_active = bool(active)
    controls_valid = bool(env_active and required_present and enable_requested and confirmation_ok and max_positions_ok and not unrelated)
    return {
        "active_lsr_v2_operator_env_count": len(active),
        "active_lsr_v2_operator_env_keys": sorted(active),
        "operator_env_absent": not env_active,
        "operator_unlock_activation_candidate_env_currently_active": env_active,
        "operator_unlock_activation_candidate_enable_env_name": OPERATOR_UNLOCK_ENABLE_ENV,
        "operator_unlock_activation_candidate_confirmation_env_name": OPERATOR_UNLOCK_CONFIRMATION_ENV,
        "operator_unlock_activation_candidate_max_positions_env_name": OPERATOR_UNLOCK_MAX_POSITIONS_ENV,
        "operator_unlock_activation_candidate_enable_env_value": enable_raw,
        "operator_unlock_activation_candidate_confirmation_env_value_present": bool(confirmation_raw),
        "operator_unlock_activation_candidate_max_positions_env_value": max_positions_raw,
        "operator_unlock_activation_candidate_enable_requested": enable_requested,
        "operator_unlock_activation_candidate_confirmation_ok": confirmation_ok,
        "operator_unlock_activation_candidate_confirmation_phrase": OPERATOR_UNLOCK_CONFIRMATION_PHRASE,
        "operator_unlock_activation_candidate_max_positions": OPERATOR_UNLOCK_MAX_POSITIONS,
        "operator_unlock_activation_candidate_max_positions_ok": max_positions_ok,
        "operator_unlock_activation_candidate_required_envs_present": required_present,
        "operator_unlock_activation_candidate_unrelated_env_keys": unrelated,
        "operator_unlock_activation_candidate_controls_valid": controls_valid,
        "operator_unlock_activation_candidate_preflight_armed": controls_valid,
    }


def _required_reports(data_dir: Path) -> dict[str, dict[str, Any]]:
    return {
        "activation_preflight": _read_json(data_dir / ACTIVATION_PREFLIGHT_REPORT_NAME),
        "unlock_draft": _read_json(data_dir / UNLOCK_DRAFT_REPORT_NAME),
        "operator_gate": _read_json(data_dir / OPERATOR_GATE_REPORT_NAME),
        "rearm_readiness": _read_json(data_dir / REARM_READINESS_REPORT_NAME),
        "candidate_hook": _read_json(data_dir / CANDIDATE_HOOK_REPORT_NAME),
        "handoff": _read_json(data_dir / HANDOFF_REPORT_NAME),
        "parity": _read_json(data_dir / PARITY_REPORT_NAME),
...
def _source_marker_audit(project_root: str | Path) -> dict[str, Any]:
    ...
    return {
        "unlock_activation_candidate_source_files_present": not missing_files,
        "unlock_activation_candidate_required_markers_present": not missing_files and not missing_markers,
        "missing_unlock_activation_candidate_source_files": missing_files,
        "missing_unlock_activation_candidate_source_markers": missing_markers,
    }
...
def build_lsr_v2_fourth_trade_unlock_activation_candidate_preflight_report(
    *,
    ...
    activation = reports.get("activation_preflight", {})
    unlock_draft = reports.get("unlock_draft", {})
    ...
    report_readiness = {
        "operator_unlock_activation_preflight_ready": _pass_report(activation, ACTIVATION_PREFLIGHT_PASS_DECISION) and _safe_bool(activation.get("operator_unlock_activation_preflight_ready"), False),
        "operator_unlock_draft_ready": _pass_report(unlock_draft, UNLOCK_DRAFT_PASS_DECISION) and _safe_bool(unlock_draft.get("operator_unlock_draft_ready"), False),
        "operator_rearm_gate_preflight_ready": _pass_report(operator_gate, OPERATOR_GATE_PASS_DECISION) and _safe_bool(operator_gate.get("operator_rearm_gate_preflight_ready"), False),
        "rearm_readiness_preflight_ready": _pass_report(rearm, REARM_READINESS_PASS_DECISION) and _safe_bool(rearm.get("rearm_readiness_preflight_ready"), False),
        "candidate_lifecycle_hook_preflight_ready": _pass_report(candidate_hook, CANDIDATE_HOOK_PASS_DECISION) and _safe_bool(candidate_hook.get("candidate_lifecycle_hook_preflight_ready"), False),
        "read_only_handoff_preflight_ready": _pass_report(handoff, HANDOFF_PASS_DECISION) and _safe_bool(handoff.get("read_only_handoff_preflight_ready"), False),
        "parity_audit_ready": _pass_report(parity, PARITY_PASS_DECISION) and _safe_bool(parity.get("launcher_runner_visibility_parity_ok"), False),
    ...
    inherited = [activation, unlock_draft, operator_gate, rearm, candidate_hook, handoff, parity, engine_hook, lifecycle, dashboard, postmortem]
    ...
    env_active = _safe_bool(env.get("operator_unlock_activation_candidate_env_currently_active"), False)
    env_valid = _safe_bool(env.get("operator_unlock_activation_candidate_controls_valid"), False)

    blockers: list[str] = []
    if not all(report_readiness.values()):
        blockers.append("required_unlock_activation_candidate_reports_not_ready")
    if not _safe_bool(source_audit.get("unlock_activation_candidate_required_markers_present"), False):
        blockers.append("unlock_activation_candidate_required_markers_missing")
    ...
    if env_active and not env_valid:
        blockers.append("operator_unlock_activation_candidate_env_invalid")
    if live_enabled:
    ...
    elif "operator_unlock_activation_candidate_env_invalid" in blockers:
        status = "WARN"; decision = ENV_INVALID_DECISION
    elif "unlock_activation_candidate_required_markers_missing" in blockers:
    ...
    elif "required_unlock_activation_candidate_reports_not_ready" in blockers:
    ...
    armed = preflight_ready and env_valid
    aggregate_realized_pnl = _safe_float(activation.get("aggregate_realized_pnl") or postmortem.get("aggregate_realized_pnl"), 0.0)
    balance_after = _safe_float(state.get("balance_after") or activation.get("balance_after"), 0.0)
    ...
    return {
        ...
        "classification_labels": [
            "FOURTH_TRADE_UNLOCK_ACTIVATION_CANDIDATE_PREFLIGHT",
            "ACTIVATION_CANDIDATE_PREFLIGHT_ONLY",
            "GUARDED_PAPER_ONLY",
            "NO_ENGINE_MUTATION",
            "NO_STATE_MUTATION",
            "NO_REENTRY",
            ...
            "NO_SCHEDULER",
            "FAIL_CLOSED",
        ] + (["OPERATOR_CONTROLS_VALID_BUT_CANDIDATE_DETECTION_STILL_BLOCKED"] if armed else []),
        **report_readiness,
        **state,
        **source_audit,
        **env,
        "unlock_activation_candidate_preflight_ready": preflight_ready,
        "guarded_unlock_activation_candidate_preflight_ready": preflight_ready,
        "candidate_detection_preflight_ready": preflight_ready,
        "candidate_detection_plan_ready": preflight_ready,
        "candidate_detection_preflight_armed": armed,
        "operator_unlock_activation_candidate_preflight_ready": preflight_ready,
        "operator_unlock_activation_candidate_preflight_armed": armed,
        "operator_unlock_activation_candidate_allowed": False,
        "operator_unlock_activation_candidate_requires_detection_patch": True,
        "operator_unlock_activation_candidate_would_unlock_fourth_trade": armed,
        "operator_unlock_activation_candidate_would_enable_candidate_detection": armed,
        "operator_unlock_activation_candidate_would_enable_paper_only_execution": False,
        ...
        "broker_submit_called_by_unlock_activation_candidate_preflight": False,
        "broker_close_called_by_unlock_activation_candidate_preflight": False,
        "orders_submitted_by_unlock_activation_candidate_preflight": 0,
        "positions_opened_by_unlock_activation_candidate_preflight": 0,
        "positions_closed_by_unlock_activation_candidate_preflight": 0,
        "paper_state_modified_by_unlock_activation_candidate_preflight": False,
        "paper_status_modified_by_unlock_activation_candidate_preflight": False,
        "submit_execution_events_total": _safe_int(activation.get("submit_execution_events_total") or 3, 3),
        "close_execution_events_total": _safe_int(activation.get("close_execution_events_total") or 3, 3),
        "aggregate_realized_pnl": aggregate_realized_pnl,
        "balance_after": balance_after,
        "realized_pnl_after": _safe_float(state.get("realized_pnl_after"), 0.0),
        "lifecycle_state": str(lifecycle.get("lifecycle_state") or activation.get("lifecycle_state") or ""),
        "dashboard_mode": str(dashboard.get("dashboard_mode") or activation.get("dashboard_mode") or ""),
        "visual_sl_tp_progress_bar_ready": _safe_bool(dashboard.get("visual_sl_tp_progress_bar_ready"), False),
        "visual_sl_tp_progress_bar": str(dashboard.get("visual_sl_tp_progress_bar") or activation.get("visual_sl_tp_progress_bar") or ""),
        ...
        "blocked_until_explicit_candidate_detection_patch": [
            "candidate_detection_execution",
        ...
        "unlock_activation_candidate_inputs": [
        ...
        "next_step": "validate_unlock_activation_candidate_preflight_then_prepare_candidate_detection_audit",
        "recommended_next_patch": "29.4.4t-13 — LSR-v2 fourth-trade candidate detection audit",
    }


def build_lsr_v2_fourth_trade_unlock_activation_candidate_preflight_report_from_files(
    *,
    ...
    report = build_lsr_v2_fourth_trade_unlock_activation_candidate_preflight_report(
        reports=reports,
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_lsr_v2_fourth_trade_unlock_activation_candidate_preflight_report_from_files(
