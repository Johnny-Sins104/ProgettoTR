"""Generic LSR-v2 supervised paper postmortem execution.

29.4.4u-40 is a controlled postmortem execution gate, read-only by default, and fail-closed. It
consumes u-39's supervised paper postmortem execution scaffold artifact and models the
controlled requirements for running a real paper postmortem after a complete
...
execution model while keeping runtime postmortem execution, paper state/status
...
PROMPT = "29.4.4u-40"
EVENT_TYPE = "LSR_V2_GENERIC_SUPERVISED_PAPER_POSTMORTEM_EXECUTION"
READY_DECISION = "LSR_V2_GENERIC_SUPERVISED_PAPER_POSTMORTEM_EXECUTION_READY"
KEEP_DIAGNOSTIC_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_GENERIC_SUPERVISED_PAPER_POSTMORTEM_EXECUTION_NOT_READY"
U39_READY_DECISION = "LSR_V2_GENERIC_SUPERVISED_PAPER_POSTMORTEM_EXECUTION_SCAFFOLD_READY"
...
POSTMORTEM_EXECUTION_STAGES: Tuple[str, ...] = (
    "load_generic_postmortem_execution_scaffold",
    "verify_postmortem_execution_scaffold_ready",
    ...
    "map_postmortem_execution_gate",
    "publish_generic_postmortem_execution_artifact",
)

BLOCKED_UNTIL_RUNTIME_POSTMORTEM_PREREQUISITES: Tuple[str, ...] = (
...
REQUIRED_SOURCE_MARKERS: Tuple[Tuple[str, Tuple[str, ...]], ...] = (
    (
        "trading_bot/core/lsr_v2_generic_supervised_paper_postmortem_execution_scaffold.py",
        (
            "GENERIC_LSR_V2_SUPERVISED_PAPER_POSTMORTEM_EXECUTION_SCAFFOLD",
            "generic_postmortem_execution_scaffold_ready",
        ),
    ),
...
class Settings:
    ...
    data_dir: str = "data"
    report_name: str = "lsr_v2_generic_supervised_paper_postmortem_execution_report.json"
    jsonl_name: str = "lsr_v2_generic_supervised_paper_postmortem_execution.jsonl"
...
def _dry_run_payload(scaffold_report: Mapping[str, Any]) -> Dict[str, Any]:
    source_map = _as_dict(scaffold_report.get("generic_supervised_paper_postmortem_execution_scaffold_map", {}))
...
def _build_postmortem_execution_map(scaffold_report: Mapping[str, Any], active_env: Mapping[str, str]) -> Dict[str, Any]:
    source_map = _as_dict(scaffold_report.get("generic_supervised_paper_postmortem_execution_scaffold_map", {}))
    source_gate = _as_dict(source_map.get("postmortem_execution_scaffold_gate_model", {}))
    ...
    scaffold_report_ready = scaffold_report.get("generic_supervised_paper_postmortem_execution_scaffold_ready") is True
    scaffold_model_ready = scaffold_report.get("generic_postmortem_execution_scaffold_model_ready") is True
    scaffold_gate_model_ready = source_gate.get("postmortem_execution_scaffold_gate_model_ready") is True
    scaffold_future_conditions_satisfied = source_gate.get("future_postmortem_execution_conditions_satisfied") is True
    scaffold_execution_allowed = source_gate.get("postmortem_execution_allowed") is True
    ...
    if not scaffold_report_ready:
        blocked_reasons.append("postmortem_execution_scaffold_report_not_ready")
    if not scaffold_model_ready:
        blocked_reasons.append("postmortem_execution_scaffold_model_not_ready")
    if not scaffold_gate_model_ready:
        blocked_reasons.append("postmortem_execution_scaffold_gate_model_not_ready")
    if not scaffold_future_conditions_satisfied:
        blocked_reasons.append("postmortem_execution_scaffold_conditions_not_satisfied")
    if not scaffold_execution_allowed:
        blocked_reasons.append("postmortem_execution_scaffold_execution_not_allowed")
    ...
    gate_model = {
        "postmortem_execution_model_ready": True,
        "postmortem_execution_gate_model_ready": True,
        "postmortem_execution_conditions_satisfied": future_conditions_satisfied,
        "postmortem_execution_allowed": future_conditions_satisfied,
        "postmortem_execution_blocked_reasons": blocked_reasons,
        "postmortem_execution_scaffold_report_required": True,
        "postmortem_execution_scaffold_report_ready": scaffold_report_ready,
        "postmortem_execution_scaffold_model_ready": scaffold_model_ready,
        "postmortem_execution_scaffold_gate_model_ready": scaffold_gate_model_ready,
        "postmortem_execution_scaffold_conditions_satisfied": scaffold_future_conditions_satisfied,
        "postmortem_execution_scaffold_execution_allowed": scaffold_execution_allowed,
    ...
    context = {
        "blocked_reason": "postmortem_execution_blocked_by_missing_final_audit_realized_pnl_close_receipt_or_gates",
        "generic_supervised_paper_postmortem_execution_ready": True,
        "generic_postmortem_execution_model_ready": True,
        "paper_postmortem_execution_model_ready": True,
        "generic_postmortem_execution_allowed": future_conditions_satisfied,
    ...
    return {
        "mode": "generic_supervised_paper_postmortem_execution_controlled",
        "preflight_source": "29.4.4u-39",
        "postmortem_execution_patch": True,
        "read_only": True,
        ...
        "postmortem_execution_gate_model": gate_model,
        "postmortem_execution_context": context,
        "source_postmortem_execution_scaffold_map": source_map,
        "required_future_controls_before_lifecycle_completion_audit": {
        ...
        "stages": [
            ...
            for stage in POSTMORTEM_EXECUTION_STAGES
        ],
    }


def run_generic_supervised_paper_postmortem_execution(settings: Settings | None = None) -> Dict[str, Any]:
    settings = settings or Settings()
    data_path = settings.data_path
    upstream_name = "lsr_v2_generic_supervised_paper_postmortem_execution_scaffold_report.json"
    ...
    upstream_ready = (
        scaffold_report.get("decision") == U39_READY_DECISION
        and scaffold_report.get("generic_supervised_paper_postmortem_execution_scaffold_ready") is True
    )
    not_ready_upstream_reports = [] if not scaffold_report or upstream_ready else [upstream_name]

    scaffold_map = _build_postmortem_execution_map(scaffold_report, active_env)
    gate_model = _as_dict(scaffold_map.get("postmortem_execution_gate_model", {}))
    context = _as_dict(scaffold_map.get("postmortem_execution_context", {}))
    ...
    diagnostic_count = _safe_int(scaffold_report.get("postmortem_execution_scaffold_diagnostic_count"), 0)
    ...
    diagnostic_available = scaffold_report.get("postmortem_execution_scaffold_diagnostic_available") is True or diagnostic_count > 0

    required_readiness = {
        "u39_postmortem_execution_scaffold_report_present": bool(scaffold_report),
        "u39_postmortem_execution_scaffold_ready": upstream_ready,
        "source_markers_present": source_markers_present,
        "postmortem_execution_model_ready": gate_model.get("postmortem_execution_model_ready") is True,
        "postmortem_execution_gate_model_ready": gate_model.get("postmortem_execution_gate_model_ready") is True,
        ...
        "postmortem_currently_blocked": gate_model.get("postmortem_execution_conditions_satisfied") is False,
    ...
    classification_labels = [
        ...
        "POSTMORTEM_EXECUTION_CONTROLLED",
        ...
        "NO_SCHEDULER",
        "FAIL_CLOSED",
        "GENERIC_SUPERVISED_PAPER_POSTMORTEM_EXECUTION_READY" if ready else "GENERIC_SUPERVISED_PAPER_POSTMORTEM_EXECUTION_NOT_READY",
    ]
    ...
    report: Dict[str, Any] = {
        ...
        "generic_supervised_paper_postmortem_execution_ready": ready,
        "generic_postmortem_execution_ready": ready,
        "generic_postmortem_execution_model_ready": gate_model.get("postmortem_execution_model_ready") is True,
        "generic_postmortem_execution_patch_ready": ready,
        "paper_postmortem_execution_ready": ready,
        "postmortem_execution_diagnostic_available": ready and diagnostic_available,
        "postmortem_execution_diagnostic_count": diagnostic_count if ready else 0,
        "postmortem_execution_contract_shape_modelable": gate_model.get("postmortem_execution_model_ready") is True,
        "postmortem_execution_contract_missing_fields": _as_list(scaffold_map.get("missing_contract_fields", [])),
        "postmortem_execution_contract_present_fields": _as_list(scaffold_map.get("present_contract_fields", [])),
        "postmortem_execution_contract_required_fields": _as_list(scaffold_map.get("required_contract_fields", [])),
        "generic_supervised_paper_postmortem_preflight_ready": scaffold_report.get("generic_supervised_paper_postmortem_preflight_ready") is True,
        "generic_postmortem_preflight_ready": scaffold_report.get("generic_postmortem_preflight_ready") is True,
        "generic_postmortem_preflight_model_ready": scaffold_report.get("generic_postmortem_preflight_model_ready") is True,
        "generic_postmortem_preflight_patch_ready": scaffold_report.get("generic_postmortem_preflight_patch_ready") is True,
        "postmortem_execution_scaffold_diagnostic_available": diagnostic_available,
        "postmortem_execution_scaffold_diagnostic_count": diagnostic_count,
        "final_audit_execution_diagnostic_available": scaffold_report.get("final_audit_execution_diagnostic_available") is True,
        "final_audit_execution_diagnostic_count": _safe_int(scaffold_report.get("final_audit_execution_diagnostic_count"), diagnostic_count),
        ...
        "generic_supervised_paper_postmortem_execution_map": scaffold_map,
        "generic_supervised_paper_postmortem_preflight_map": _as_dict(scaffold_report.get("generic_supervised_paper_postmortem_preflight_map", {})),
        "generic_supervised_paper_final_audit_execution_map": _as_dict(scaffold_report.get("generic_supervised_paper_final_audit_execution_map", {})),
        ...
        "blocked_until_explicit_postmortem_execution_patch": list(BLOCKED_UNTIL_RUNTIME_POSTMORTEM_PREREQUISITES),
        ...
        "paper_state_modified_by_generic_postmortem_execution": False,
        "paper_status_modified_by_generic_postmortem_execution": False,
        "orders_submitted_by_generic_postmortem_execution": 0,
        "positions_opened_by_generic_postmortem_execution": 0,
        "positions_closed_by_generic_postmortem_execution": 0,
        "broker_submit_called_by_generic_postmortem_execution": False,
        "broker_close_called_by_generic_postmortem_execution": False,
        "telegram_network_called_by_generic_postmortem_execution": False,
        "scheduler_started_by_generic_postmortem_execution": False,
    }
...
def main() -> None:
    report = run_generic_supervised_paper_postmortem_execution()
