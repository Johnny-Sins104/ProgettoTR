"""Prompt 29.4.4s-10ac — LSR-v2 second supervised paper close execution.

Closes exactly one already-open second LSR-v2 paper position when the second trade close preflight
has detected a TP/SL/close-required diagnostic and the operator supplies the
explicit second close confirmation.  This module is paper-state only: it never opens a
...
PROMPT_ID = "29.4.4s-10ac"
EVENT_TYPE = "LSR_V2_SECOND_TRADE_CLOSE_EXECUTION"
REPORT_NAME = "lsr_v2_second_trade_close_execution_report.json"
JSONL_NAME = "lsr_v2_second_trade_close_execution.jsonl"
BACKUP_DIR_NAME = "lsr_v2_second_trade_close_execution_backups"

CLOSE_PREFLIGHT_REPORT_NAME = "lsr_v2_second_trade_close_preflight_report.json"
CLOSE_PREFLIGHT_JSONL_NAME = "lsr_v2_second_trade_close_preflight.jsonl"
OPEN_MONITOR_REPORT_NAME = "lsr_v2_second_open_position_monitor_report.json"
...
CLOSE_ENABLE_ENV = "LSR_V2_SECOND_TRADE_CLOSE_ENABLE"
CLOSE_CONFIRM_ENV = "LSR_V2_SECOND_TRADE_CLOSE_CONFIRMATION"
CLOSE_MAX_POSITIONS_ENV = "LSR_V2_SECOND_TRADE_CLOSE_MAX_POSITIONS"
CLOSE_CONFIRMATION_PHRASE = "I_UNDERSTAND_CLOSE_SECOND_PAPER_POSITION_ONLY"

CLOSED_DECISION = "LSR_V2_SECOND_SINGLE_PAPER_POSITION_CLOSED"
NOT_ENABLED_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_SECOND_CLOSE_NOT_ENABLED"
CONFIRMATION_MISSING_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_SECOND_CLOSE_CONFIRMATION_MISSING"
PREFLIGHT_MISSING_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_SECOND_CLOSE_PREFLIGHT_MISSING"
...
MAX_POSITION_CAP_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_SECOND_CLOSE_MAX_POSITION_CAP_BLOCKED"
REJECT_DECISION = "REJECT_LSR_V2_SECOND_CLOSE_EXECUTION_SAFETY_FAILED"
...
class LSRV2SecondTradeCloseExecutionSettings:
    data_dir: str = "data"
    ...
    @classmethod
    def from_env(cls, data_dir: str = "data") -> "LSRV2SecondTradeCloseExecutionSettings":
...
def select_second_close_cycle_id(data_dir: str | Path, requested_cycle_id: str = "", settings: LSRV2SecondTradeCloseExecutionSettings | None = None) -> str:
    settings = settings or LSRV2SecondTradeCloseExecutionSettings(data_dir=str(data_dir))
    base = Path(data_dir)
...
def select_cycle_scoped_second_close_preflight_events(data_dir: str | Path, cycle_id: str, settings: LSRV2SecondTradeCloseExecutionSettings | None = None) -> list[dict[str, Any]]:
    settings = settings or LSRV2SecondTradeCloseExecutionSettings(data_dir=str(data_dir))
    rows = _iter_jsonl_tail(Path(data_dir) / settings.close_preflight_jsonl_name, max_lines=settings.max_event_lines)
    selected = [dict(r) for r in rows if r.get("event_type") == "LSR_V2_SECOND_TRADE_CLOSE_PREFLIGHT" and (not cycle_id or str(r.get("cycle_id") or "") == cycle_id)]
    if selected:
        return selected
...
def _close_position_payload(position: Mapping[str, Any], event: Mapping[str, Any], *, close_price: float, close_reason: str) -> dict[str, Any]:
    ...
    pos.update({
        ...
        "paper_close_source": "lsr_v2_second_trade_close_execution",
...
def _operator_controls(settings: LSRV2SecondTradeCloseExecutionSettings) -> dict[str, Any]:
    return {
...
def build_lsr_v2_second_trade_close_execution_report_from_files(
    *,
    data_dir: str | Path = "data",
    cycle_id: str = "",
    settings: LSRV2SecondTradeCloseExecutionSettings | None = None,
) -> dict[str, Any]:
    settings = settings or LSRV2SecondTradeCloseExecutionSettings.from_env(data_dir=str(data_dir))
    base = Path(data_dir)
    selected_cycle_id = select_second_close_cycle_id(data_dir, requested_cycle_id=cycle_id, settings=settings)
    preflight_events = select_cycle_scoped_second_close_preflight_events(data_dir, selected_cycle_id, settings)
    ...
    if can_close:
        ...
        for event in close_required[: settings.close_max_positions]:
            ...
            closed_events.append({
                ...
                "source": "lsr_v2_second_trade_close_preflight",
    ...
    if safety_violation:
        ...
        decision = "KEEP_DIAGNOSTIC_LSR_V2_SECOND_CLOSE_NOT_REQUIRED"
        status_text = "WARN"
    ...
    report = {
        ...
        "status": status_text,
        "decision": decision,
        "classification_labels": ["LSR_V2_SECOND_TRADE_CLOSE_EXECUTION"],
        "blockers": sorted(set(blockers)),
        "cycle_id": selected_cycle_id,
        "event_source": "second_trade_close_preflight_jsonl",
        ...
        "positions_closed_by_second_trade_close_execution": int(positions_closed),
        "orders_submitted_by_second_trade_close_execution": 0,
        "positions_opened_by_second_trade_close_execution": 0,
        ...
        "open_second_lsr_v2_positions_before": len(open_lsr_positions),
        "open_second_lsr_v2_positions_after": len(after_open_lsr),
