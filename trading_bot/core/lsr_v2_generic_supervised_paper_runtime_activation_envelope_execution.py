"""Generic LSR-v2 supervised paper runtime activation envelope execution.

29.4.4u-64 consumes the u-63 runtime activation envelope execution scaffold
artifact and models the controlled runtime activation envelope execution gate.
This patch removes only the explicit "future runtime activation envelope execution
patch required" blocker. It does not build or run a runtime activation envelope,
...
The execution remains read-only by default and fail-closed. Complete runtime
...
and all operator gates are still required before any future real paper runtime
activation.
"""
...
PROMPT = "29.4.4u-64"
EVENT_TYPE = "LSR_V2_GENERIC_SUPERVISED_PAPER_RUNTIME_ACTIVATION_ENVELOPE_EXECUTION"
READY_DECISION = "LSR_V2_GENERIC_SUPERVISED_PAPER_RUNTIME_ACTIVATION_ENVELOPE_EXECUTION_READY"
KEEP_DIAGNOSTIC_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_GENERIC_SUPERVISED_PAPER_RUNTIME_ACTIVATION_ENVELOPE_EXECUTION_NOT_READY"
U63_READY_DECISION = "LSR_V2_GENERIC_SUPERVISED_PAPER_RUNTIME_ACTIVATION_ENVELOPE_EXECUTION_SCAFFOLD_READY"
...
RUNTIME_ACTIVATION_ENVELOPE_EXECUTION_STAGES: Tuple[str, ...] = (
    "load_generic_runtime_activation_envelope_execution_scaffold_report",
    "verify_runtime_activation_envelope_execution_scaffold_model_ready",
    "verify_runtime_activation_envelope_execution_scaffold_was_blocked",
    ...
    "verify_runtime_activation_envelope_execution_patch_current_step_but_runtime_still_blocked",
    ...
    "map_runtime_activation_envelope_execution_gate",
    "publish_generic_runtime_activation_envelope_execution_artifact",
)

BLOCKED_UNTIL_RUNTIME_ACTIVATION_ENVELOPE_EXECUTION_PREREQUISITES: Tuple[str, ...] = (
...
REQUIRED_SOURCE_MARKERS: Tuple[Tuple[str, Tuple[str, ...]], ...] = (
    (
        ...
        (
            ...
            "generic_runtime_activation_envelope_execution_scaffold_ready",
        ),
    ),
...
class Settings:
    ...
    data_dir: str = "data"
    report_name: str = "lsr_v2_generic_supervised_paper_runtime_activation_envelope_execution_report.json"
    jsonl_name: str = "lsr_v2_generic_supervised_paper_runtime_activation_envelope_execution.jsonl"
...
def _active_lsr_env() -> Dict[str, str]:
    return {key: value for key, value in os.environ.items() if key.startswith("LSR_V2_GENERIC_")}


def _operator_gate_satisfied(env: Mapping[str, str], enable_env: str, confirmation_env: str, sentinel: str) -> bool:
    return env.get(enable_env) == "1" and env.get(confirmation_env) == sentinel


def _operator_gate_snapshot(env: Mapping[str, str]) -> Dict[str, Any]:
    gates = {
    ...
    gates["operator_runtime_activation_envelope_gate_required"] = True
    gates["operator_runtime_activation_envelope_enable_env"] = RUNTIME_ACTIVATION_ENVELOPE_ENABLE_ENV
    gates["operator_runtime_activation_envelope_confirmation_env"] = RUNTIME_ACTIVATION_ENVELOPE_CONFIRMATION_ENV
    gates["operator_runtime_activation_envelope_confirmation_required_value"] = RUNTIME_ACTIVATION_ENVELOPE_CONFIRMATION_SENTINEL
    return gates
...
def _source_marker_audit(project_root: Path) -> Tuple[bool, List[str], Dict[str, List[str]]]:
    ...
    for relative, markers in REQUIRED_SOURCE_MARKERS:
        ...
        if absent:
            missing_markers[relative] = absent
    return not missing_files and not missing_markers, missing_files, missing_markers


def _dry_run_order_intent_payload() -> Dict[str, Any]:
    payload: Dict[str, Any] = {field: DEFERRED_SENTINEL for field in REQUIRED_ORDER_INTENT_FIELDS}
    payload.update({
        "paper_only": True,
...
def _candidate_examples(source: Mapping[str, Any]) -> List[Any]:
    return _as_list(source.get("candidate_diagnostic_examples"))[:5]


def _build_runtime_activation_envelope_execution_map(source: Mapping[str, Any], env: Mapping[str, str]) -> Dict[str, Any]:
    source_map = _as_dict(source.get("generic_supervised_paper_runtime_activation_envelope_execution_scaffold_map"))
    runtime_values_present = False
    present_fields = list(REQUIRED_ORDER_INTENT_FIELDS)
    missing_fields: List[str] = []

    blocked_reasons: List[str] = [
        "runtime_activation_envelope_execution_scaffold_not_allowed",
    ...
    gate_model: Dict[str, Any] = {
        "runtime_activation_envelope_execution_gate_model_ready": True,
        "runtime_activation_envelope_execution_model_ready": True,
        "runtime_activation_envelope_execution_patch_ready": True,
        "runtime_activation_envelope_execution_allowed": False,
        "runtime_activation_envelope_execution_conditions_satisfied": False,
        "runtime_activation_envelope_execution_scaffold_report_required": True,
        "runtime_activation_envelope_execution_scaffold_report_ready": source.get("decision") == U63_READY_DECISION,
        "runtime_activation_envelope_execution_scaffold_ready": bool(source.get("generic_runtime_activation_envelope_execution_scaffold_ready")),
        "runtime_activation_envelope_execution_scaffold_model_ready": bool(source.get("generic_runtime_activation_envelope_execution_scaffold_model_ready")),
        "runtime_activation_envelope_execution_scaffold_patch_ready": bool(source.get("generic_runtime_activation_envelope_execution_scaffold_patch_ready")),
        ...
        "paper_state_mutation_allowed_after_runtime_activation_envelope_execution": False,
        "paper_status_mutation_allowed_after_runtime_activation_envelope_execution": False,
        "telegram_network_send_allowed_after_runtime_activation_envelope_execution": False,
        "scheduler_start_allowed_after_runtime_activation_envelope_execution": False,
        "integrated_operation_execution_allowed": False,
        "future_integrated_operation_allowed": False,
        "runtime_activation_envelope_execution_blocked_reasons": blocked_reasons,
    }
    gate_model.update(_operator_gate_snapshot(env))
    ...
    return {
        ...
        "paper_only": True,
        "preflight_source": "29.4.4u-63",
        "dry_run_order_intent_payload": _dry_run_order_intent_payload(),
        ...
        "runtime_activation_envelope_execution_context": {
            "blocked_reason": "runtime_activation_envelope_execution_blocked_until_complete_runtime_lifecycle_evidence_operator_gates_and_runtime_still_blocked",
        ...
        "runtime_activation_envelope_execution_gate_model": gate_model,
        "source_runtime_activation_envelope_execution_scaffold_map": source_map,
        ...
        "stages": [
            ...
            for stage in RUNTIME_ACTIVATION_ENVELOPE_EXECUTION_STAGES
        ],
    }


def run_generic_supervised_paper_runtime_activation_envelope_execution(settings: Settings | None = None) -> Dict[str, Any]:
    settings = settings or Settings()
    project_root = settings.project_root
    data_path = settings.data_path
    report_path = data_path / settings.report_name
    jsonl_path = data_path / settings.jsonl_name
    upstream_path = data_path / "lsr_v2_generic_supervised_paper_runtime_activation_envelope_execution_scaffold_report.json"
    ...
    if upstream is not None:
        not_ready_upstream = upstream.get("decision") != U63_READY_DECISION
    ...
    envelope_map = _build_runtime_activation_envelope_execution_map(upstream, active_env)
    candidate_counts = _as_dict(upstream.get("candidate_detection_source_counts"))
    diagnostic_count = _safe_int(
        upstream.get(
            "runtime_activation_envelope_execution_scaffold_diagnostic_count",
    ...
    report: Dict[str, Any] = {
        ...
        "classification_labels": [
            ...
            "RUNTIME_ACTIVATION_ENVELOPE_EXECUTION_CONTROLLED",
            ...
            "NO_BROKER_CALL",
            "FAIL_CLOSED",
            "GENERIC_SUPERVISED_PAPER_RUNTIME_ACTIVATION_ENVELOPE_EXECUTION_READY",
        ],
        "blockers": blockers,
        ...
        "generic_supervised_paper_runtime_activation_envelope_execution_ready": status == "PASS",
        "generic_runtime_activation_envelope_execution_ready": status == "PASS",
        ...
        "paper_runtime_activation_envelope_execution_ready": status == "PASS",
        "paper_runtime_activation_envelope_ready": False,
        "runtime_activation_envelope_execution_diagnostic_available": True,
        "runtime_activation_envelope_execution_diagnostic_count": diagnostic_count,
        "blocked_until_runtime_activation_envelope_execution_prerequisites": list(BLOCKED_UNTIL_RUNTIME_ACTIVATION_ENVELOPE_EXECUTION_PREREQUISITES),
        "generic_supervised_paper_runtime_activation_envelope_execution_map": envelope_map,
        ...
        "generic_integrated_operation_execution_allowed": False,
        "future_integrated_operation_allowed": False,
        "broker_submit_called_by_generic_runtime_activation_envelope_execution": False,
        "broker_close_called_by_generic_runtime_activation_envelope_execution": False,
        "orders_submitted_by_generic_runtime_activation_envelope_execution": 0,
        "orders_closed_by_generic_runtime_activation_envelope_execution": 0,
        "positions_opened_by_generic_runtime_activation_envelope_execution": 0,
        "positions_closed_by_generic_runtime_activation_envelope_execution": 0,
        "paper_state_modified_by_generic_runtime_activation_envelope_execution": False,
        "paper_status_modified_by_generic_runtime_activation_envelope_execution": False,
        "scheduler_started_by_generic_runtime_activation_envelope_execution": False,
        "telegram_network_send_called_by_generic_runtime_activation_envelope_execution": False,
        "would_build_runtime_activation_envelope": False,
        "would_run_runtime_activation_envelope": False,
