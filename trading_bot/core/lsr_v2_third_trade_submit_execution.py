"""Prompt 29.4.4s-10ao — LSR-v2 third supervised paper-only submit execution.

This module adds the execution boundary for the *third* LSR-v2 supervised
paper trade.  It is separate from the first-trade execution path and consumes
only the third-trade submit-boundary artifacts introduced in 29.4.4s-10an.

The boundary is disabled by default and requires:

* the third-trade submit boundary arm/confirmation from 29.4.4s-10an;
* a new execution arm/confirmation specific to the third paper trade;
...
try:  # package import in the full project
    from .lsr_v2_third_trade_submit_boundary import (  # type: ignore
...
PROMPT_ID = "29.4.4s-10ao"
EVENT_TYPE = "LSR_V2_THIRD_TRADE_SUBMIT_EXECUTION"
REPORT_NAME = "lsr_v2_third_trade_submit_execution_report.json"
JSONL_NAME = "lsr_v2_third_trade_submit_execution.jsonl"

EXECUTE_ENV = "LSR_V2_THIRD_TRADE_EXECUTE"
EXECUTE_CONFIRM_ENV = "LSR_V2_THIRD_TRADE_EXECUTE_CONFIRMATION"
REQUIRED_EXECUTE_VALUE = "1"
REQUIRED_EXECUTE_CONFIRMATION = "I_UNDERSTAND_EXECUTE_THIRD_PAPER_ORDER_ONLY"

EXECUTED_DECISION = "LSR_V2_THIRD_SINGLE_PAPER_ORDER_EXECUTED"
NOT_ARMED_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_THIRD_EXECUTION_NOT_ARMED"
CONFIRMATION_MISSING_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_THIRD_EXECUTION_CONFIRMATION_MISSING"
BOUNDARY_MISSING_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_THIRD_SUBMIT_BOUNDARY_MISSING"
STATE_NOT_CLEAN_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_THIRD_PAPER_STATE_NOT_CLEAN"
SUBMITTER_NOT_AVAILABLE_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_THIRD_PAPER_SUBMITTER_NOT_AVAILABLE"
MAX_ORDER_CAP_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_THIRD_EXECUTION_MAX_ORDER_CAP_BLOCKED"
REJECT_DECISION = "REJECT_LSR_V2_THIRD_EXECUTION_SAFETY_FAILED"
...
class LSRV2ThirdTradeSubmitExecutionSettings:
    data_dir: str = "data"
    ...
    @classmethod
    def from_env(cls, data_dir: str = "data") -> "LSRV2ThirdTradeSubmitExecutionSettings":
...
def _clone_boundary_settings(
    *,
    data_dir: str | Path,
    execution_settings: LSRV2ThirdTradeSubmitExecutionSettings,
) -> LSRV2ThirdTradeSubmitBoundarySettings:
    base = LSRV2ThirdTradeSubmitBoundarySettings.from_env(data_dir=str(data_dir))
    return LSRV2ThirdTradeSubmitBoundarySettings(
        **{
            **base.to_dict(),
...
def read_third_trade_paper_state_cleanliness(data_dir: str | Path) -> tuple[bool, dict[str, Any]]:
    """Return whether paper state/status is clean enough for the third submit."""
    base = Path(data_dir)
...
def select_third_trade_submit_boundary_events(
    *,
    data_dir: str | Path,
    settings: LSRV2ThirdTradeSubmitExecutionSettings | None = None,
    requested_cycle_id: str = "",
) -> tuple[str, list[dict[str, Any]], dict[str, Any]]:
    settings = settings or LSRV2ThirdTradeSubmitExecutionSettings(data_dir=str(data_dir))
    base = Path(data_dir)
    ...
    ready = [row for row in scoped if _safe_bool(row.get("third_trade_submit_ready"), False)]
    metadata = {
        "event_source": "third_trade_submit_boundary_jsonl" if rows else "third_trade_submit_boundary_report",
        "strict_cycle_scope": True,
        "historical_third_trade_submit_boundary_events": max(0, len(rows) - len(scoped)),
...
def build_third_trade_project_paper_submitter(
    *,
    data_dir: str | Path,
    settings: LSRV2ThirdTradeSubmitExecutionSettings | None = None,
...
def build_lsr_v2_third_trade_submit_execution_event(
    *,
    boundary_event: Mapping[str, Any],
    settings: LSRV2ThirdTradeSubmitExecutionSettings | None = None,
    boundary_settings: LSRV2ThirdTradeSubmitBoundarySettings | None = None,
    paper_state: Mapping[str, Any] | None = None,
    paper_submitter: PaperSubmitter | None = None,
    submitter_error: str = "",
    index: int = 0,
) -> dict[str, Any]:
    settings = settings or LSRV2ThirdTradeSubmitExecutionSettings()
    boundary_settings = boundary_settings or LSRV2ThirdTradeSubmitBoundarySettings()
    ...
    boundary_ready = _safe_bool(boundary_event.get("third_trade_submit_ready"), False)
    ...
    if not boundary_ready:
        blockers.append("third_trade_submit_boundary_not_ready")
    ...
    if not settings.execute_armed:
        blockers.append("third_trade_execute_not_armed")
    if not settings.execute_confirmation_ok:
        blockers.append("third_trade_execute_confirmation_missing")
    ...
    if not settings.execute_armed:
        blocked_reason = "third_trade_execute_not_armed"
    elif not settings.execute_confirmation_ok:
        blocked_reason = "third_trade_execute_confirmation_missing"
        ...
        blocked_reason = "third_trade_submit_not_executed"
    ...
    return {
        ...
        "source": "lsr_v2_third_trade_submit_boundary",
        ...
        "third_trade_submit_armed": bool(boundary_settings.submit_armed),
        "third_trade_submit_confirmation_ok": bool(boundary_settings.submit_confirmation_ok),
        "third_trade_submit_ready": bool(boundary_ready),
        "third_trade_execute_armed": bool(settings.execute_armed),
        "third_trade_execute_confirmation_ok": bool(settings.execute_confirmation_ok),
        ...
        "third_trade_submit_enabled": bool(order_submitted),
        "third_trade_execute_enabled": bool(order_submitted),
        ...
        "orders_submitted_by_third_trade_execution": 1 if order_submitted else 0,
        "positions_opened_by_third_trade_execution": 1 if position_opened else 0,
        "positions_closed_by_third_trade_execution": 0,
...
def build_lsr_v2_third_trade_submit_execution_events(
    ...
    *,
    ...
    settings = settings or LSRV2ThirdTradeSubmitExecutionSettings()
    boundary_settings = boundary_settings or LSRV2ThirdTradeSubmitBoundarySettings()
    eligible = [dict(e) for e in _dedupe_boundary_events(boundary_events) if _safe_bool(e.get("third_trade_submit_ready"), False)]
    events: list[dict[str, Any]] = []
    for idx, boundary in enumerate(eligible):
        events.append(build_lsr_v2_third_trade_submit_execution_event(
...
def summarize_lsr_v2_third_trade_submit_execution(
    *,
    data_dir: str | Path,
    cycle_id: str,
    boundary_events: Iterable[Mapping[str, Any]],
    execution_events: Iterable[Mapping[str, Any]],
    settings: LSRV2ThirdTradeSubmitExecutionSettings,
    boundary_settings: LSRV2ThirdTradeSubmitBoundarySettings,
    ...
    ready_rows = [e for e in rows if _safe_bool(e.get("third_trade_submit_ready"), False)]
    would_submit_rows = [e for e in rows if _safe_bool(e.get("would_submit"), False)]
    submitted_rows = [e for e in rows if _safe_int(e.get("orders_submitted_by_third_trade_execution"), 0) > 0]
    position_rows = [e for e in rows if _safe_int(e.get("positions_opened_by_third_trade_execution"), 0) > 0]
    ...
    if not boundary_rows or not _safe_bool(metadata.get("boundary_report_ready"), False):
        blockers.append("third_trade_submit_boundary_missing_or_not_ready")
    if not boundary_settings.submit_armed or not settings.execute_armed:
        blockers.append("third_trade_execution_not_armed")
    if not boundary_settings.submit_confirmation_ok or not settings.execute_confirmation_ok:
        blockers.append("third_trade_execution_confirmation_missing")
    if settings.max_orders != 1:
        blockers.append("max_orders_not_one")
    if not _safe_bool(paper_state.get("paper_state_clean"), False):
        blockers.append("paper_state_not_clean")
    ...
    safety_violation = any(
        ...
        or _safe_int(e.get("orders_submitted_by_third_trade_execution"), 0) > 1
        or _safe_int(e.get("positions_opened_by_third_trade_execution"), 0) > 1
        or _safe_int(e.get("positions_closed_by_third_trade_execution"), 0) != 0
        for e in rows
    )
    if len(submitted_rows) > 1 or len(position_rows) > 1:
        safety_violation = True
        blockers.append("more_than_one_third_trade_order_or_position")
    if safety_violation:
    ...
    return {
        ...
        "classification_labels": [
            "LSR_V2_THIRD_SUPERVISED_PAPER_ONLY_SUBMIT",
            "THIRD_TRADE_SINGLE_ORDER_EXECUTION_BOUNDARY",
        ] + (["THIRD_PAPER_ORDER_EXECUTED"] if decision == EXECUTED_DECISION else ["KEEP_DIAGNOSTIC"]),
        "blockers": sorted(set(blockers)),
        "cycle_id": str(cycle_id or ""),
        "event_source": str(metadata.get("event_source") or "third_trade_submit_boundary_jsonl"),
        "strict_cycle_scope": bool(metadata.get("strict_cycle_scope", True)),
        "historical_third_trade_submit_boundary_events": _safe_int(metadata.get("historical_third_trade_submit_boundary_events"), 0),
        ...
        "broker_submit_called": bool(broker_called),
        "broker_submit_called_by_third_trade_execution": bool(broker_called),
        "broker_submit_called_count": sum(1 for e in rows if _safe_bool(e.get("broker_submit_called"), False)),
        "orders_submitted_by_third_trade_execution": len(submitted_rows),
        "positions_opened_by_third_trade_execution": len(position_rows),
        "positions_closed_by_third_trade_execution": 0,
        "paper_order_submission_enabled": bool(would_submit_rows),
        "routing_enabled": False,
        "execution_enabled": bool(would_submit_rows),
        "third_trade_execute_enabled": bool(would_submit_rows),
        "third_trade_submit_enabled": bool(would_submit_rows),
        "live_enabled": False,
        "testnet_enabled": False,
        "exchange_broker_enabled": False,
        "operational_unlock_allowed": False,
...
def build_lsr_v2_third_trade_submit_execution_report_from_files(
    *,
    ...
    settings = settings or LSRV2ThirdTradeSubmitExecutionSettings.from_env(data_dir=str(data_dir))
    boundary_settings = _clone_boundary_settings(data_dir=data_dir, execution_settings=settings)
    selected_cycle_id, boundary_events, metadata = select_third_trade_submit_boundary_events(
        data_dir=data_dir,
        settings=settings,
        requested_cycle_id=cycle_id,
    )
    paper_state_clean, paper_state_meta = read_third_trade_paper_state_cleanliness(data_dir)
    ...
    if execute_controls_ok and submit_controls_ok and basic_safety_ok:
        ...
        if submitter_to_use is None and allow_project_submitter:
            submitter_to_use, submitter_error = build_third_trade_project_paper_submitter(data_dir=data_dir, settings=settings)

    execution_events = build_lsr_v2_third_trade_submit_execution_events(
        boundary_events,
        settings=settings,
        boundary_settings=boundary_settings,
    ...
    report = summarize_lsr_v2_third_trade_submit_execution(
        data_dir=data_dir,
