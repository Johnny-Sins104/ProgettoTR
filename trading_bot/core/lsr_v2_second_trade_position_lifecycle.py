"""Prompt 29.4.4s-10z — LSR-v2 second paper position lifecycle audit.

Second-trade-aware lifecycle audit for the supervised LSR-v2 paper order opened
by 29.4.4s-10y.  This module intentionally does not reuse the first-trade
lifecycle artifact names, because those are scoped to the first paper trade and
can point to pc_000228... after the second trade is already open.

The module reads only the second-trade submit execution artifacts plus paper
state/status/events and verifies that exactly one second LSR-v2 paper position
is visible, that paper_status is aligned, and that no third submit/re-entry is
present.  It never submits, closes, opens, re-enters, or mutates state.
"""
...
PROMPT_ID = "29.4.4s-10z"
EVENT_TYPE = "LSR_V2_SECOND_TRADE_POSITION_LIFECYCLE_AUDIT"
REPORT_NAME = "lsr_v2_second_trade_position_lifecycle_report.json"
JSONL_NAME = "lsr_v2_second_trade_position_lifecycle.jsonl"

EXECUTION_EVENT_TYPE = "LSR_V2_SECOND_TRADE_SUBMIT_EXECUTION"
EXECUTION_REPORT_NAME = "lsr_v2_second_trade_submit_execution_report.json"
EXECUTION_JSONL_NAME = "lsr_v2_second_trade_submit_execution.jsonl"
NEW_EXECUTION_EVENT_TYPE = "LSR_V2_SECOND_PAPER_ORDER_SUBMIT_EXECUTION"
NEW_EXECUTION_REPORT_NAME = "lsr_v2_second_paper_order_submit_execution_report.json"
NEW_EXECUTION_JSONL_NAME = "lsr_v2_second_paper_order_submit_execution.jsonl"
PAPER_STATE_NAME = "paper_state.json"
PAPER_STATUS_NAME = "paper_status.json"
PAPER_EVENTS_NAME = "paper_events.jsonl"

READY_DECISION = "LSR_V2_SECOND_TRADE_POSITION_LIFECYCLE_READY"
...
THIRD_SUBMIT_DETECTED_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_THIRD_SUBMIT_OR_REENTRY_DETECTED"
REJECT_DECISION = "REJECT_LSR_V2_SECOND_POSITION_LIFECYCLE_SAFETY_FAILED"

SOURCE_TAG = "lsr_v2_second_trade_submit_execution"
NEW_SOURCE_TAG = "lsr_v2_second_paper_order_submit_execution"
SOURCE_TAGS = (SOURCE_TAG, NEW_SOURCE_TAG)
...
def _submitted_count(row: Mapping[str, Any]) -> int:
    return _safe_int(
        row.get("orders_submitted_by_second_trade_execution")
        or row.get("orders_submitted_by_second_paper_execution"),
        0,
    )


def _opened_count(row: Mapping[str, Any]) -> int:
    return _safe_int(
        row.get("positions_opened_by_second_trade_execution")
        or row.get("positions_opened_by_second_paper_execution"),
        0,
    )
...
def _dedupe_execution_events(rows: Iterable[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    for item in rows:
        row = dict(item)
        if row.get("event_type") not in {EXECUTION_EVENT_TYPE, NEW_EXECUTION_EVENT_TYPE}:
            continue
...
def _row_trade_ordinal(row: Mapping[str, Any]) -> int:
    meta = _metadata_of(row)
    return _safe_int(row.get("trade_ordinal") or meta.get("trade_ordinal"), 0)
...
class LSRV2SecondTradePositionLifecycleSettings:
    data_dir: str = "data"
...
def select_cycle_scoped_second_trade_execution_events(
    *,
    data_dir: str | Path,
    settings: LSRV2SecondTradePositionLifecycleSettings | None = None,
    requested_cycle_id: str = "",
) -> tuple[str, list[dict[str, Any]], dict[str, Any]]:
    settings = settings or LSRV2SecondTradePositionLifecycleSettings(data_dir=str(data_dir))
    base = Path(data_dir)
    old_rows = _iter_jsonl_tail(base / settings.execution_jsonl_name, max_lines=settings.max_event_lines)
    new_rows = _iter_jsonl_tail(base / NEW_EXECUTION_JSONL_NAME, max_lines=settings.max_event_lines)
    rows = _dedupe_execution_events([*old_rows, *new_rows])
    old_report = _read_json(base / settings.execution_report_name)
    new_report = _read_json(base / NEW_EXECUTION_REPORT_NAME)
    report = new_report if _submitted_count(new_report) > 0 else old_report
    ...
    submitted = [
        ...
        if _submitted_count(row) > 0
    ]
    # Report fallback if JSONL was rotated/missing.
    if not submitted and str(report.get("cycle_id") or "") == cycle_id and _submitted_count(report) > 0:
        payload = _payload_of(report)
        fallback = {
            "event_type": NEW_EXECUTION_EVENT_TYPE if report is new_report else EXECUTION_EVENT_TYPE,
            "cycle_id": cycle_id,
            "symbol": report.get("symbol") or payload.get("symbol", ""),
            "side": report.get("side") or payload.get("side", ""),
            ...
            "position_opened": _opened_count(report) > 0,
            "orders_submitted_by_second_trade_execution": _submitted_count(report),
            "positions_opened_by_second_trade_execution": _opened_count(report),
            "risk_amount": report.get("total_risk_amount") or payload.get("risk_amount", 0),
            "notional": report.get("total_notional") or payload.get("notional", 0),
            "trade_ordinal": 2,
        }
        scoped = [fallback]
        submitted = [fallback]
    ...
    metadata = {
        "prompt_id": PROMPT_ID,
        "event_source": "second_trade_execution_jsonl" if rows else "second_trade_execution_report_json",
        "strict_cycle_scope": True,
        "execution_report_decision": report.get("decision"),
        "execution_report_cycle_id": report.get("cycle_id"),
        "historical_second_trade_execution_events": len(historical),
        "historical_second_trade_submitted_events": sum(1 for row in historical if _submitted_count(row) > 0),
        "paper_events_scanned": len(paper_rows),
        "total_scoped_execution_events": len(scoped),
        "total_scoped_submitted_events": len(submitted),
    }
    return cycle_id, submitted, metadata


def _is_second_trade_row(row: Mapping[str, Any], cycle_id: str = "") -> bool:
    source = _row_source(row)
    if any(tag in source for tag in SOURCE_TAGS):
        return True
    if _row_trade_ordinal(row) == 2:
        return True
...
def find_matching_order(execution_event: Mapping[str, Any], orders: Iterable[Mapping[str, Any]]) -> dict[str, Any] | None:
    ...
    for order in rows:
        ...
        if cycle_id and _row_cycle(order) == cycle_id:
            score += 5
        if any(tag in _row_source(order) for tag in SOURCE_TAGS):
            score += 4
        if _row_trade_ordinal(order) == 2:
            score += 3
...
def find_matching_position(execution_event: Mapping[str, Any], positions: Iterable[Mapping[str, Any]], order: Mapping[str, Any] | None = None) -> dict[str, Any] | None:
    ...
    for pos in rows:
        ...
        if cycle_id and _row_cycle(pos) == cycle_id:
            score += 5
        if any(tag in _row_source(pos) for tag in SOURCE_TAGS):
            score += 4
        if _row_trade_ordinal(pos) == 2:
            score += 3
...
def build_lsr_v2_second_trade_position_lifecycle_event(
    *,
    ...
    settings = settings or LSRV2SecondTradePositionLifecycleSettings()
    ...
    cycle_id = _row_cycle(position) or _row_cycle(order) or _event_cycle(execution_event)
    return {
        "event_type": EVENT_TYPE,
        "prompt_id": PROMPT_ID,
        "created_at": utc_now_iso(),
        "cycle_id": cycle_id,
        "source_event_type": EXECUTION_EVENT_TYPE,
        "trade_ordinal": 2,
        ...
        "orders_submitted_by_second_trade_lifecycle_audit": 0,
        "positions_opened_by_second_trade_lifecycle_audit": 0,
        "positions_closed_by_second_trade_lifecycle_audit": 0,
        "broker_submit_called_by_second_trade_lifecycle_audit": False,
        "broker_close_called_by_second_trade_lifecycle_audit": False,
        "live_enabled": False,
...
def build_lsr_v2_second_trade_position_lifecycle_report_from_files(
    *,
    ...
    settings = settings or LSRV2SecondTradePositionLifecycleSettings(data_dir=str(data_dir))
    base = Path(data_dir)
    selected_cycle_id, execution_events, metadata = select_cycle_scoped_second_trade_execution_events(
        data_dir=data_dir,
        settings=settings,
    ...
    for event in execution_events:
        ...
        lifecycle_events.append(build_lsr_v2_second_trade_position_lifecycle_event(
    ...
    report = summarize_lsr_v2_second_trade_position_lifecycle(
        data_dir=data_dir,
...
def summarize_lsr_v2_second_trade_position_lifecycle(
    *,
    data_dir: str | Path,
    cycle_id: str,
    ...
    active_second_positions = [
        p for p in position_rows
        if _is_position_open(p) and _is_second_trade_row(p, cycle_id)
    ]
    active_second_orders = [
        o for o in order_rows
        if _is_order_active(o) and _is_second_trade_row(o, cycle_id)
    ]
    all_lsr_active_positions = [
        p for p in position_rows
        if _is_position_open(p) and ("lsr_v2" in _row_source(p).lower() or _row_cycle(p))
    ]
    ...
    paper_status_consistency = True
    if "open_positions" in status and status_open_positions != len(active_second_positions):
        paper_status_consistency = False
    if "pending_orders" in status and status_pending_orders < len(active_second_orders):
        paper_status_consistency = False
    duplicate_position_check = len(active_second_positions) <= settings.max_positions
    max_positions_check = len(active_second_positions) <= settings.max_positions

    submitted_event_count = sum(_submitted_count(e) for e in exec_rows)
    opened_event_count = sum(_opened_count(e) for e in exec_rows)
    third_submit_or_reentry_detected = bool(submitted_event_count > 1 or opened_event_count > 1 or len(active_second_positions) > settings.max_positions)

    blockers: list[str] = []
    if not exec_rows:
        blockers.append("second_trade_execution_event_not_found")
    if exec_rows and not matched_positions:
        blockers.append("second_trade_matching_position_not_found")
    if exec_rows and not matched_orders:
        blockers.append("second_trade_matching_order_not_found")
    ...
    if not duplicate_position_check:
        blockers.append("duplicate_second_trade_positions_detected")
    ...
    safety_violation = any(
        ...
        or _safe_int(e.get("orders_submitted_by_second_trade_lifecycle_audit"), 0) > 0
        or _safe_int(e.get("positions_opened_by_second_trade_lifecycle_audit"), 0) > 0
        or _safe_int(e.get("positions_closed_by_second_trade_lifecycle_audit"), 0) > 0
        or _safe_bool(e.get("broker_submit_called_by_second_trade_lifecycle_audit"), False)
        or _safe_bool(e.get("broker_close_called_by_second_trade_lifecycle_audit"), False)
        for e in rows
    )
    if safety_violation:
        blockers.append("second_trade_lifecycle_safety_violation_detected")
    ...
    if safety_violation or not duplicate_position_check or not max_positions_check:
        ...
        decision = THIRD_SUBMIT_DETECTED_DECISION
        status_text = "WARN"
    ...
    safety_checks = {
        "no_new_order_by_second_lifecycle_audit": True,
        "no_new_position_by_second_lifecycle_audit": True,
        "no_close_by_second_lifecycle_audit": True,
        "no_broker_submit_by_second_lifecycle_audit": True,
        "no_broker_close_by_second_lifecycle_audit": True,
        "live_disabled": True,
        ...
        "no_third_submit_or_reentry": not third_submit_or_reentry_detected,
    }
    ...
    return {
        ...
        "classification_labels": [
            "LSR_V2_SECOND_TRADE_POSITION_LIFECYCLE_AUDIT",
            "SECOND_ORDER_MONITORING",
        ] + (["SECOND_POSITION_OPEN_MONITORING"] if open_rows else []) + (["KEEP_DIAGNOSTIC"] if decision != READY_DECISION else ["LIFECYCLE_READY"]),
        "blockers": sorted(set(blockers)),
        "cycle_id": str(cycle_id or ""),
        "event_source": str(metadata.get("event_source") or "second_trade_execution_jsonl"),
        "strict_cycle_scope": bool(metadata.get("strict_cycle_scope", True)),
        "historical_second_trade_execution_events": _safe_int(metadata.get("historical_second_trade_execution_events"), 0),
        "historical_second_trade_submitted_events": _safe_int(metadata.get("historical_second_trade_submitted_events"), 0),
        ...
        "second_trade_execution_events": len(exec_rows),
        ...
        "active_second_trade_state_positions": len(active_second_positions),
        "active_second_trade_state_orders": len(active_second_orders),
        "state_open_lsr_v2_positions": len(active_second_positions),
        "state_all_open_lsr_v2_positions": len(all_lsr_active_positions),
        ...
        "third_submit_or_reentry_detected": bool(third_submit_or_reentry_detected),
