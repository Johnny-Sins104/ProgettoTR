...
Diagnostic-only/fail-closed handoff dry-run for a future fourth LSR-v2 paper
trade. The module consumes the validated route preflight and upstream safety
artifacts, then reports whether a paper order intent would be created.
...
PROMPT_ID = "29.4.4t-15"
EVENT_TYPE = "LSR_V2_FOURTH_TRADE_HANDOFF_DRY_RUN"
REPORT_NAME = "lsr_v2_fourth_trade_handoff_dry_run_report.json"
JSONL_NAME = "lsr_v2_fourth_trade_handoff_dry_run.jsonl"
...
PASS_DECISION = "LSR_V2_FOURTH_TRADE_HANDOFF_DRY_RUN_READY"
REPORTS_NOT_READY_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_FOURTH_HANDOFF_DRY_RUN_REPORTS_NOT_READY"
STATE_NOT_LOCKED_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_FOURTH_HANDOFF_DRY_RUN_STATE_NOT_LOCKED"
SOURCE_MARKERS_MISSING_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_FOURTH_HANDOFF_DRY_RUN_SOURCE_MARKERS_MISSING"
ENV_INVALID_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_FOURTH_HANDOFF_DRY_RUN_OPERATOR_ENV_INVALID"
FAILED_DECISION = "REJECT_LSR_V2_FOURTH_HANDOFF_DRY_RUN_SAFETY_FAILED"
...
def _source_marker_audit(project_root: str | Path) -> dict[str, Any]:
    ...
    return {
        "handoff_dry_run_source_files_present": not missing_files,
        "handoff_dry_run_required_markers_present": not missing_files and not missing_markers,
        "missing_handoff_dry_run_source_files": missing_files,
        "missing_handoff_dry_run_source_markers": missing_markers,
    }
...
def _operator_env_controls(env_map: Mapping[str, str] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "handoff_dry_run_operator_env_currently_active": bool(active),
    }
...
def _paper_order_intent_from_route(route: Mapping[str, Any]) -> dict[str, Any]:
    candidate = route.get("route_candidate") if isinstance(route.get("route_candidate"), Mapping) else {}
    would_route = _safe_bool(route.get("would_route"), False)
    route_available = _safe_bool(route.get("route_candidate_available"), False)
    symbol = str(candidate.get("symbol") or "")
    side = str(candidate.get("side") or "").upper()
    if not would_route or not route_available:
        return {}
    ...
    return {
        "symbol": symbol,
        "side": side,
        "trade_ordinal": _safe_int(candidate.get("trade_ordinal"), 4) or 4,
        "cycle_id": str(candidate.get("cycle_id") or route.get("cycle_id") or ""),
        "source_event_type": str(candidate.get("event_type") or ""),
        "source_jsonl": str(candidate.get("source_jsonl") or ""),
        "order_type": "MARKET_DIAGNOSTIC_DRY_RUN",
        "time_in_force": "GTC_DIAGNOSTIC_DRY_RUN",
...
def _handoff_model_from_route(route: Mapping[str, Any]) -> dict[str, Any]:
    route_ready = _safe_bool(route.get("route_preflight_ready"), False)
    route_candidate_available = _safe_bool(route.get("route_candidate_available"), False)
    would_route = _safe_bool(route.get("would_route"), False)
    route_blocked_reason = str(route.get("route_blocked_reason") or "")
    order_intent = _paper_order_intent_from_route(route)
    would_create_order = bool(order_intent)
    handoff_blocked_reason = ""
    if not route_ready:
        handoff_blocked_reason = "route_preflight_not_ready"
    elif not route_candidate_available:
        handoff_blocked_reason = "no_route_candidate_available"
    elif not would_route:
        handoff_blocked_reason = route_blocked_reason or "would_route_false"
    elif not would_create_order:
        handoff_blocked_reason = "order_intent_not_constructible"
    return {
        ...
        "paper_order_intent_ready": would_create_order,
        ...
        "route_blocked_reason": route_blocked_reason,
        "handoff_dry_run_diagnostic": "WOULD_CREATE_ORDER_DIAGNOSTIC" if would_create_order else "NO_ORDER_DRY_RUN_DIAGNOSTIC",
    }


def build_lsr_v2_fourth_trade_handoff_dry_run_report(
    *,
    ...
    inherited = [route, t13, t12, t11, t10, t9, t8, t7, t6, t5, engine_hook, lifecycle, dashboard, postmortem]
    ...
    env_active = _safe_bool(env.get("handoff_dry_run_operator_env_currently_active"), False)
    handoff_model = _handoff_model_from_route(route)

    blockers: list[str] = []
    if not all(report_readiness.values()):
        blockers.append("required_handoff_dry_run_reports_not_ready")
    if not _safe_bool(source_audit.get("handoff_dry_run_required_markers_present"), False):
        blockers.append("handoff_dry_run_required_markers_missing")
    ...
    if env_active:
        blockers.append("operator_env_active_for_handoff_dry_run")
    if live_enabled:
    ...
    elif "operator_env_active_for_handoff_dry_run" in blockers:
        status = "WARN"; decision = ENV_INVALID_DECISION
    elif "handoff_dry_run_required_markers_missing" in blockers:
    ...
    elif "required_handoff_dry_run_reports_not_ready" in blockers:
    ...
    dry_run_ready = status == "PASS"
    would_create_order = _safe_bool(handoff_model.get("would_create_order"), False)
    ...
    return {
        ...
        "classification_labels": [
            "FOURTH_TRADE_HANDOFF_DRY_RUN",
            "HANDOFF_DRY_RUN_ONLY",
            "READ_ONLY",
            "NO_ENGINE_MUTATION",
            "NO_STATE_MUTATION",
            "NO_REENTRY",
            ...
            "NO_SCHEDULER",
            "FAIL_CLOSED",
        ] + (["WOULD_CREATE_ORDER_DIAGNOSTIC_ONLY"] if would_create_order else ["NO_ORDER_DRY_RUN_DIAGNOSTIC"]),
        **report_readiness,
        **state,
        **source_audit,
        **env,
        **handoff_model,
        "handoff_dry_run_ready": dry_run_ready,
        "candidate_handoff_dry_run_ready": dry_run_ready,
        ...
        "broker_submit_called_by_handoff_dry_run": False,
        "broker_close_called_by_handoff_dry_run": False,
        "orders_submitted_by_handoff_dry_run": 0,
        "positions_opened_by_handoff_dry_run": 0,
        "positions_closed_by_handoff_dry_run": 0,
        "paper_state_modified_by_handoff_dry_run": False,
        "paper_status_modified_by_handoff_dry_run": False,
        "submit_execution_events_total": _safe_int(route.get("submit_execution_events_total") or 3, 3),
        "close_execution_events_total": _safe_int(route.get("close_execution_events_total") or 3, 3),
        "aggregate_realized_pnl": _safe_float(route.get("aggregate_realized_pnl") or postmortem.get("aggregate_realized_pnl"), 0.0),
        "balance_after": _safe_float(state.get("balance_after") or route.get("balance_after"), 0.0),
        "realized_pnl_after": _safe_float(state.get("realized_pnl_after"), 0.0),
        "lifecycle_state": str(lifecycle.get("lifecycle_state") or route.get("lifecycle_state") or ""),
        "dashboard_mode": str(dashboard.get("dashboard_mode") or route.get("dashboard_mode") or ""),
        "visual_sl_tp_progress_bar_ready": _safe_bool(dashboard.get("visual_sl_tp_progress_bar_ready"), False),
        "visual_sl_tp_progress_bar": str(dashboard.get("visual_sl_tp_progress_bar") or route.get("visual_sl_tp_progress_bar") or ""),
        ...
        "blocked_until_explicit_submit_preflight_patch": [
            "candidate_submit_preflight",
        ...
        "next_step": "validate_handoff_dry_run_then_wait_for_fourth_candidate_or_prepare_submit_preflight_when_would_create_order",
        "recommended_next_patch": "29.4.4t-16 — LSR-v2 fourth-trade submit preflight",
    }


def build_lsr_v2_fourth_trade_handoff_dry_run_report_from_files(
    *,
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_lsr_v2_fourth_trade_handoff_dry_run_report_from_files(
