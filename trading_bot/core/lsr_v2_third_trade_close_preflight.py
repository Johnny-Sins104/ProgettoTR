"""Prompt 29.4.4s-10ar — LSR-v2 third supervised paper close preflight.

Third-trade-specific close preflight for the LSR-v2 paper position opened by
29.4.4s-10ao and monitored by 29.4.4s-10aq.  The module reads the third
open-position monitor, third lifecycle, paper state and paper status, then
...
PROMPT_ID = "29.4.4s-10ar"
EVENT_TYPE = "LSR_V2_THIRD_TRADE_CLOSE_PREFLIGHT"
REPORT_NAME = "lsr_v2_third_trade_close_preflight_report.json"
JSONL_NAME = "lsr_v2_third_trade_close_preflight.jsonl"

MONITOR_EVENT_TYPE = "LSR_V2_THIRD_OPEN_POSITION_MONITOR"
MONITOR_REPORT_NAME = "lsr_v2_third_open_position_monitor_report.json"
MONITOR_JSONL_NAME = "lsr_v2_third_open_position_monitor.jsonl"
LIFECYCLE_REPORT_NAME = "lsr_v2_third_trade_position_lifecycle_report.json"
LIFECYCLE_JSONL_NAME = "lsr_v2_third_trade_position_lifecycle.jsonl"
PAPER_STATE_NAME = "paper_state.json"
PAPER_STATUS_NAME = "paper_status.json"
PAPER_EVENTS_NAME = "paper_events.jsonl"

READY_DECISION = "LSR_V2_THIRD_TRADE_CLOSE_PREFLIGHT_READY"
CLOSE_NOT_REQUIRED_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_THIRD_CLOSE_NOT_REQUIRED"
...
REJECT_DECISION = "REJECT_LSR_V2_THIRD_CLOSE_PREFLIGHT_FAILED"
...
CLOSE_PREFLIGHT_ENABLE_ENV = "LSR_V2_THIRD_TRADE_CLOSE_PREFLIGHT_ENABLE"
CLOSE_PREFLIGHT_CONFIRM_ENV = "LSR_V2_THIRD_TRADE_CLOSE_PREFLIGHT_CONFIRMATION"
CLOSE_PREFLIGHT_CONFIRMATION = "I_UNDERSTAND_THIRD_CLOSE_PREFLIGHT_ONLY"
...
def _is_third_lsr_v2_position(row: Mapping[str, Any], *, cycle_id: str = "") -> bool:
    ...
    if SOURCE_TAG in source:
        return True
    if "THIRD" in source.upper() and "LSR_V2" in source.upper():
        return True
...
def _select_cycle_id(*, data_dir: str | Path, requested_cycle_id: str = "", settings: "LSRV2ThirdTradeClosePreflightSettings") -> str:
    base = Path(data_dir)
...
def _select_monitor_events(*, data_dir: str | Path, cycle_id: str, settings: "LSRV2ThirdTradeClosePreflightSettings") -> list[dict[str, Any]]:
    base = Path(data_dir)
...
def _find_matching_position(event: Mapping[str, Any], positions: Iterable[Mapping[str, Any]], *, cycle_id: str) -> dict[str, Any] | None:
    ...
    for pos in rows:
        ...
        if _is_position_open(pos):
            score += 2
        if _is_third_lsr_v2_position(pos, cycle_id=cycle_id):
            score += 5
...
class LSRV2ThirdTradeClosePreflightSettings:
    data_dir: str = "data"
...
def third_close_preflight_env_state(env: Mapping[str, str] | None = None) -> dict[str, Any]:
...
def build_lsr_v2_third_trade_close_preflight_event(
    *,
    monitor_event: Mapping[str, Any],
    position: Mapping[str, Any] | None,
    env_state: Mapping[str, Any] | None = None,
    settings: LSRV2ThirdTradeClosePreflightSettings | None = None,
) -> dict[str, Any]:
    settings = settings or LSRV2ThirdTradeClosePreflightSettings()
    env_state = dict(env_state or third_close_preflight_env_state())
    ...
    return {
        ...
        "source": "lsr_v2_third_open_position_monitor",
        ...
        "third_trade_position_open": bool(position_open),
        ...
        "orders_submitted_by_third_trade_close_preflight": 0,
        "positions_closed_by_third_trade_close_preflight": 0,
        "positions_opened_by_third_trade_close_preflight": 0,
...
def build_lsr_v2_third_trade_close_preflight_report_from_files(
    *,
    data_dir: str | Path = "data",
    cycle_id: str = "",
    settings: LSRV2ThirdTradeClosePreflightSettings | None = None,
) -> dict[str, Any]:
    settings = settings or LSRV2ThirdTradeClosePreflightSettings(data_dir=str(data_dir))
    base = Path(data_dir)
    ...
    env_state = third_close_preflight_env_state()
    ...
    for monitor_event in monitor_events:
        ...
        preflight_events.append(build_lsr_v2_third_trade_close_preflight_event(
    ...
    report = summarize_lsr_v2_third_trade_close_preflight(
        data_dir=data_dir,
...
def summarize_lsr_v2_third_trade_close_preflight(
    *,
    data_dir: str | Path,
    cycle_id: str,
    ...
    open_lsr_positions = [p for p in position_rows if _is_position_open(p) and _is_third_lsr_v2_position(p, cycle_id=cycle_id)]
    ...
    blockers: list[str] = []
    if not mon_rows:
        blockers.append("third_open_position_monitor_missing")
    if close_required_rows and not found_rows:
        blockers.append("matching_third_position_not_found")
    if close_required_rows and not open_rows:
        blockers.append("matching_third_position_not_open")
    ...
    safety_violation = any(
        ...
        or _safe_int(e.get("orders_submitted_by_third_trade_close_preflight"), 0) > 0
        or _safe_int(e.get("positions_closed_by_third_trade_close_preflight"), 0) > 0
        or _safe_int(e.get("positions_opened_by_third_trade_close_preflight"), 0) > 0
        for e in rows
    )
    if safety_violation:
        blockers.append("third_trade_close_preflight_safety_violation_detected")
    ...
    safety_checks = {
        "no_new_order_by_third_trade_close_preflight": True,
        "no_new_position_by_third_trade_close_preflight": True,
        "no_close_by_third_trade_close_preflight": True,
        "no_broker_submit_by_third_trade_close_preflight": True,
        "no_broker_close_by_third_trade_close_preflight": True,
        "live_disabled": True,
    ...
    return {
        ...
        "classification_labels": [
            "LSR_V2_THIRD_TRADE_CLOSE_PREFLIGHT",
            "THIRD_TRADE_CLOSE_BOUNDARY",
        ] + (["CLOSE_REQUIRED_DIAGNOSTIC"] if close_required_rows else ["CLOSE_NOT_REQUIRED"]),
        "blockers": sorted(set(blockers)),
        "cycle_id": str(cycle_id or ""),
        "event_source": "third_open_position_monitor_jsonl",
        ...
        "open_third_lsr_v2_position_count": len(open_lsr_positions),
        ...
        "positions_closed_by_third_trade_close_preflight": 0,
        "positions_opened_by_third_trade_close_preflight": 0,
        "broker_submit_called_by_third_trade_close_preflight": False,
        "broker_close_called_by_third_trade_close_preflight": False,
        "automatic_close_enabled": False,
        "automatic_reentry_enabled": False,
        "live_enabled": False,
        "testnet_enabled": False,
...
__all__ = [
    ...
    "REJECT_DECISION",
    "LSRV2ThirdTradeClosePreflightSettings",
    "build_lsr_v2_third_trade_close_preflight_event",
    "build_lsr_v2_third_trade_close_preflight_report_from_files",
    "third_close_preflight_env_state",
]
