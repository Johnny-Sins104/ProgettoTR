"""Prompt 29.4.4s-10ak — LSR-v2 third-trade rearm-aware route/order-intent preflight.

This module is the non-operational route and order-intent preflight for a
third supervised LSR-v2 paper trade.  It requires the third-trade eligibility
...
PROMPT_ID = "29.4.4s-10ak"
REPORT_NAME = "lsr_v2_third_trade_route_preflight_report.json"
JSONL_NAME = "lsr_v2_third_trade_route_preflight.jsonl"

ELIGIBILITY_REPORT_NAME = "lsr_v2_third_trade_eligibility_gate_report.json"
REARM_REPORT_NAME = "lsr_v2_third_trade_rearm_gate_report.json"
...
THIRD_TRADE_ROUTE_EVENT_TYPE = "LSR_V2_THIRD_TRADE_ROUTE_PREFLIGHT"
THIRD_TRADE_INTENT_EVENT_TYPE = "LSR_V2_THIRD_TRADE_ORDER_INTENT_PREFLIGHT"

READY_DECISION = "LSR_V2_THIRD_TRADE_ROUTE_PREFLIGHT_READY"
REARM_MISSING_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_THIRD_TRADE_REARM_MISSING"
CANDIDATE_MISSING_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_THIRD_TRADE_CANDIDATE_MISSING"
OPERATOR_CONFIRMATION_MISSING_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_THIRD_TRADE_OPERATOR_CONFIRMATION_MISSING"
STATE_NOT_CLEAN_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_THIRD_TRADE_STATE_NOT_CLEAN"
REJECT_DECISION = "REJECT_LSR_V2_THIRD_TRADE_ROUTE_PREFLIGHT_FAILED"

REQUIRED_ROUTE_VALUE = "1"
REQUIRED_ROUTE_CONFIRMATION = "I_UNDERSTAND_THIRD_PAPER_TRADE_ROUTE_ONLY"
...
class LSRV2ThirdTradeRoutePreflightSettings:
    data_dir: str = "data"
    ...
    @classmethod
    def from_env(cls, data_dir: str = "data") -> "LSRV2ThirdTradeRoutePreflightSettings":
        max_orders = _safe_int(os.getenv("LSR_V2_THIRD_TRADE_MAX_ORDERS"), 1)
        if max_orders <= 0:
            max_orders = 1
        return cls(
            data_dir=data_dir,
            route_enable=str(os.getenv("LSR_V2_THIRD_TRADE_ROUTE_ENABLE") or ""),
            route_confirmation=str(os.getenv("LSR_V2_THIRD_TRADE_ROUTE_CONFIRMATION") or ""),
            max_orders=max_orders,
            mode=str(os.getenv("LSR_V2_THIRD_TRADE_ROUTE_MODE") or "paper"),
        )
...
def build_third_trade_route_event(*, candidate: Mapping[str, Any], settings: LSRV2ThirdTradeRoutePreflightSettings) -> dict[str, Any]:
    ...
    return {
        "event_type": THIRD_TRADE_ROUTE_EVENT_TYPE,
        "prompt": PROMPT_ID,
        "ts": utc_now_iso(),
        ...
        "third_trade_route_enabled": True,
        "third_trade_route_confirmation_ok": True,
        "third_trade_would_route": True,
        "would_route": True,
        "would_submit": False,
        "broker_submit_called": False,
        "blocked_reason": "third_trade_submit_still_disabled",
...
def build_third_trade_order_intent_event(*, route_event: Mapping[str, Any], settings: LSRV2ThirdTradeRoutePreflightSettings) -> dict[str, Any]:
...
def build_lsr_v2_third_trade_route_preflight_report_from_files(
    *,
    data_dir: str | Path = "data",
    cycle_id: str = "",
    settings: LSRV2ThirdTradeRoutePreflightSettings | None = None,
    write_outputs: bool = True,
) -> dict[str, Any]:
    base = Path(data_dir)
    settings = settings or LSRV2ThirdTradeRoutePreflightSettings.from_env(str(base))
    ...
    eligibility_pass = eligibility.get("status") == "PASS" and eligibility.get("decision") == "LSR_V2_THIRD_PAPER_TRADE_ELIGIBILITY_PASS"
    third_trade_eligible = _safe_bool(eligibility.get("third_trade_eligible"), False) and eligibility_pass
    rearm_pass = rearm.get("status") == "PASS" and rearm.get("decision") in {
        "LSR_V2_THIRD_TRADE_REARM_READY_DIAGNOSTIC",
        "LSR_V2_THIRD_TRADE_CANDIDATE_WAIT_GATE_READY",
    }
    third_trade_rearm_ready = _safe_bool(rearm.get("third_trade_rearm_ready"), False)
    ...
    upstream_activity = any(
        ...
        for obj in (eligibility, rearm)
        for key in [
            "orders_submitted_by_third_trade_gate",
            "positions_opened_by_third_trade_gate",
            "orders_submitted_by_third_trade_rearm_gate",
            "positions_opened_by_third_trade_rearm_gate",
            "positions_closed_by_third_trade_rearm_gate",
        ]
    )

    blockers: list[str] = []
    if not third_trade_eligible:
        blockers.append("third_trade_eligibility_not_pass")
    if not rearm_pass or not candidate_wait_gate_ready:
        blockers.append("third_trade_rearm_missing")
    if not third_trade_rearm_ready:
        blockers.append("third_trade_rearm_not_ready")
    if not clean_state:
    ...
    if route_enabled and route_confirmation_ok and not ready_candidate:
        blockers.append("third_trade_candidate_missing")

    route_events: list[dict[str, Any]] = []
    intent_events: list[dict[str, Any]] = []
    if route_enabled and route_confirmation_ok and ready_candidate and not blockers:
        route_event = build_third_trade_route_event(candidate=ready_candidate, settings=settings)
        intent_event = build_third_trade_order_intent_event(route_event=route_event, settings=settings)
        route_events = [route_event]
        intent_events = [intent_event]

    third_trade_would_route_count = sum(1 for row in route_events if _safe_bool(row.get("third_trade_would_route"), False))
    ...
    if unsafe_upstream or upstream_activity or settings.max_orders != 1 or runtime_would_submit_count > 0:
        ...
        status = "WARN"
    elif not third_trade_eligible or not rearm_pass or not candidate_wait_gate_ready or not third_trade_rearm_ready:
    ...
    if status == "PASS":
        classification_labels.extend(["THIRD_TRADE_ROUTE_PREFLIGHT_PASS", "THIRD_TRADE_ORDER_INTENT_READY", "SUBMIT_STILL_DISABLED"])
    elif decision == REARM_MISSING_DECISION:
        classification_labels.append("THIRD_TRADE_REARM_MISSING")
    elif decision == CANDIDATE_MISSING_DECISION:
        classification_labels.append("THIRD_TRADE_CANDIDATE_MISSING")
    elif decision == OPERATOR_CONFIRMATION_MISSING_DECISION:
        classification_labels.append("THIRD_TRADE_ROUTE_CONFIRMATION_MISSING")
    elif decision == STATE_NOT_CLEAN_DECISION:
        classification_labels.append("THIRD_TRADE_STATE_NOT_CLEAN")
    else:
        classification_labels.append("THIRD_TRADE_ROUTE_PREFLIGHT_REJECTED")
    ...
    report = {
        ...
        "third_trade_eligible": bool(third_trade_eligible),
        "third_trade_rearm_ready": bool(third_trade_rearm_ready),
        "candidate_wait_gate_ready": bool(candidate_wait_gate_ready),
        "third_trade_route_enabled": bool(route_enabled),
        "third_trade_route_confirmation_ok": bool(route_confirmation_ok),
        "third_trade_route_preflight_ready": bool(decision == READY_DECISION),
        "third_trade_order_intent_ready": bool(decision == READY_DECISION and would_create_order_count > 0),
        "third_trade_execute_enabled": False,
        "third_trade_submit_enabled": False,
        "paper_order_submission_enabled": False,
        "routing_enabled": False,
        "execution_enabled": False,
        "broker_submit_called_by_third_trade_route_preflight": False,
        "orders_submitted_by_third_trade_route_preflight": 0,
        "positions_opened_by_third_trade_route_preflight": 0,
        "positions_closed_by_third_trade_route_preflight": 0,
        "paper_state_modified_by_third_trade_route_preflight": False,
        "paper_status_modified_by_third_trade_route_preflight": False,
        "live_enabled": False,
        "testnet_enabled": False,
        "exchange_broker_enabled": False,
        "operational_unlock_allowed": False,
        ...
        "third_trade_route_events": len(route_events),
        "third_trade_would_route_count": third_trade_would_route_count,
        "third_trade_order_intent_events": third_trade_order_intent_events,
