"""Generic LSR-v2 supervised paper close preflight.

29.4.4u-31 is intentionally preflight-only, read-only, and fail-closed.
It consumes u-30's open-position monitor preflight artifact and models whether a
future paper close preflight could safely evaluate an open paper position for
close readiness.
...
expected validation environment no broker submit receipt exists, no real open
position exists, the monitor is not active, operator gates are absent, and
runtime values are still deferred, so close execution remains blocked while the
close preflight model is marked ready.
"""
...
PROMPT = "29.4.4u-31"
EVENT_TYPE = "LSR_V2_GENERIC_SUPERVISED_PAPER_CLOSE_PREFLIGHT"
READY_DECISION = "LSR_V2_GENERIC_SUPERVISED_PAPER_CLOSE_PREFLIGHT_READY"
KEEP_DIAGNOSTIC_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_GENERIC_SUPERVISED_PAPER_CLOSE_PREFLIGHT_NOT_READY"
U30_READY_DECISION = "LSR_V2_GENERIC_SUPERVISED_PAPER_OPEN_POSITION_MONITOR_PREFLIGHT_READY"
...
CLOSE_PREFLIGHT_STAGES: Tuple[str, ...] = (
    "load_generic_open_position_monitor_preflight",
    "verify_open_position_monitor_preflight_ready",
    "verify_monitor_execution_was_blocked",
    "verify_no_broker_submit_receipt",
    "verify_no_open_position_available",
    "verify_close_preflight_requires_open_position_before_execution",
    "verify_close_trigger_absent",
    "verify_close_operator_gate_absent",
    ...
    "map_close_preflight_gate",
    "publish_generic_close_preflight_artifact",
)

BLOCKED_UNTIL_EXPLICIT_CLOSE_EXECUTION_PATCH: Tuple[str, ...] = (
...
class Settings:
    ...
    data_dir: str = "data"
    report_name: str = "lsr_v2_generic_supervised_paper_close_preflight_report.json"
    jsonl_name: str = "lsr_v2_generic_supervised_paper_close_preflight.jsonl"
...
def _dry_run_payload(monitor_report: Mapping[str, Any]) -> Dict[str, Any]:
    monitor_map = _as_dict(monitor_report.get("generic_supervised_paper_open_position_monitor_preflight_map", {}))
    payload = _as_dict(monitor_map.get("dry_run_order_intent_payload", {}))
    if payload:
        return payload
    submit_map = _as_dict(monitor_report.get("generic_supervised_paper_submit_execution_map", {}))
    payload = _as_dict(submit_map.get("dry_run_order_intent_payload", {}))
    if payload:
        return payload
    activation_map = _as_dict(monitor_report.get("generic_order_intent_activation_map", {}))
    return _as_dict(activation_map.get("dry_run_order_intent_payload", {}))


def _build_close_preflight_map(monitor_report: Mapping[str, Any], active_env: Mapping[str, str]) -> Dict[str, Any]:
    payload = _dry_run_payload(monitor_report)
    ...
    broker_submit_receipt_available = monitor_report.get("broker_submit_receipt_available") is True
    open_position_available = monitor_report.get("paper_open_position_available") is True
    monitor_ready = monitor_report.get("paper_open_position_monitor_ready") is True
    monitor_allowed = monitor_report.get("generic_open_position_monitor_allowed") is True
    close_trigger_available = (
        monitor_report.get("would_trigger_stop_loss") is True
        or monitor_report.get("would_trigger_take_profit") is True
    )
    close_preflight_allowed = (
    ...
    if not monitor_allowed:
        blocked_reasons.append("open_position_monitor_not_allowed")
    ...
    return {
        ...
        "stages": [
            ...
            for stage in CLOSE_PREFLIGHT_STAGES
        ],
        ...
        "close_preflight_gate_model": {
            ...
            "close_preflight_allowed": close_preflight_allowed,
            "close_preflight_blocked_reasons": blocked_reasons,
            ...
            "generic_open_position_monitor_allowed": monitor_allowed,
            "close_trigger_required": True,
            "close_trigger_available": close_trigger_available,
            "stop_loss_triggered": monitor_report.get("would_trigger_stop_loss") is True,
            "take_profit_triggered": monitor_report.get("would_trigger_take_profit") is True,
        ...
        "source_open_position_monitor_preflight_map": _as_dict(
            monitor_report.get("generic_supervised_paper_open_position_monitor_preflight_map", {})
        ),
        "required_future_controls_before_close_execution": {
            ...
            "explicit_generic_close_execution_patch_required": True,
...
def run_generic_supervised_paper_close_preflight(settings: Settings | None = None) -> Dict[str, Any]:
    settings = settings or Settings()
    data_path = settings.data_path
    upstream_name = "lsr_v2_generic_supervised_paper_open_position_monitor_preflight_report.json"
    monitor_report = _read_json(data_path / upstream_name)
    ...
    elif monitor_report.get("decision") != U30_READY_DECISION:
    ...
    close_map = _build_close_preflight_map(monitor_report, active_env)
    gate_model = _as_dict(close_map.get("close_preflight_gate_model", {}))
    context = _as_dict(close_map.get("close_preflight_context", {}))

    source_counts = _as_dict(monitor_report.get("candidate_detection_source_counts", {}))
    diagnostic_count = _safe_int(monitor_report.get("open_position_monitor_preflight_diagnostic_count"), 0) or _safe_int(
        monitor_report.get("submit_execution_diagnostic_count"), 0
    )
    upstream_diagnostic_available = monitor_report.get("open_position_monitor_preflight_diagnostic_available") is True
    present_fields_ok = _as_list(close_map.get("present_contract_fields", [])) == list(REQUIRED_ORDER_INTENT_FIELDS)
    required_fields_ok = _as_list(close_map.get("required_contract_fields", [])) == list(REQUIRED_ORDER_INTENT_FIELDS)
    missing_fields_ok = _as_list(close_map.get("missing_contract_fields", [])) == []
    ...
    close_preflight_allowed = gate_model.get("close_preflight_allowed") is True

    required_readiness = {
        "open_position_monitor_preflight_report_ready": monitor_report.get("generic_supervised_paper_open_position_monitor_preflight_ready") is True,
        "generic_open_position_monitor_preflight_ready": monitor_report.get("generic_open_position_monitor_preflight_ready") is True,
        "paper_open_position_monitor_preflight_ready": monitor_report.get("paper_open_position_monitor_preflight_ready") is True,
        "open_position_monitor_preflight_diagnostic_available": upstream_diagnostic_available,
        "open_position_monitor_contract_shape_modelable": monitor_report.get("open_position_monitor_contract_shape_modelable") is True,
        "close_preflight_model_ready": gate_model.get("close_preflight_model_ready") is True,
        "broker_submit_receipt_required": True,
        ...
        "close_trigger_absent": gate_model.get("close_trigger_available") is False,
        "close_preflight_blocked": not close_preflight_allowed,
        ...
        "runtime_validation_deferred": monitor_report.get("paper_order_intent_runtime_value_validation_deferred") is True,
        "runtime_validation_not_executed": monitor_report.get("paper_order_intent_runtime_value_validation_executed") is False,
        "no_broker_call": (
            monitor_report.get("broker_submit_called_by_generic_open_position_monitor_preflight") is False
            and monitor_report.get("broker_close_called_by_generic_open_position_monitor_preflight") is False
        ),
        "no_submit_or_close_occurred": (
            monitor_report.get("orders_submitted_by_generic_open_position_monitor_preflight") == 0
            and monitor_report.get("positions_opened_by_generic_open_position_monitor_preflight") == 0
            and monitor_report.get("positions_closed_by_generic_open_position_monitor_preflight") == 0
        ),
        "no_state_mutation": (
            monitor_report.get("paper_state_modified_by_generic_open_position_monitor_preflight") is False
            and monitor_report.get("paper_status_modified_by_generic_open_position_monitor_preflight") is False
        ),
        "flat_locked_state": monitor_report.get("lifecycle_state") == "FLAT_LOCKED",
        "flat_positions": monitor_report.get("open_positions_after") == 0,
        "pending_orders_clear": monitor_report.get("pending_orders_after") == 0,
        "single_position_policy_ready": True,
        "paper_only_policy_ready": True,
        "no_existing_route_candidate": monitor_report.get("route_candidate_available") is False,
        "no_existing_handoff_candidate": monitor_report.get("handoff_candidate_available") is False,
        "operator_env_absent": len(active_env) == 0,
        "live_disabled": monitor_report.get("live_enabled") is False,
        "testnet_disabled": monitor_report.get("testnet_enabled") is False,
        "exchange_broker_disabled": monitor_report.get("exchange_broker_enabled") is False,
        ...
        "execution_flags_fail_closed": _all_false(monitor_report, FALSE_EXECUTION_KEYS),
    ...
    classification_labels = [
        ...
        "NO_SCHEDULER",
        "FAIL_CLOSED",
        "OPEN_POSITION_MONITOR_REQUIRED",
        "CLOSE_TRIGGER_REQUIRED",
        "GENERIC_SUPERVISED_PAPER_CLOSE_PREFLIGHT_READY" if ready else "GENERIC_SUPERVISED_PAPER_CLOSE_PREFLIGHT_NOT_READY",
    ]
    ...
    report: Dict[str, Any] = {
        ...
        "upstream_reports_present": [] if not monitor_report else [upstream_name],
        "missing_upstream_reports": missing_upstream_reports,
        "not_ready_upstream_reports": not_ready_upstream_reports,
        "generic_supervised_paper_close_preflight_ready": ready,
        "generic_close_preflight_ready": ready,
        "paper_close_preflight_ready": ready,
        "generic_close_preflight_model_ready": gate_model.get("close_preflight_model_ready") is True,
        "generic_close_preflight_plan_ready": ready,
        "generic_close_preflight_map_ready": ready,
        "generic_close_preflight_contract_ready": ready,
        "generic_close_preflight_fail_closed_ready": ready,
        "generic_close_preflight_source_files_present": not missing_source_files,
        "generic_close_preflight_required_markers_present": not missing_source_markers,
        "close_preflight_diagnostic_available": ready,
        "close_preflight_diagnostic_count": diagnostic_count if ready else 0,
        "close_preflight_contract_shape_modelable": gate_model.get("close_preflight_model_ready") is True,
        "close_preflight_contract_missing_fields": _as_list(close_map.get("missing_contract_fields", [])),
        "close_preflight_contract_present_fields": _as_list(close_map.get("present_contract_fields", [])),
        "close_preflight_contract_required_fields": _as_list(close_map.get("required_contract_fields", [])),
        "generic_supervised_paper_open_position_monitor_preflight_ready": monitor_report.get("generic_supervised_paper_open_position_monitor_preflight_ready") is True,
        "generic_open_position_monitor_preflight_ready": monitor_report.get("generic_open_position_monitor_preflight_ready") is True,
        "paper_open_position_monitor_preflight_ready": monitor_report.get("paper_open_position_monitor_preflight_ready") is True,
        "open_position_monitor_preflight_diagnostic_available": upstream_diagnostic_available,
        "open_position_monitor_preflight_diagnostic_count": diagnostic_count,
        "open_position_monitor_contract_shape_modelable": monitor_report.get("open_position_monitor_contract_shape_modelable") is True,
        "submit_execution_diagnostic_available": monitor_report.get("submit_execution_diagnostic_available") is True,
        "submit_execution_diagnostic_count": _safe_int(monitor_report.get("submit_execution_diagnostic_count"), 0),
        "order_intent_activation_diagnostic_available": monitor_report.get("order_intent_activation_diagnostic_available") is True,
        "order_intent_activation_diagnostic_count": _safe_int(monitor_report.get("order_intent_activation_diagnostic_count"), 0),
        "candidate_detection_source_counts": source_counts,
        "candidate_diagnostic_examples": _candidate_examples(monitor_report),
        "broker_submit_receipt_available": broker_submit_receipt_available,
        "paper_open_position_available": open_position_available,
        "paper_position_open": False,
        "paper_open_position_monitor_ready": False,
        ...
        "paper_close_execution_ready": False,
        "broker_close_allowed": False,
        "would_materialize_order_intent_dry_run": monitor_report.get("would_materialize_order_intent_dry_run") is True and ready,
        ...
        "would_call_paper_broker_close": False,
        "would_close": False,
        "paper_close_preflight_blocked_reason": "close_preflight_blocked_until_real_open_position_monitor_and_close_trigger",
        "paper_close_execution_blocked_reason": "close_execution_not_allowed_close_preflight_only",
        "generic_close_preflight_blocked_reason": "close_preflight_not_allowed_without_open_position_monitor_and_close_trigger",
        "generic_supervised_paper_close_preflight_map": close_map,
        "generic_supervised_paper_open_position_monitor_preflight_map": _as_dict(
            monitor_report.get("generic_supervised_paper_open_position_monitor_preflight_map", {})
        ),
        "generic_supervised_paper_submit_execution_map": _as_dict(monitor_report.get("generic_supervised_paper_submit_execution_map", {})),
        "generic_order_intent_activation_map": _as_dict(monitor_report.get("generic_order_intent_activation_map", {})),
        "blocked_until_explicit_close_execution_patch": list(BLOCKED_UNTIL_EXPLICIT_CLOSE_EXECUTION_PATCH),
        "active_lsr_v2_operator_env_count": len(active_env),
        "active_lsr_v2_operator_env_keys": sorted(active_env),
        "operator_env_absent": len(active_env) == 0,
        "lifecycle_state": monitor_report.get("lifecycle_state", ""),
        "fourth_trade_locked": monitor_report.get("fourth_trade_locked") is True,
        "stability_lock_active": monitor_report.get("stability_lock_active") is True,
        ...
        "paper_state_status_consistency": monitor_report.get("paper_state_status_consistency") is True,
        ...
        "missing_generic_close_preflight_source_files": missing_source_files,
        "missing_generic_close_preflight_source_markers": missing_source_markers,
        "source_markers_present": source_markers_present,
        "settings": {
        ...
        "recommended_next_patch": "29.4.4u-32 — Generic LSR-v2 supervised paper close execution scaffold",
        "next_step": "prepare_generic_supervised_paper_close_execution_scaffold_or_continue_observation",
    }
