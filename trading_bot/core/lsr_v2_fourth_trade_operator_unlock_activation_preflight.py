...
Preflight-only/fail-closed validation for a future explicit operator activation of
fourth-trade paper-only re-arm. This module reads the validated t-10 operator
unlock draft and the upstream read-only artifacts, then verifies whether the
operator activation controls are absent, invalid, or valid for a *future*
activation patch.
...
PROMPT_ID = "29.4.4t-11"
EVENT_TYPE = "LSR_V2_FOURTH_TRADE_OPERATOR_UNLOCK_ACTIVATION_PREFLIGHT"
REPORT_NAME = "lsr_v2_fourth_trade_operator_unlock_activation_preflight_report.json"
JSONL_NAME = "lsr_v2_fourth_trade_operator_unlock_activation_preflight.jsonl"
...
PASS_DECISION = "LSR_V2_FOURTH_TRADE_OPERATOR_UNLOCK_ACTIVATION_PREFLIGHT_READY"
REPORTS_NOT_READY_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_OPERATOR_UNLOCK_ACTIVATION_REPORTS_NOT_READY"
SOURCE_MARKERS_MISSING_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_OPERATOR_UNLOCK_ACTIVATION_SOURCE_MARKERS_MISSING"
STATE_NOT_LOCKED_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_OPERATOR_UNLOCK_ACTIVATION_STATE_NOT_LOCKED"
ENV_INVALID_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_OPERATOR_UNLOCK_ACTIVATION_ENV_INVALID"
FAILED_DECISION = "REJECT_LSR_V2_OPERATOR_UNLOCK_ACTIVATION_PREFLIGHT_SAFETY_FAILED"
...
_SOURCE_MARKERS: dict[str, tuple[str, ...]] = {
    "trading_bot/core/lsr_v2_fourth_trade_operator_unlock_draft.py": (
        ...
        "operator_unlock_requires_activation_patch",
    ),
...
def _operator_env_controls(env_map: Mapping[str, str] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "operator_unlock_activation_env_currently_active": env_active,
        "operator_unlock_activation_enable_env_name": OPERATOR_UNLOCK_ENABLE_ENV,
        "operator_unlock_activation_confirmation_env_name": OPERATOR_UNLOCK_CONFIRMATION_ENV,
        "operator_unlock_activation_max_positions_env_name": OPERATOR_UNLOCK_MAX_POSITIONS_ENV,
        "operator_unlock_activation_enable_env_value": enable_raw,
        "operator_unlock_activation_confirmation_env_value_present": bool(confirmation_raw),
        "operator_unlock_activation_max_positions_env_value": max_positions_raw,
        "operator_unlock_activation_enable_requested": enable_requested,
        "operator_unlock_activation_confirmation_ok": confirmation_ok,
        "operator_unlock_activation_confirmation_phrase": OPERATOR_UNLOCK_CONFIRMATION_PHRASE,
        "operator_unlock_activation_max_positions": OPERATOR_UNLOCK_MAX_POSITIONS,
        "operator_unlock_activation_max_positions_ok": max_positions_ok,
        "operator_unlock_activation_required_envs_present": required_present,
        "operator_unlock_activation_unrelated_env_keys": unrelated,
        "operator_unlock_activation_controls_valid": controls_valid,
        "operator_unlock_activation_preflight_armed": controls_valid,
    }
...
def _source_marker_audit(project_root: str | Path) -> dict[str, Any]:
    ...
    return {
        "operator_unlock_activation_source_files_present": not missing_files,
        "operator_unlock_activation_required_markers_present": not missing_files and not missing_markers,
        "missing_operator_unlock_activation_source_files": missing_files,
        "missing_operator_unlock_activation_source_markers": missing_markers,
    }
...
def build_lsr_v2_fourth_trade_operator_unlock_activation_preflight_report(
    *,
    ...
    report_readiness = {
        "operator_unlock_draft_ready": _pass_report(unlock_draft, UNLOCK_DRAFT_PASS_DECISION)
        and _safe_bool(unlock_draft.get("operator_unlock_draft_ready"), False),
    ...
    inherited = [unlock_draft, operator_gate, rearm, candidate_hook, handoff, parity, engine_hook, lifecycle, dashboard, postmortem]
    ...
    env_active = _safe_bool(env.get("operator_unlock_activation_env_currently_active"), False)
    env_valid = _safe_bool(env.get("operator_unlock_activation_controls_valid"), False)
    env_valid_if_active = (not env_active) or env_valid

    blockers: list[str] = []
    if not all(report_readiness.values()):
        blockers.append("required_operator_unlock_activation_reports_not_ready")
    if not _safe_bool(source_audit.get("operator_unlock_activation_required_markers_present"), False):
        blockers.append("operator_unlock_activation_required_markers_missing")
    ...
    if env_active and not env_valid_if_active:
        blockers.append("operator_unlock_activation_env_invalid")
    if live_enabled:
    ...
    elif "operator_unlock_activation_env_invalid" in blockers:
        status = "WARN"
        decision = ENV_INVALID_DECISION
    elif "operator_unlock_activation_required_markers_missing" in blockers:
        status = "WARN"
    ...
    elif "required_operator_unlock_activation_reports_not_ready" in blockers:
        status = "WARN"
    ...
    activation_preflight_ready = status == "PASS"
    activation_armed = activation_preflight_ready and env_valid

    aggregate_realized_pnl = _safe_float(
        unlock_draft.get("aggregate_realized_pnl")
        or operator_gate.get("aggregate_realized_pnl")
    ...
    balance_after = _safe_float(
        ...
        or unlock_draft.get("balance_after")
    ...
    report: dict[str, Any] = {
        ...
        "classification_labels": [
            "FOURTH_TRADE_OPERATOR_UNLOCK_ACTIVATION_PREFLIGHT",
            ...
            "NO_SCHEDULER",
            "FAIL_CLOSED",
        ] + (["OPERATOR_CONTROLS_VALID_BUT_EXECUTION_STILL_BLOCKED"] if activation_armed else []),
        **report_readiness,
        **state,
        **source_audit,
        **env,
        "operator_unlock_activation_preflight_ready": activation_preflight_ready,
        "guarded_operator_unlock_activation_preflight_ready": activation_preflight_ready,
        "operator_unlock_activation_plan_ready": activation_preflight_ready,
        "operator_unlock_activation_preflight_armed": activation_armed,
        "operator_unlock_activation_allowed": False,
        "operator_unlock_activation_requires_execution_patch": True,
        "operator_unlock_activation_would_unlock_fourth_trade": activation_armed,
        "operator_unlock_activation_would_enable_candidate_detection": activation_armed,
        "operator_unlock_activation_would_enable_paper_only_execution": False,
        ...
        "broker_submit_called_by_operator_unlock_activation_preflight": False,
        "broker_close_called_by_operator_unlock_activation_preflight": False,
        ...
        "submit_execution_events_total": _safe_int(unlock_draft.get("submit_execution_events_total") or operator_gate.get("submit_execution_events_total"), 3),
        "close_execution_events_total": _safe_int(unlock_draft.get("close_execution_events_total") or operator_gate.get("close_execution_events_total"), 3),
        "aggregate_realized_pnl": aggregate_realized_pnl,
        "balance_after": balance_after,
        "realized_pnl_after": _safe_float(state.get("realized_pnl_after"), 0.0),
        "lifecycle_state": str(lifecycle.get("lifecycle_state") or unlock_draft.get("lifecycle_state") or operator_gate.get("lifecycle_state") or ""),
        "dashboard_mode": str(dashboard.get("dashboard_mode") or unlock_draft.get("dashboard_mode") or operator_gate.get("dashboard_mode") or ""),
        "visual_sl_tp_progress_bar_ready": _safe_bool(dashboard.get("visual_sl_tp_progress_bar_ready"), False),
        "visual_sl_tp_progress_bar": str(dashboard.get("visual_sl_tp_progress_bar") or unlock_draft.get("visual_sl_tp_progress_bar") or ""),
        ...
        "blocked_until_explicit_execution_patch": [
            "operator_unlock_execution",
            "fourth_trade_rearm",
            "candidate_detection",
        ...
        "operator_unlock_activation_inputs": [
        ...
        "next_step": "validate_activation_preflight_then_prepare_guarded_unlock_execution_or_candidate_detection_preflight",
        "recommended_next_patch": "29.4.4t-12 — LSR-v2 guarded paper-only fourth-trade unlock activation / candidate detection preflight",
    }
    return report


def build_lsr_v2_fourth_trade_operator_unlock_activation_preflight_report_from_files(
    *,
    ...
    report = build_lsr_v2_fourth_trade_operator_unlock_activation_preflight_report(
        reports=reports,
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_lsr_v2_fourth_trade_operator_unlock_activation_preflight_report_from_files(
