...
Closes exactly one already-open third LSR-v2 paper position when the third trade close preflight
has detected a TP/SL/close-required diagnostic and the operator supplies the
explicit third close confirmation.  This module is paper-state only: it never opens a
...
CLOSE_PREFLIGHT_JSONL_NAME = "lsr_v2_third_trade_close_preflight.jsonl"
...
CLOSE_CONFIRMATION_PHRASE = "I_UNDERSTAND_CLOSE_THIRD_PAPER_POSITION_ONLY"

CLOSED_DECISION = "LSR_V2_THIRD_SINGLE_PAPER_POSITION_CLOSED"
NOT_ENABLED_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_THIRD_CLOSE_NOT_ENABLED"
CONFIRMATION_MISSING_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_THIRD_CLOSE_CONFIRMATION_MISSING"
PREFLIGHT_MISSING_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_THIRD_CLOSE_PREFLIGHT_MISSING"
...
MAX_POSITION_CAP_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_THIRD_CLOSE_MAX_POSITION_CAP_BLOCKED"
REJECT_DECISION = "REJECT_LSR_V2_THIRD_CLOSE_EXECUTION_SAFETY_FAILED"
...
class LSRV2ThirdTradeCloseExecutionSettings:
    data_dir: str = "data"
    ...
    @classmethod
    def from_env(cls, data_dir: str = "data") -> "LSRV2ThirdTradeCloseExecutionSettings":
...
def select_third_close_cycle_id(data_dir: str | Path, requested_cycle_id: str = "", settings: LSRV2ThirdTradeCloseExecutionSettings | None = None) -> str:
    settings = settings or LSRV2ThirdTradeCloseExecutionSettings(data_dir=str(data_dir))
    base = Path(data_dir)
...
def select_cycle_scoped_third_close_preflight_events(data_dir: str | Path, cycle_id: str, settings: LSRV2ThirdTradeCloseExecutionSettings | None = None) -> list[dict[str, Any]]:
    settings = settings or LSRV2ThirdTradeCloseExecutionSettings(data_dir=str(data_dir))
    rows = _iter_jsonl_tail(Path(data_dir) / settings.close_preflight_jsonl_name, max_lines=settings.max_event_lines)
    selected = [dict(r) for r in rows if r.get("event_type") == "LSR_V2_THIRD_TRADE_CLOSE_PREFLIGHT" and (not cycle_id or str(r.get("cycle_id") or "") == cycle_id)]
    if selected:
        return selected
...
def _operator_controls(settings: LSRV2ThirdTradeCloseExecutionSettings) -> dict[str, Any]:
    return {
...
def build_lsr_v2_third_trade_close_execution_report_from_files(
    *,
    data_dir: str | Path = "data",
    cycle_id: str = "",
    settings: LSRV2ThirdTradeCloseExecutionSettings | None = None,
) -> dict[str, Any]:
    settings = settings or LSRV2ThirdTradeCloseExecutionSettings.from_env(data_dir=str(data_dir))
    base = Path(data_dir)
    selected_cycle_id = select_third_close_cycle_id(data_dir, requested_cycle_id=cycle_id, settings=settings)
    preflight_events = select_cycle_scoped_third_close_preflight_events(data_dir, selected_cycle_id, settings)
    ...
    open_lsr_positions = [p for p in positions if _is_position_open(p) and _is_third_lsr_v2_position(p, cycle_id=selected_cycle_id)]
    ...
    if can_close:
        ...
        for event in close_required[: settings.close_max_positions]:
            ...
            closed_events.append({
                ...
                "source": "lsr_v2_third_trade_close_preflight",
    ...
    if safety_violation:
        ...
        decision = "KEEP_DIAGNOSTIC_LSR_V2_THIRD_CLOSE_NOT_REQUIRED"
        status_text = "WARN"
    ...
    after_open_lsr = [p for p in after_positions if _is_position_open(p) and _is_third_lsr_v2_position(p, cycle_id=selected_cycle_id)]
    ...
    report = {
        ...
        "status": status_text,
        "decision": decision,
        "classification_labels": ["LSR_V2_THIRD_TRADE_CLOSE_EXECUTION"],
        "blockers": sorted(set(blockers)),
        "cycle_id": selected_cycle_id,
        "event_source": "third_trade_close_preflight_jsonl",
        ...
        "positions_closed_by_third_trade_close_execution": int(positions_closed),
        "orders_submitted_by_third_trade_close_execution": 0,
        "positions_opened_by_third_trade_close_execution": 0,
        ...
        "open_third_lsr_v2_positions_before": len(open_lsr_positions),
        "open_third_lsr_v2_positions_after": len(after_open_lsr),
