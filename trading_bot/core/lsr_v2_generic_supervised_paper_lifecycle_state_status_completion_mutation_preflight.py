"""Generic LSR-v2 supervised paper lifecycle state/status completion mutation preflight.

29.4.4u-44 is a preflight-only, read-only-by-default, fail-closed lifecycle
state/status completion mutation checkpoint. It consumes u-43's lifecycle
completion audit execution artifact and models the future requirements for
mutating paper lifecycle state/status into a completed terminal state.
...
must return a PASS/ready diagnostic for the lifecycle state/status completion
mutation preflight model while keeping paper_state/paper_status mutation,
Telegram/network sending, scheduler startup, broker calls, submit/close, and
...
PROMPT = "29.4.4u-44"
EVENT_TYPE = "LSR_V2_GENERIC_SUPERVISED_PAPER_LIFECYCLE_STATE_STATUS_COMPLETION_MUTATION_PREFLIGHT"
READY_DECISION = "LSR_V2_GENERIC_SUPERVISED_PAPER_LIFECYCLE_STATE_STATUS_COMPLETION_MUTATION_PREFLIGHT_READY"
KEEP_DIAGNOSTIC_DECISION = "KEEP_DIAGNOSTIC_LSR_V2_GENERIC_SUPERVISED_PAPER_LIFECYCLE_STATE_STATUS_COMPLETION_MUTATION_PREFLIGHT_NOT_READY"
U43_READY_DECISION = "LSR_V2_GENERIC_SUPERVISED_PAPER_LIFECYCLE_COMPLETION_AUDIT_EXECUTION_READY"
...
STATE_STATUS_COMPLETION_MUTATION_PREFLIGHT_STAGES: Tuple[str, ...] = (
    "load_generic_lifecycle_completion_audit_execution_report",
    "verify_lifecycle_completion_audit_execution_model_ready",
    "verify_lifecycle_completion_audit_execution_was_blocked",
    ...
    "map_lifecycle_state_status_completion_mutation_preflight_gate",
    "publish_generic_lifecycle_state_status_completion_mutation_preflight_artifact",
)

BLOCKED_UNTIL_EXPLICIT_LIFECYCLE_STATE_STATUS_COMPLETION_MUTATION_PATCH: Tuple[str, ...] = (
...
class Settings:
    ...
    data_dir: str = "data"
    report_name: str = "lsr_v2_generic_supervised_paper_lifecycle_state_status_completion_mutation_preflight_report.json"
    jsonl_name: str = "lsr_v2_generic_supervised_paper_lifecycle_state_status_completion_mutation_preflight.jsonl"
...
def _build_lifecycle_state_status_completion_mutation_preflight_map(
    completion_audit_report: Mapping[str, Any], active_env: Mapping[str, str]
) -> Dict[str, Any]:
    source_map = _as_dict(completion_audit_report.get("generic_supervised_paper_lifecycle_completion_audit_execution_map", {}))
    source_gate = _as_dict(source_map.get("lifecycle_completion_audit_execution_gate_model", {}))

    audit_report_ready = completion_audit_report.get("generic_supervised_paper_lifecycle_completion_audit_execution_ready") is True
    audit_model_ready = completion_audit_report.get("generic_lifecycle_completion_audit_execution_model_ready") is True
    audit_patch_ready = completion_audit_report.get("generic_lifecycle_completion_audit_execution_patch_ready") is True
    audit_gate_model_ready = source_gate.get("lifecycle_completion_audit_execution_gate_model_ready") is True
    audit_conditions_satisfied = source_gate.get("lifecycle_completion_audit_execution_conditions_satisfied") is True
    audit_execution_allowed = source_gate.get("lifecycle_completion_audit_execution_allowed") is True

    broker_submit_receipt_available = completion_audit_report.get("broker_submit_receipt_available") is True
    broker_close_receipt_available = completion_audit_report.get("broker_close_receipt_available") is True
    real_close_execution_available = completion_audit_report.get("real_close_execution_available") is True
    paper_position_closed = completion_audit_report.get("paper_position_closed") is True
    paper_realized_pnl_reconciliation_ready = completion_audit_report.get("paper_realized_pnl_reconciliation_ready") is True
    realized_pnl_written = completion_audit_report.get("realized_pnl_written") is True
    paper_final_audit_ready = completion_audit_report.get("paper_final_audit_ready") is True
    paper_postmortem_ready = completion_audit_report.get("paper_postmortem_ready") is True
    paper_lifecycle_completion_ready = completion_audit_report.get("paper_lifecycle_completion_ready") is True
    final_audit_runtime_executed = completion_audit_report.get("final_audit_runtime_executed") is True
    postmortem_runtime_executed = completion_audit_report.get("postmortem_runtime_executed") is True
    lifecycle_completion_audit_runtime_executed = completion_audit_report.get("generic_lifecycle_completion_audit_execution_allowed") is True
    lifecycle_state_complete = completion_audit_report.get("lifecycle_state_complete") is True
    ...
    lifecycle_completion_gate = _operator_gate(
        active_env,
        LIFECYCLE_COMPLETION_ENABLE_ENV,
        LIFECYCLE_COMPLETION_CONFIRMATION_ENV,
        LIFECYCLE_COMPLETION_CONFIRMATION_SENTINEL,
    )
    state_status_mutation_gate = _operator_gate(
        active_env,
        STATE_STATUS_MUTATION_ENABLE_ENV,
        STATE_STATUS_MUTATION_CONFIRMATION_ENV,
        STATE_STATUS_MUTATION_CONFIRMATION_SENTINEL,
    )
    runtime_values_present = source_gate.get("runtime_values_present") is True

    preflight_conditions_satisfied = all(
        (
            audit_report_ready,
            audit_model_ready,
            audit_patch_ready,
            audit_gate_model_ready,
            audit_conditions_satisfied,
    ...
    checks = [
        (audit_report_ready, "lifecycle_completion_audit_execution_report_not_ready"),
        (audit_model_ready, "lifecycle_completion_audit_execution_model_not_ready"),
        (audit_patch_ready, "lifecycle_completion_audit_execution_patch_not_ready"),
        (audit_gate_model_ready, "lifecycle_completion_audit_execution_gate_model_not_ready"),
        (audit_conditions_satisfied, "lifecycle_completion_audit_execution_conditions_not_satisfied"),
        (audit_execution_allowed, "lifecycle_completion_audit_execution_not_allowed"),
    ...
    blocked_reasons.append("explicit_lifecycle_state_status_completion_mutation_execution_patch_required")

    gate_model = {
        "lifecycle_state_status_completion_mutation_preflight_model_ready": True,
        "lifecycle_state_status_completion_mutation_preflight_gate_model_ready": True,
        "lifecycle_state_status_completion_mutation_conditions_satisfied": preflight_conditions_satisfied,
        "lifecycle_state_status_completion_mutation_allowed": False,
        ...
        "lifecycle_completion_audit_execution_report_required": True,
        "lifecycle_completion_audit_execution_report_ready": audit_report_ready,
        "lifecycle_completion_audit_execution_model_ready": audit_model_ready,
        "lifecycle_completion_audit_execution_patch_ready": audit_patch_ready,
        "lifecycle_completion_audit_execution_gate_model_ready": audit_gate_model_ready,
        "lifecycle_completion_audit_execution_conditions_satisfied": audit_conditions_satisfied,
        "lifecycle_completion_audit_execution_allowed": audit_execution_allowed,
        ...
        "runtime_values_required_before_state_status_completion_mutation": True,
        ...
        "lifecycle_state_status_completion_mutation_blocked_reasons": blocked_reasons,
    ...
    return {
        "mode": "generic_supervised_paper_lifecycle_state_status_completion_mutation_preflight_only",
        "preflight_only": True,
        "read_only": True,
        ...
        "preflight_source": "29.4.4u-43",
        "dry_run_order_intent_payload": _dry_run_order_intent_payload(source_map),
        "source_lifecycle_completion_audit_execution_map": source_map,
        "lifecycle_state_status_completion_mutation_preflight_patch": True,
        "lifecycle_state_status_completion_mutation_preflight_gate_model": gate_model,
        "lifecycle_state_status_completion_mutation_preflight_context": {
            "blocked_reason": "lifecycle_state_status_completion_mutation_preflight_blocked_until_complete_runtime_lifecycle_evidence_and_explicit_mutation_patch",
            ...
            "paper_lifecycle_completion_ready": paper_lifecycle_completion_ready,
            "lifecycle_state": completion_audit_report.get("lifecycle_state", "FLAT_LOCKED"),
            "lifecycle_state_complete": lifecycle_state_complete,
            "broker_submit_receipt_available": broker_submit_receipt_available,
            "broker_close_receipt_available": broker_close_receipt_available,
            "real_close_execution_available": real_close_execution_available,
        ...
        "required_future_controls_before_lifecycle_state_status_completion_mutation_execution": {
            "explicit_generic_lifecycle_state_status_completion_mutation_execution_patch_required": True,
        ...
        "stages": [{"stage": stage, "execution_allowed": False, "state_mutation_allowed": False} for stage in STATE_STATUS_COMPLETION_MUTATION_PREFLIGHT_STAGES],
    }


def run_generic_supervised_paper_lifecycle_state_status_completion_mutation_preflight(
    settings: Settings | None = None,
) -> Dict[str, Any]:
    settings = settings or Settings()
    data_path = settings.data_path
    active_env = _active_lsr_env()
    upstream_path = data_path / "lsr_v2_generic_supervised_paper_lifecycle_completion_audit_execution_report.json"
    ...
    state_status_map = _build_lifecycle_state_status_completion_mutation_preflight_map(upstream, active_env)
    gate = _as_dict(state_status_map.get("lifecycle_state_status_completion_mutation_preflight_gate_model", {}))
    ...
    upstream_diagnostic_count = _safe_int(upstream.get("lifecycle_completion_audit_execution_diagnostic_count"), 0)
    candidate_counts = _as_dict(upstream.get("candidate_detection_source_counts", {}))
    diagnostic_count = max(upstream_diagnostic_count, sum(_safe_int(v, 0) for v in candidate_counts.values()))
    if diagnostic_count <= 0:
        diagnostic_count = 1

    no_broker_submit = upstream.get("broker_submit_called_by_generic_lifecycle_completion_audit_execution") is not True
    no_broker_close = upstream.get("broker_close_called_by_generic_lifecycle_completion_audit_execution") is not True
    no_state_mutation = upstream.get("paper_state_modified_by_generic_lifecycle_completion_audit_execution") is not True
    no_status_mutation = upstream.get("paper_status_modified_by_generic_lifecycle_completion_audit_execution") is not True
    no_telegram = upstream.get("telegram_network_called_by_generic_lifecycle_completion_audit_execution") is not True
    no_scheduler = upstream.get("scheduler_started_by_generic_lifecycle_completion_audit_execution") is not True

    base_false = {key: False for key in FALSE_EXECUTION_KEYS}
    report: Dict[str, Any] = {
        **base_false,
        "prompt": PROMPT,
        ...
        "classification_labels": [
            "GENERIC_LSR_V2_SUPERVISED_PAPER_LIFECYCLE_STATE_STATUS_COMPLETION_MUTATION_PREFLIGHT",
            "LIFECYCLE_STATE_STATUS_COMPLETION_MUTATION_PREFLIGHT_ONLY",
            ...
            "NO_BROKER_CALL",
            "FAIL_CLOSED",
            "GENERIC_SUPERVISED_PAPER_LIFECYCLE_STATE_STATUS_COMPLETION_MUTATION_PREFLIGHT_READY",
        ],
        "generic_supervised_paper_lifecycle_state_status_completion_mutation_preflight_ready": True,
        "generic_lifecycle_state_status_completion_mutation_preflight_ready": True,
        ...
        "lifecycle_state_status_completion_mutation_preflight_diagnostic_count": diagnostic_count,
        "lifecycle_completion_audit_execution_diagnostic_available": upstream.get("lifecycle_completion_audit_execution_diagnostic_available") is True,
        "lifecycle_completion_audit_execution_diagnostic_count": upstream_diagnostic_count,
        "lifecycle_state_status_completion_mutation_preflight_contract_required_fields": contract_required,
        "lifecycle_state_status_completion_mutation_preflight_contract_present_fields": contract_present,
        "lifecycle_state_status_completion_mutation_preflight_contract_missing_fields": contract_missing,
        "lifecycle_state_status_completion_mutation_preflight_contract_shape_modelable": contract_shape_modelable,
        "generic_supervised_paper_lifecycle_state_status_completion_mutation_preflight_map": state_status_map,
        "blocked_until_explicit_lifecycle_state_status_completion_mutation_patch": list(BLOCKED_UNTIL_EXPLICIT_LIFECYCLE_STATE_STATUS_COMPLETION_MUTATION_PATCH),
        "blockers": [],
        ...
        "u43_lifecycle_completion_audit_execution_report_present": bool(upstream),
        "u43_lifecycle_completion_audit_execution_ready": upstream.get("generic_supervised_paper_lifecycle_completion_audit_execution_ready") is True,
        ...
        "lifecycle_completion_audit_runtime_executed": upstream.get("generic_lifecycle_completion_audit_execution_allowed") is True,
        ...
        "would_submit": False,
        "would_close": False,
        "orders_submitted_by_generic_lifecycle_state_status_completion_mutation_preflight": 0,
        "positions_opened_by_generic_lifecycle_state_status_completion_mutation_preflight": 0,
        "positions_closed_by_generic_lifecycle_state_status_completion_mutation_preflight": 0,
        "paper_state_modified_by_generic_lifecycle_state_status_completion_mutation_preflight": False,
        "paper_status_modified_by_generic_lifecycle_state_status_completion_mutation_preflight": False,
        "telegram_network_called_by_generic_lifecycle_state_status_completion_mutation_preflight": False,
        "scheduler_started_by_generic_lifecycle_state_status_completion_mutation_preflight": False,
        "broker_submit_called_by_generic_lifecycle_state_status_completion_mutation_preflight": False,
        "broker_close_called_by_generic_lifecycle_state_status_completion_mutation_preflight": False,
        "broker_submit_called_by_upstream_lifecycle_completion_audit_execution": not no_broker_submit,
        "broker_close_called_by_upstream_lifecycle_completion_audit_execution": not no_broker_close,
        "paper_state_modified_by_upstream_lifecycle_completion_audit_execution": not no_state_mutation,
        "paper_status_modified_by_upstream_lifecycle_completion_audit_execution": not no_status_mutation,
        "telegram_network_called_by_upstream_lifecycle_completion_audit_execution": not no_telegram,
        "scheduler_started_by_upstream_lifecycle_completion_audit_execution": not no_scheduler,
    ...
    if not (source_markers_present and contract_shape_modelable and report["all_execution_flags_fail_closed"]):
        report["status"] = "KEEP_DIAGNOSTIC"
        report["decision"] = KEEP_DIAGNOSTIC_DECISION
        blockers: List[str] = []
        if not source_markers_present:
            blockers.append("source_markers_missing")
        if not contract_shape_modelable:
            blockers.append("contract_shape_not_modelable")
        if not report["all_execution_flags_fail_closed"]:
            blockers.append("execution_flags_not_fail_closed")
...
def main() -> None:
    report = run_generic_supervised_paper_lifecycle_state_status_completion_mutation_preflight()
